...
function tVICE.getStreetNameAtCoord(v, w, H)
...
RegisterNetEvent("VICE:playClientNuiSound",function(aI, aJ, ap) -- use for cuffing and other sounds in areas
    local ax = tVICE.getPlayerCoords()
    if #(ax - aI) <= ap then
        SendNUIMessage({transactionType = aJ})
    end
end)
function tVICE.getNetId(aW, aX)
    if aX == nil then
        aX = ""
    end
    local aY = 0
...
function tVICE.getObjectId(a_, aX)
    if aX == nil then
        aX = ""
    end
    local aL = 0
...
function tVICE.getVehicleNameFromId(aZ)
    if aT[string.lower(aZ)] then
        return aT[string.lower(aZ)].name
    end
    return ""
end
function tVICE.getGarageNameFromId(aZ)
    return aT[string.lower(aZ)].garageType
end
function tVICE.getVehicleIdFromModel(s)
    return aU[s]
end
local a_ = math.rad
local b0 = math.cos
local b1 = math.sin
local b2 = math.abs
function tVICE.rotationToDirection(b3)
    local B = a_(b3.x)
    local D = a_(b3.z)
