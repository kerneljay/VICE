    ...
    window.addEventListener('message', function(event){
        ...
        if(event.data.openRadialMenu == true){
            if(event.data.type == "ped"){
                if (event.data.police == true) {
                    pedMenu = new RadialMenu({
                        ...
                        onClick     : function (item) {
                            $.post('https://vice/radialClick', JSON.stringify({
                                itemid: item.id,
                                entity: event.data.entityId
                            }));
                        }
                    });
                } else if (event.data.ukbf == true) {
                    pedMenu = new RadialMenu({
                        parent      : document.getElementById("centerRadial"),
                        size        : 500,
                        closeOnClick: true,
                        menuItems   : pedItemsUKBF,
                        onClick     : function (item) {
                            $.post('https://vice/radialClick', JSON.stringify({
                                itemid: item.id,
                                entity: event.data.entityId
                ...
                } else if (event.data.aa == true) {
                    vehicleMenu = new RadialMenu({
                        parent      : document.getElementById("centerRadial"),
                        size        : 500,
                        closeOnClick: true,
                        menuItems   : vehicleItemsAA,
                        onClick     : function (item) {
                            $.post('http://vice/radialClick', JSON.stringify({
                                itemid: item.id,
                                entity: event.data.entityId
                            }));
                        }
        ...
        if(event.data.closeRadialMenu == true){
            ...
            $.post('https://vice/radialDisablenuifocus', JSON.stringify({
                nuifocus: false
            }));
