$(document).ready(function(){
  ...
  $('input').change(function(){
    ...
    $.post('https://vice/updateSkin', JSON.stringify({
      value: false,
      // Face
  ...
  $(document).keypress(function(e) {
    ...
    if(e.which == 113){ // A pressed
      $.post('https://vice/rotaterightheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 101){ // E pressed
      $.post('https://vice/rotateleftheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 119 ){ // W pressed
      $.post('https://vice/zoomin', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 115){ // S pressed
      $.post('https://vice/zoomout', JSON.stringify({
        value: 10
      }));
    }
  });
  ...
  $('#skin-tabs skin-label').on('click', function(e){
    ...
    $.post('https://vice/zoom', JSON.stringify({
...
function leftWristComponent() {
    $.post('https://vice/updateLeftWristComponent', JSON.stringify({
...
function leftWristTexture() {
    ...
    $.post('https://vice/updateLeftWristTexture', JSON.stringify({
        componentID: $('.montre .active').attr('data'),
...
function rightWristComponent() {
    $.post('https://vice/updateRightWristComponent', JSON.stringify({
...
function rightWristTexture() {
    ...
    $.post('https://vice/updateRightWristTexture', JSON.stringify({
        componentID: $('.montre-right .active').attr('data'),
