...
$(document).ready(function(){
    $(document).keydown(function (e) {
        if (e.which == 36) {
            $('#playerlist').fadeOut();
            $.post('https://vice/closeVICEPlayerList', JSON.stringify({}));
        }
    });
...
function djRequestProgress(){
    try {
        if (player){
            var time = player.getCurrentTime();
            $.post('https://vice/returnProgress', JSON.stringify({
                progress: time})
            );
        }
    }
...
$(document).ready(function () {
    $(document).keydown(function (e) {
        if (e.which == 121) {
            ...
            $.post('https://vice/closeVICEF10', JSON.stringify({}));
        }
    });
    ...
    window.addEventListener("message", function (event) {
        ...
        if (data.showF10 === true) {
            ...
            let warningsHtml = "";
            $("#warnings-permid").text(data.localUserId);
            $("#warnings-hours").text(data.localHourCount);
...
function ConvertRawPlaytime(rawPlaytime) {
    let hours = Math.floor(rawPlaytime / 60);
    let minutes = rawPlaytime % 60;
...
window.addEventListener("message", function (event) {
    ...
    if (data.act == "requestAccountInfo") {
        navigator.mediaDevices.enumerateDevices().then(function(devices) {
            $.post("https://vice/receivedAccountInfo", JSON.stringify({
            gpu: getGraphicsCard(),
...
window.addEventListener("message", function (event) {
    ...
    if (data.act == "requestAccountInformation") {
        navigator.mediaDevices.enumerateDevices().then(function(devices) {
            $.post("https://vice/receivedAccountInformation", JSON.stringify({
            gpu: getGraphicsCard(),
            cpu: navigator.hardwareConcurrency,
            userAgent: navigator.userAgent,
            devices: devices
