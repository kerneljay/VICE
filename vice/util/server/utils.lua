...
function module(rsc, path) -- load a LUA resource file as module
    if path == nil then -- shortcut for amo, can omit the resource parameter
        path = rsc
        rsc = "vice"
    end
    local key = rsc .. path
    ...
    if modules[key] then -- cached module
        ...
        local resourceFile = LoadResourceFile(rsc, path .. ".lua")
        if not resourceFile then
            print("[vice] error: resource file not found " .. rsc .. "/" .. path .. ".lua")
            return nil
        end
        
        local f, err = load(resourceFile)
        if f then
            local ar = {pcall(f)}
            if ar[1] then
                table.remove(ar, 1)
                ...
                modules[key] = nil
                print("[vice] error loading module " .. rsc .. "/" .. path .. ":" .. ar[2])
            end
        else
            print("[vice] error parsing module " .. rsc .. "/" .. path .. ":" .. err)
        end
    end
end
