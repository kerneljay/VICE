RMenu.Add('VICEclothing','mainMenu',RageUI.CreateMenu("","~s~C~w~lothing Store",VICE.getRageUIMenuWidth(),VICE.getRageUIMenuHeight(),"menus","clothes"))
RMenu.Add('VICEclothing','changePed',RageUI.CreateSubMenu(RMenu:Get('VICEclothing','mainMenu'),"","~s~C~w~hange Gender"))
RMenu.Add('VICEclothing','changeClothing',RageUI.CreateSubMenu(RMenu:Get('VICEclothing','mainMenu'),"","~s~C~w~hange your Clothes"))
RMenu.Add('VICEclothing','clearProps',RageUI.CreateSubMenu(RMenu:Get('VICEclothing','mainMenu'),"","~s~C~w~lear Props"))
...
local r = {
    ...
    vector3(3072.0727539062,-3723.2316894531,53.164859771729),
    vector3(277.69772338867, -1335.5568847656, 24.537786483765),
    vector3(-2671.8981933594,-1489.1380615234,3.9521214962006), -- VICE Island
}
local s=1
local t=false
local u=false
local v={
    ...
    ["Mask"]={offset=vector3(0.0,0.5,0.68),point=vector3(0.0,0.0,0.68),fov=50.0},
    ...
    while true do 
        ...
        for H=1,#r do 
            if#(G-r[H])<=2.5 and not t then 
                ...
                h=true
                RageUI.Visible(RMenu:Get('VICEclothing','mainMenu'),true)
            elseif s==H then 
                ...
                if not RageUI.Visible(RMenu:Get('VICEclothing','changeClothing'))then 
                    F()
                end 
            end 
        end
...
RageUI.CreateWhile(1.0, true, function()
    if not n then
            ...
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                ...
                RageUI.Button('~y~Wardrobe',nil,{RightLabel=">>>"},true,function(L,M,N)
                    if N then 
                        if tVICE.isPlusClub()or tVICE.isPlatClub()then
                            ...
                            VICE.notify("~y~Available @ store.vicestudios.co.uk")
                        end 
                    end 
                end)
                if PlayerIsInGang then
                    RageUI.Button('Apply Gang Outfit', 'Applys the outfit set by the Gang Leader.', {RightLabel="→→→"}, true, function(L,M,N)
                        if N then
                                TriggerServerEvent("VICE:applyGangFit", gangID)
                            end
                        end)
                    end
                RageUI.Button('Apply Outfit Code','Applys an outfit code given by a player with ~y~VICE Plus~w~ or ~y~VICE Platinum~w~.',{RightLabel=">>>"},true,function(L,M,N)
                    if N then 
                        tVICE.clientPrompt("Outfit Code:","",function(Q)
                            local R = string.gsub(Q, "%s+", "")
                            if #R == 5 then
                                TriggerServerEvent("VICE:applyOutfitCode", string.upper(R))
                            else
                ...
                end,RMenu:Get('VICEclothing','clearProps'))
            end)
        end
        if RageUI.Visible(RMenu:Get('VICEclothing', 'changePed')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                RageUI.Button('MP Male','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            VICE.loadCustomisationPreset("DefaultMale")
                        else
                            VICE.notify("~r~Cannot change gender, you're dead.")
                        end
                    end 
                end,function()
                end)
                RageUI.Button('MP Female','WARNING: Resets all your Body Options',{},true,function(L,M,N)
                    if N then 
                        if GetEntityHealth(PlayerPedId()) > 102 then
                            VICE.loadCustomisationPreset("DefaultFemale")
                        else
                            VICE.notify("~r~Cannot change gender, you're dead.")
                        end
                    end 
        ...
        if RageUI.Visible(RMenu:Get('VICEclothing', 'changeClothing')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
                m=true
                if h then 
                    for H=1,#a,1 do 
                        ...
                        RageUI.List(P.name,get_drawables(P.type,P.index),a[H].currentListIndex,"Current Texture Index: "..k.textureN.."/"..j-1,{},true,function(L,M,N,Q)
                            ...
                            if N then 
                                ...
                                if k.index==2 then 
                                    ...
                                    TriggerServerEvent("VICE:saveClothingHairData",GetPedDrawableVariation(VICE.getPlayerPed(),2),GetPedHairColor(VICE.getPlayerPed()))
...
function current_clothing(R,i,S)
    if R=="prop"then 
        if S then 
            if GetPedPropTextureIndex(VICE.getPlayerPed(),i)~=nil then 
                return GetPedPropTextureIndex(VICE.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedPropIndex(VICE.getPlayerPed(),i)~=nil then 
                if GetPedPropIndex(VICE.getPlayerPed(),i)==-1 then 
                    return GetPedPropIndex(VICE.getPlayerPed(),i)+2 
                else 
                    return GetPedPropIndex(VICE.getPlayerPed(),i)+1 
                end 
            else 
                return 0 
            end 
    ...
    elseif R=="drawable"then 
        ...
        elseif S then 
            if GetPedTextureVariation(VICE.getPlayerPed(),i)~=nil then 
                return GetPedTextureVariation(VICE.getPlayerPed(),i)
            else 
                return 0 
            end 
        else 
            if GetPedDrawableVariation(VICE.getPlayerPed(),i)~=nil then 
                return GetPedDrawableVariation(VICE.getPlayerPed(),i)+1 
            else 
                return 0 
            end 
        end 
...
function set_clothing(T,U,i,V)
    ...
    if T=="drawable" then 
        ...
        if U==11 and i-1==200 or i-1==398 then
            VICE.notify("~r~You are unable to wear this shirt.")
            return 
        end
...
function get_textures(T,R,i)
    if T=="drawable"then 
        ...
        j=GetNumberOfPedTextureVariations(VICE.getPlayerPed(),R,i-1)
        return j 
    else 
        j=GetNumberOfPedPropTextureVariations(VICE.getPlayerPed(),R,i-1)
        return j 
    end 
end
local function Y()
...
Citizen.CreateThread(function()
    while true do 
        if k.index~=nil and RageUI.Visible(RMenu:Get('VICEclothing','changeClothing'))then 
...
AddEventHandler("VICE:onClientSpawn",function(D, E)
    if E then
		for a7,a8 in pairs(r)do 
            tVICE.addBlip(a8.x,a8.y,a8.z,73,3,"Clothing Store",0.7)
            tVICE.addMarker(a8.x,a8.y,a8.z,0.6,0.6,0.6,10,255,81,170,50,9,false,false,true,"dp_clothing","top",90.0,90.0,0.0)
        end 
	end
end)
