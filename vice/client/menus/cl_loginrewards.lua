...
RMenu.Add("loginrewards", "mainmenu", RageUI.CreateMenu("", "VICE REWARDS", VICE.getRageUIMenuWidth(), VICE.getRageUIMenuHeight(), "menus", "loginrewards"))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get("loginrewards", "mainmenu")) then
        RageUI.DrawContent({header = true, glare = false, instructionalButton = false}, function()
            ...
            if VICE.getgrindBoost() > 1.1 then
                RageUI.Separator(string.format("~g~%dx grind boost.", VICE.getgrindBoost()))
            end
            if b == 0 then
            ...
            RageUI.Button(string.format("Day %d", t), g(s), {RightLabel = n(VICE.getgrindBoost()*t, not s)}, s, function(p, p, u)
                if u then
                    TriggerServerEvent("VICE:he") 
                    b = b + 1
            ...
            for j = t + 1, t + v do
                RageUI.Button(string.format("Day %d", j), i(j, s), {RightLabel = n(VICE.getgrindBoost()*j, false)}, false, function()
                end)
            end
            if t < 55 then
...
RegisterNetEvent("VICE:setDailyRewardInfo", function(w, x, y)
    b = w
    c = GetGameTimer() + (86400 - (y - x)) * 1000
    if w == 0 then
        Citizen.Wait(20000)
        VICE.notify("~y~You have not started your daily reward. Do /dailyrewards to start now!")
    end
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        if r() and b > 0 then
            VICE.notify("~g~You can claim your next daily reward at /dailyrewards!")
            break
        end
        Citizen.Wait(6000)
    end
