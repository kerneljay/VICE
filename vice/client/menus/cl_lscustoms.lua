...
RMenu.Add("lscustoms", "repair", RageUI.CreateMenu("", "Repair Vehicle", VICE.getRageUIMenuWidth(), VICE.getRageUIMenuHeight(), "menus", "lscustoms"))
RMenu.Add("lscustoms", "mainmenu", RageUI.CreateMenu("", "Los Santos Customs", VICE.getRageUIMenuWidth(), VICE.getRageUIMenuHeight(), "menus", "lscustoms"))
local b = {}
local function c(d)
	...
	if d.name ~= "Main Menu" then
		RMenu.Add("lscustoms", d.menu, RageUI.CreateSubMenu(RMenu:Get("lscustoms", e), "", d.name, VICE.getRageUIMenuWidth(), VICE.getRageUIMenuHeight()))
...
local function a0(d)
	for a1 = - 1, GetNumVehicleMods(j, d.modType) - 1 do
		...
		RageUI.Button(a2, nil, a6, true, function(a7, a8, a9)
			...
			if a9 then
				if a5 == true then
					VICE.notify("~r~You have already applied this mod")
				elseif a5 == false then
					TriggerServerEvent("VICE:setActiveModList", k, a.categoryToIndentifier[d], a1)
					V()
				else
					TriggerServerEvent("VICE:purchaseModList", k, a.categoryToIndentifier[d], a1)
					V()
				end
			end
		end)
        ...
        RageUI.Button(a2 > 0 and string.format("%s %d", d.prefix, a2) or "Stock",nil,aa,true,function(ab, ac, ad)
            ...
            if ad then
                ...
                elseif a9 == false then
                    TriggerServerEvent("VICE:setActiveModList", k, a.categoryToIndentifier[d], a2)
                    V()
                else
                    TriggerServerEvent("VICE:purchaseModList", k, a.categoryToIndentifier[d], a2)
                    V()
                end
            end
        end)
		...
		RageUI.Button(ah.name, nil, a6, true, function(a7, a8, a9)
			...
			if a9 then
				if a5 == true then
					VICE.notify("~r~You have already applied this mod")
				elseif a5 == false then
					TriggerServerEvent("VICE:setActiveStaticList", k, a.categoryToIndentifier[d], Z)
					ar(d)
				else
					TriggerServerEvent("VICE:purchaseStaticList", k, a.categoryToIndentifier[d], Z)
					ar(d)
				end
			end
		end)
		...
		RageUI.Button(ah.name, nil, a6, true, function(a7, a8, a9)
			if a9 then
				if aA == d.ownedValue then
					...
					TriggerServerEvent("VICE:purchaseStaticValueList", k, a.categoryToIndentifier[d], Z)
					V()
				end
			end
		end)
...
local function aB(d)
		...
		RageUI.Button(string.format("%s%s", d.indexPrefix, Z), "", {RightLabel = tostring(aD)}, true, function(a7, a8, a9)
			if a8 then
				if IsControlJustPressed(0, 178) then
					TriggerServerEvent("VICE:deleteValueInputList", k, a.categoryToIndentifier[d], Z)
				end
			end
			if a9 then
				tVICE.clientPrompt(tostring(d.inputTitle), "", function(aE)
					local aF = aC[Z]
					local aG = nil
					if d.valueType == "number" then
						...
						if aH then
							...
							VICE.notify("~r~Could not parse number.")
						end
					else
						aG = aE
					end
					if aG and aG ~= aF then
						TriggerServerEvent("VICE:setValueInputList", k, a.categoryToIndentifier[d], Z, aG)
					end
				end)
			end
		end)
	...
	RageUI.Button(d.buyTitle, "", {RightLabel = "£" .. getMoneyStringFormatted(d.price)}, true, function(a7, a8, a9)
		if a9 then
			TriggerServerEvent("VICE:purchaseValueInputList", k, a.categoryToIndentifier[d])
		end
	end)
end
...
local function aI(d)
	RageUI.Button(d.name, isButtonEnabled(d.disabled) and (d.description or "") or "~r~This feature is currently disabled", isButtonEnabled(d.disabled) and {RightLabel = "→→→"} or {}, isButtonEnabled(d.disabled), function(a7, a8, a9)
		if a9 then
			TriggerEvent("VICE:lsCustomsOpenExternalMenu", d.menuType, j, k, h)
		end
	...
	if RageUI.Visible(RMenu:Get('lscustoms', 'repair')) then
		RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
			RageUI.Button("Repair Vehicle", nil, {RightLabel = "£1,000"}, true, function(a7, a8, a9)
				if a9 then
					TriggerServerEvent("VICE:lscustomsRepairVehicle")
				end
			end)
		end)
	end
...
local function aQ()
	k = VICE.getVehicleIdFromHash(GetEntityModel(j))
	if k == nil then
		VICE.notify("~r~Could not identify the vehicle you are in.")
		i = nil
		j = 0
		return
	end
	TriggerServerEvent("VICE:getBoughtUpgrades", k)
	while not h do
		Citizen.Wait(0)
	end
	DisplayRadar(false)
	SetPlayerControl(PlayerId(), false, 0)
	TriggerServerEvent("VICE:setCustomsGarageStatus", i.index, true)
	DoScreenFadeOut(500)
...
local function aX()
	...
	tVICE.applyModsOnVehicle(h, k, j)
	local aY = i.driveOut
	...
	TriggerServerEvent("VICE:setCustomsGarageStatus", i.index, false)
	i = nil
...
local function a_(b0)
	local b1, b2 = VICE.getPlayerVehicle()
	if b1 == 0 or not b2 or i then
		return
	end
	local b3 = GetEntityModel(b1)
	if b0.isLocked then
		VICE.DrawText(0.5, 0.8, "~r~Locked, please wait~w~", 1.0, 4, 0)
	elseif b0.type == "plane" and not IsThisModelAPlane(b3) then
		VICE.DrawText(0.5, 0.8, "~r~You must be a in a plane to use this~w~", 1.0, 4, 0)
	elseif b0.type == "boat" and not IsThisModelABoat(b3) then
		VICE.DrawText(0.5, 0.8, "~r~You must be a in a boat to use this~w~", 1.0, 4, 0)
	else
		VICE.DrawText(0.5, 0.8, "Press ~b~ENTER~w~ to enter ~b~" .. b0.name .. "~w~", 1.0, 4, 0)
...
Citizen.CreateThread(function()
	for Z, b0 in pairs(a.garages) do
		b0.index = Z
		VICE.createArea("lscustoms_" .. tostring(Z), b0.driveIn.position, 5.0, 6.0, function()
		end, function()
		end, a_, b0)
		tVICE.addBlip(b0.inside.position.x, b0.inside.position.y, b0.inside.position.z, 72, nil, "LS Customs")
	end
	VICE.createThreadOnTick(b5)
end)
RegisterNetEvent("VICE:setCustomsGarageStatus", function(b6, b7)
	a.garages[b6].isLocked = b7
end)
RegisterNetEvent("VICE:syncCustomsGarageStatus", function(b8)
	for Z, f in pairs(b8) do
		a.garages[Z].isLocked = true
	end
end)
RegisterNetEvent("VICE:gotBoughtUpgrades", function(b9)
	h = b9
end)
RegisterNetEvent("VICE:setSpecificOwnedUpgrade", function(P, Q)
	h[P] = Q
end)
RegisterNetEvent("VICE:lscustomsRepairVehicle", function()
	SetVehicleFixed(j)
	RageUI.Visible(RMenu:Get("lscustoms", "repair"), false)
	RageUI.Visible(RMenu:Get("lscustoms", "mainmenu"), true)
end)
function VICE.isInsideLsCustoms()
	return i ~= nil
end
