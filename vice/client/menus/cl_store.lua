...
RMenu.Add(
    "store",
    "mainmenu",
    RageUI.CreateMenu("", "Inventory", VICE.getRageUIMenuWidth(), VICE.getRageUIMenuHeight(), "menus", "store")
)
RMenu.Add(
    "store",
    "info",
...
RMenu.Add(
    "store",
    "redeem",
    RageUI.CreateSubMenu(RMenu:Get("store", "info"), "", "Redeem", VICE.getRageUIMenuWidth(), VICE.getRageUIMenuHeight())
)
RMenu.Add(
    "store",
    "vehicleList",
    RageUI.CreateSubMenu(
        ...
        "Vehicles",
        VICE.getRageUIMenuWidth(),
        VICE.getRageUIMenuHeight(), "menus", "vice_minigames_choosevehicle"))
RMenu.Add(
    "store",
    "vehicleListInner",
    RageUI.CreateSubMenu(
...
local function a2(K)
    l = true
    TriggerServerEvent("VICE:setInVehicleTestingBucket", true)
    z = VICE.getPlayerCoords()
    w = 1
    ...
    m = VICE.spawnVehicle(K, a4.x, a4.y, a4.z, a5, true, false, false)
    n = GetGameTimer()
end
local function a6(a7)
    local a8 = {}
    for a9, aa in pairs(a7) do
        if a9 and aa and a.items and a.items[aa] then
            ...
            RageUI.DrawContent(
                ...
                function()
                    if i then
                        ...
                        RageUI.Separator("~r~You do not have a rank.")
                    end
                    if b and next(b) then
                        for L, a9 in a6(b) do
                            if L and a9 then
                                RageUI.ButtonWithStyle(
                                    L,
                        ...
                        RageUI.Separator("~r~No items in store")
                    end
                end
            )
        end
                ...
                function()
                    ...
                    if U.manuallyRedeemable then
                        RageUI.ButtonWithStyle(
                            ...
                            function(X, Y, Z)
                                if Z then
                                    TriggerServerEvent("VICE:getStoreLockedVehicleCategories")
                                end
                            end,
                            RMenu:Get("store", "redeem")
                        )
                            ...
                            function(X, Y, Z)
                                if Z then
                                    if isInGreenzone then
                                        TriggerServerEvent("VICE:startSellStoreItem", d)
                                    else
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(X, Y, Z)
                            if Z then
                                tVICE.CopyToClipBoard(d)
                                j = true
                                TriggerEvent("VICE:showNotification",
                                {
                                    text = d.." Copied To Clipboard.",
                                    height = "200px",
                                    width = "auto",
                                    colour = "#FFF",
                                    background = "#32CD32",
                ...
                function()
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(X, Y, Z)
                            if Z then
                                if T(U) then
                                    TriggerServerEvent("VICE:redeemStoreItem", d, e)
                                else
                ...
                function()
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(X, Y, Z)
                            if Z then
                                if
                                    VICE.canAnim() and VICE.getPlayerCombatTimer() == 0 and VICE.getPlayerVehicle() == 0 and
                                        not VICE.isPlayerInRedZone() and
                                        VICE.getPlayerBucket() == 0
                                 then
                                    RageUI.Visible(RMenu:Get("store", "vehicleSelection"), false)
                                    a2(g)
                                else
...
RMenu.Add(
    "vehicletesting",
    "mainmenu",
    RageUI.CreateMenu("", "Main Menu", VICE.getRageUIMenuWidth(), VICE.getRageUIMenuHeight(), "menus", "store")
)
RMenu.Add(
    "vehicletesting",
    "extras",
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(X, Y, Z)
                            if Z then
                                ...
                                TriggerServerEvent("VICE:setInVehicleTestingBucket", false)
                                RageUI.Visible(RMenu:Get("vehicletesting", "mainmenu"), false)
...
local function az()
    if l then
        ...
        local vehicle, aA = VICE.getPlayerVehicle()
...
function VICE.isInStoreTesting()
    return l
end
RegisterNetEvent(
    "VICE:storeDrawEffects",
    function()
        VICE.loadPtfx("scr_xs_celebration")
        VICE.loadPtfx("scr_rcpaparazzo1")
        for I = 1, 4 do
...
RegisterNetEvent(
    "VICE:setStoreLockedVehicleCategories",
    function(aB)
        c = aB
    end
)
RegisterNetEvent("VICE:OpenStoreMenu")
AddEventHandler(
    "VICE:OpenStoreMenu",
    function()
...
RegisterCommand(
    "store",
    function()
        TriggerServerEvent("VICE:OpenStore")
    end
)
RegisterNetEvent(
    "VICE:setStoreRankName",
    function(aC)
        i = aC
    end
)
