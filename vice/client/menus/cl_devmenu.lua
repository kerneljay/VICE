RMenu.Add(
    "devmenu",
    "main",
    RageUI.CreateMenu("", "~s~D~w~eveloper Tools", VICE.getRageUIMenuWidth(), VICE.getRageUIMenuHeight(),"menus","devtools"))
RMenu.Add(
    "devmenu",
    "animations",
            ...
            RageUI.DrawContent(
                ...
                function()
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                tVICE.clientPrompt(
                                    "Customization [JSON]",
                                    json.encode(tVICE.getCustomization()),
                                    function()
                                    end
                                )
                            end
                        ...
                        function(A, B, C)
                            if C then
                                tVICE.clientPrompt(
                                    ...
                                    "",
                                    function(D)
                                        tVICE.setCustomization(json.decode(D))
                                    end
                                )
                            end
                        end
                        ...
                        function(A, B, C)
                            if C then
                                ...
                                tVICE.clientPrompt(
                                    ...
                                    "",
                                    function(G)
                                        local H = VICE.loadModel(tonumber(G))
                                        if H then
                                            local I =
                                                GetEntityCoords(PlayerPedId()) +
                                                GetEntityForwardVector(PlayerPedId()) * 2.0
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                TriggerEvent("VICE:toggleTrafficMenu")
                            end
                        end
                    )
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                local J = VICE.loadModel("a_f_m_beach_01")
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                local J = VICE.loadModel("s_m_m_pilot_02")
                                local K = VICE.getPlayerCoords()
                                CreatePed(1, J, K.x, K.y, K.z, 0.0, true, true)
                                SetModelAsNoLongerNeeded(J)
                            end
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                ...
                                VICE.notify("~y~Triggered clear all")
                            end
                        end
                    )
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                TriggerServerEvent('VICE:CleanVeh')
                                VICE.notify("~y~Triggered clear vehicles")
                            end
                        end
                    )
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                TriggerServerEvent('VICE:CleanObj')
                                VICE.notify("~y~Triggered clear objects")
                            end
                        end
                    )
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                TriggerServerEvent('VICE:CleanPed')
                                notify("~y~Triggered clear peds")
                            end
                        end
                    )
                    RageUI.Button(
                                ...
                                tVICE.clientPrompt(
                                    ...
                                    function(P)
                                        if P and P ~= "" then
                                            ...
                                            tVICE.giveWeapons({[P] = {ammo = 250}}, false,decorpasskey)
                                            VICE.notify("~o~Spawned weapon:\n ~b~" .. P .. "")
                                            TriggerServerEvent("VICE:logWeaponSpawn", 'weapon-logs', "VICE Weapon Logs", P)
                                        end
                                    end
                                )
                            end
            ...
            RageUI.DrawContent(
                ...
                function(U)
                    RageUI.ButtonWithStyle(
                        ...
                        function(A, B, C)
                            if C then
                                tVICE.clientPrompt("Prop Name: ","",function(T)
                                    l = T
                                end)
                            end
                        end
...
function tVICE.gridPositionSaver(aa, L, M)
    VICE.notify("pick 1st coord, press Y when done")
    ...
    Wait(250)
    VICE.notify("pick 2nd coord, press Y when done")
    while not IsControlJustPressed(0, 246) do
        Wait(0)
    end
    local ac = GetEntityCoords(PlayerPedId())
    Wait(250)
    VICE.notify("pick 3rd coord, press Y when done")
    while not IsControlJustPressed(0, 246) do
        Wait(0)
    end
    ...
    for c = 0, M - 1, 1 do
        for af = 0, L - 1, 1 do
            ...
            VICE.loadModel("a_m_m_fatlatin_01")
            local aj = VICE.spawnVehicle(aa, ai.x, ai.y, ai.z, heading, false, false, false)
    ...
    function()
        while true do
            ...
            if a then
                ...
                VICE.DrawText(0.32, 0.00, "~r~X: ~s~" .. roundx, 0.4)
                VICE.DrawText(0.38, 0.00, "~r~Y: ~s~" .. roundy, 0.4)
                VICE.DrawText(0.445, 0.00, "~r~Z: ~s~" .. roundz, 0.4)
                heading = GetEntityHeading(ak)
                roundh = tonumber(string.format("%.2f", heading))
                VICE.DrawText(0.50, 0.00, "~r~H:~s~ " .. roundh, 0.4)
                local al, am, an = table.unpack(GetEntityRotation(PlayerPedId(), 1))
                VICE.DrawText(0.38, 0.03, "~r~RX: ~s~" .. tonumber(string.format("%.2f", al)), 0.4)
                VICE.DrawText(0.44, 0.03, "~r~RY: ~s~" .. tonumber(string.format("%.2f", am)), 0.4)
                VICE.DrawText(0.495, 0.03, "~r~RZ: ~s~" .. tonumber(string.format("%.2f", an)), 0.4)
                speed = GetEntitySpeed(PlayerPedId())
                rounds = tonumber(string.format("%.2f", speed))
                VICE.DrawText(0.40, 0.92, "~r~Player Speed: ~s~" .. rounds, 0.4)
                health = GetEntityHealth(PlayerPedId())
                VICE.DrawText(0.40, 0.95, "~r~Player Health: ~s~" .. health, 0.4)
                camRotX = GetGameplayCamRot(2).x
                VICE.DrawText(0.36, 0.88, "~r~CR X: ~s~" .. tonumber(string.format("%.2f", camRotX)), 0.4)
                camRotY = GetGameplayCamRot(2).y
                VICE.DrawText(0.44, 0.88, "~r~CR Y: ~s~" .. tonumber(string.format("%.2f", camRotY)), 0.4)
                camRotZ = GetGameplayCamRot(2).z
                VICE.DrawText(0.51, 0.88, "~r~CR Z: ~s~" .. tonumber(string.format("%.2f", camRotZ)), 0.4)
                ...
                if IsPedInAnyVehicle(PlayerPedId(), 1) then
                    ...
                    VICE.DrawText(0.015, 0.76, "~r~Engine Health: ~s~" .. vehenground, 0.4)
                    VICE.DrawText(0.015, 0.73, "~r~Body Health: ~s~" .. vehbodround, 0.4)
                    VICE.DrawText(
                        0.015,
                        0.70,
                        "~r~Vehicle Fuel: ~s~" ..
...
function tVICE.createDebugObject(Q)
    local aX = VICE.loadModel(Q)
...
local propretriever = false
local lastAimedEntity = nil
RegisterCommand("propretriever", function(source, args, rawCommand)
    if not VICE.isDev() then return end
    local playerPed = PlayerPedId()
    local rayGunHash = GetHashKey("WEAPON_RAYPISTOL")
    if propretriever then
        RemoveWeaponFromPed(playerPed, rayGunHash)
        propretriever = false
    else
        propretriever = true
        GiveWeaponToPed(playerPed, rayGunHash, 1, false, true)
        SetCurrentPedWeapon(playerPed, rayGunHash, true)
    end
end, false)
        ...
        if propretriever then
            ...
            if IsControlPressed(0, 25) then
                ...
                if entity and DoesEntityExist(entity) then
                    ...
                    local propName = GetEntityModelName(model)
                    DrawText3D(GetEntityCoords(entity), propName)
                else
                    lastAimedEntity = nil
                end
            end
            if IsControlJustPressed(0, 201) and lastAimedEntity then
                local model = GetEntityModel(lastAimedEntity)
                local modelText = "Prop: " .. tostring(model)
                tVICE.CopyToClipBoard(tostring(modelText))
            end
        end
    end
end)
...
--     Wait(100) 
--     GiveWeaponToPed(PlayerPedId(), GetHashKey("WEAPON_APPISTOL"), 250, false, true)
-- end)

local function getEntityPlayerIsAimingAt(player)
...
local function GetEntityModelName(model)
    local displayName = GetDisplayNameFromVehicleModel(model)
    if displayName == "CARNOTFOUND" then
        return "Prop: " .. tostring(model)
    else
        return displayName
    end
end

RegisterCommand("pftx", function(source, args, rawCommand)
    tVICE.clientPrompt("Enter Particle and dict:","scr_env_agency3b_smoke || core - https://www.vespura.com/fivem/particle-list/",function(K)
        if K and K ~= "" then
            K = stringsplit(K," || ")
            local pfx = K[1]
            local I = K[2] or "core"
            VICE.loadPtfx(I)
            UseParticleFxAsset(I)
            UseParticleFxAssetNextCall(I)
            local q = GetEntityCoords(PlayerPedId())
            local t=StartParticleFxLoopedAtCoord(pfx,q.x,q.y,q.z,0.0,0.0,0.0,2.0,false,false,false,false)
            Citizen.Wait(5000)
            StopParticleFxLooped(t,false)
        end
    end)
end)

local IsBirdOn = false
local savedBirdOutfit = nil

RegisterCommand("bird", function(source, args, rawCommand)
    if not VICE.isDev() then
        return
    end
    IsBirdOn = not IsBirdOn
    if not IsBirdOn then
        VICE.enablePunching(false)
        if savedBirdOutfit then
            tVICE.setCustomization(savedBirdOutfit)
            savedBirdOutfit = nil
        end
    ...
    -- Save current outfit before transforming
    savedBirdOutfit = tVICE.getCustomization()
    SetPedMoveRateOverride(PlayerId(),10.0)
    SetRunSprintMultiplierForPlayer(PlayerId(),1.49)
    VICE.notify("~b~Bird Mode Activated")
    VICE.enablePunching(true)
    local birdJson = '{"p5":[-1,0],"13":[0,0,0],"p3":[-1,0],"15":[0,0,113],"11":[0,0,0],"p1":[-1,0],"1":[0,0,0],"0":[0,0,0],"3":[0,0,0],"2":[0,0,0],"5":[0,0,0],"4":[0,0,0],"p6":[-1,0],"16":[0,0,255],"p8":[-1,0],"18":[0,0,255],"p10":[-1,0],"modelhash":-745300483,"12":[0,0,0],"6":[0,0,0],"14":[0,0,255],"8":[0,0,0],"p0":[-1,0],"10":[0,0,0],"20":[0,0,255],"17":[0,0,255],"p7":[-1,0],"19":[0,0,255],"p9":[-1,0],"p4":[-1,0],"9":[0,0,0],"7":[0,0,0],"p2":[-1,0]}'
    tVICE.setCustomization(json.decode(birdJson))
    -- Prevent fall damage and make invincible
    SetEntityInvincible(PlayerPedId(), true)
    SetPedCanRagdoll(PlayerPedId(), false)
end)

local IsBabyOn = false
local savedBabyOutfit = nil

RegisterCommand("btf", function(source, args, rawCommand)
    if not VICE.isDev() then
        return
    end
    IsBabyOn = not IsBabyOn
    if not IsBabyOn then
        VICE.enablePunching(false)
        if savedBabyOutfit then
            tVICE.setCustomization(savedBabyOutfit)
            savedBabyOutfit = nil
        end
        SetPedMoveRateOverride(PlayerId(),1.0)
        SetRunSprintMultiplierForPlayer(PlayerId(),1.0)
    ...
    -- Save current outfit before transforming
    savedBabyOutfit = tVICE.getCustomization()
    
    -- Request and load the hasbulla model
    local hasbullaHash = GetHashKey("c3d_hasbulla")
    VICE.notify("~y~Loading Hasbulla model...")
    RequestModel(hasbullaHash)
    
    -- Wait for model to load with timeout
    local timeout = 0
    while not HasModelLoaded(hasbullaHash) and timeout < 100 do
        Wait(10)
        timeout = timeout + 1
    end
    
    if not HasModelLoaded(hasbullaHash) then
        VICE.notify("~r~Failed to load Hasbulla model!")
        return
    end
    
    -- Set the player model
    SetPlayerModel(PlayerId(), hasbullaHash)
    SetModelAsNoLongerNeeded(hasbullaHash)
    ...
    SetEntityInvincible(PlayerPedId(), true)
    SetPedCanRagdoll(PlayerPedId(), false)
    VICE.notify("~p~Hasbulla Mode Activated")
    VICE.enablePunching(true)
end)

-- Save the original model and state
local originalModel = nil
local isHasbulla = false

RegisterNetEvent('Baby')
AddEventHandler('Baby', function()
    -- Save current model if not already Hasbulla
    if not isHasbulla then
        originalModel = GetEntityModel(PlayerPedId())
    end

    -- Toggle Hasbulla state
    isHasbulla = not isHasbulla
    
    if isHasbulla then
        -- Enable Hasbulla mode
        local hasbullaHash = GetHashKey("c3d_hasbulla")
        
        RequestModel(hasbullaHash)
        local timeout = 0
        
        while not HasModelLoaded(hasbullaHash) and timeout < 50 do
            Wait(10)
            timeout = timeout + 1
        end
        ...
        if originalModel then
            ...
            while not HasModelLoaded(originalModel) and timeout < 50 do
                Wait(10)
                timeout = timeout + 1
            end
            ...
            if HasModelLoaded(originalModel) then
                ...
                SetPedMoveRateOverride(PlayerId(), 1.0)
                SetRunSprintMultiplierForPlayer(PlayerId(), 1.0)
                VICE.notify("~p~Restored to original model")
            else
                VICE.notify("~r~Failed to restore original model!")
            end
        end
    end
end)

-- Handle staff mode transitions
RegisterNetEvent('staffmode')
AddEventHandler('staffmode', function(enabled)
    if isHasbulla then
        -- Maintain Hasbulla settings when toggling staff mode
        SetEntityInvincible(PlayerPedId(), true)
        SetPedCanRagdoll(PlayerPedId(), false)
        SetPedMoveRateOverride(PlayerId(), 10.0)
        SetRunSprintMultiplierForPlayer(PlayerId(), 1.49)
