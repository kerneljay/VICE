...
local function extractVideoId(url)
    ...
    if string.find(url, "youtu.be/") then
        return string.match(url, "youtu.be/([^?/]+)")
    end
...
RMenu.Add('VICEDJ','main',RageUI.CreateMenu("","DJ Mixer",VICE.getRageUIMenuWidth(),VICE.getRageUIMenuHeight(),"menus","vice_musicui"))
RMenu.Add('VICEDJ','admin',RageUI.CreateSubMenu(RMenu:Get("VICEDJ","main"),"","DJ Admin Menu",VICE.getRageUIMenuWidth(),VICE.getRageUIMenuHeight(),"menus", "vice_musicui"))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('VICEDJ', 'main')) then
            ...
            elseif not a.active then 
                if h then 
                    RageUI.Button("Start Session", "Start a new DJ Session", {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
                        if Selected then 
                            if VICE.getPlayerCombatTimer() == 0 and not VICE.isPlayerInRedZone() and VICE.getPlayerVehicle() == 0 then
                                setupDj()
                            else
                                VICE.notify('You can not set up a DJ deck right now.')
                            end
                        end 
                    end)
                end 
                ...
                RageUI.Button("Play Song","Youtube Video ID",{},true,function(k,l,m)
                    if m then 
                        tVICE.clientPrompt("Enter Youtube Video URL:", "", function(url)
                            if url and url ~= '' then
                                local videoId = extractVideoId(url)
                                if videoId then
                                    TriggerServerEvent("VICE:DJMenuplay", videoId)
                                else
                                    VICE.notify("~r~Invalid YouTube URL. Please enter a valid YouTube URL or video ID.")
                                end
                            else
...
RegisterNetEvent("VICE:playDjSong")
AddEventHandler("VICE:playDjSong",function(s,coords,t,u)
...
RegisterNetEvent("VICE:requestCurrentProgress")
AddEventHandler("VICE:requestCurrentProgress",function(s,coords)
...
RegisterNetEvent("VICE:toggleDjMenu")
AddEventHandler("VICE:toggleDjMenu",function()
    RageUI.Visible(RMenu:Get('VICEDJ','main'),true)
    h=true 
end)
RegisterNetEvent("VICE:toggleDjAdminMenu")
AddEventHandler("VICE:toggleDjAdminMenu",function(x)
    RageUI.Visible(RMenu:Get('VICEDJ','admin'),true)
    i=x 
end)
RegisterNetEvent("VICE:finaliseSong")
AddEventHandler("VICE:finaliseSong",function(y)
    if a.active then 
        TriggerServerEvent("VICE:playDjSongServer",y,a.coords)
    end 
end)
...
RegisterNetEvent("VICE:updateDjVolume")
AddEventHandler("VICE:updateDjVolume",function(coords,z)
    if specificSongPlaying(coords)then 
        lastDjVolume = z
...
RegisterNetEvent("VICE:stopSong")
AddEventHandler("VICE:stopSong",function(coords)
    local A=false
    for o,p in pairs(c)do 
        if p[1]==coords then 
            A=true
...
RegisterNetEvent("VICE:skipDj")
AddEventHandler("VICE:skipDj",function(coords,B)
    if specificSongPlaying(coords)then 
        if B then 
...
function skipDj(B)
    if songPlaying()then 
        TriggerServerEvent("VICE:skipServer",a.coords,B)
    end 
end
function setupDj()
    a.active=true
    a.coords=VICE.getPlayerCoords()
    a.spawnedDecks=true
...
function createDjObject(C,D,E,F,G)
    local F=VICE.loadModel(F)
    coords=GetOffsetFromEntityInWorldCoords(VICE.getPlayerPed(),C,D,E)
    ...
    local I=GetEntityHeading(VICE.getPlayerPed())
    SetEntityHeading(H,I+G)
end
function stopDj()
    if songPlaying()then 
        TriggerServerEvent("VICE:stopSongServer",a.coords)
    end
    a.active=false
...
function updateVolume()
    if songPlaying()then 
        TriggerServerEvent("VICE:updateVolumeServer",a.coords,b.volume)
    end 
end
...
function func_checkDjSongs(J)
    for o,p in pairs(c)do 
        if#(J.playerCoords-p[1])>45.0 then 
            ...
            if not c[o][3]then 
                g=true
                TriggerServerEvent('VICE:requestCurrentProgressServer',o,p[1])
                c[o][3]=true 
            end 
        end 
    end 
end
VICE.createThreadOnTick(func_checkDjSongs)
RegisterNUICallback('returnProgress',function(K,L)
    if K.progress~=nil then 
        if K.progress~=0 then 
            TriggerServerEvent("VICE:returnProgressServer",d,e,K.progress)
        end 
    end
    L('return')
end)
RegisterNetEvent("VICE:returnProgress")
AddEventHandler("VICE:returnProgress",function(s,coords,M,N)
    if g then 
        if specificSongPlaying(coords)then 
            c[s]=nil
            c[s]={coords,s,true}
...
AddEventHandler('playerSpawned', function(spawn)
    if a.active and songPlaying() then
        TriggerEvent('VICE:toggleDjMenu')
        for o,p in pairs(c) do
            if p[1]==a.coords then 
                TriggerEvent('VICE:playDjSong', p[2], a.coords, o, "DJ")
            end 
        end
    end
end)