...
local k = j["Red"]
local l = GetResourceKvpString("vice_gang_colour") or "Red"
local function m()
    return VICEGangMembers
end
AddEventHandler(
    "VICE:onClientSpawn",
    function()
        TriggerEvent("VICE:ForceRefreshData")
        TriggerServerEvent("VICE:GetGangData")
    end
)

RegisterNetEvent("VICE:GotGangData")
AddEventHandler(
    "VICE:GotGangData",
    function(n, o, h, p, q, r, s)
        if n == nil then
            ...
            PlayerIsInGang = true
            TriggerServerEvent("VICE:setPersonalGangBlipColour", l)
            b = {}
            gangLogs = {}
            e = 1
            f = 1
...
RegisterNetEvent("VICE:disbandedGang")
AddEventHandler(
    "VICE:disbandedGang",
    function()
        a = "none"
...
RegisterNetEvent("VICE:ForceRefreshData")
AddEventHandler(
    "VICE:ForceRefreshData",
    function()
        TriggerServerEvent("VICE:GetGangData")
    end
)
RegisterNetEvent("VICE:InviteReceived")
AddEventHandler(
    "VICE:InviteReceived",
    function(v, w)
        VICEGangInvites[VICEGangInviteIndex] = w
        VICEGangInviteIndex = VICEGangInviteIndex + 1
        VICE.notify(v)
    end
)
RegisterNetEvent("VICE:gangNameNotTaken")
AddEventHandler(
    "VICE:gangNameNotTaken",
    function()
        a = "main"
        PlayerIsInGang = true
    end
...
function VICE.IsOnGangFundsButton()
...
function func_drawGangUI()
    if a == "none" then
        ...
        DrawAdvancedText(0.558, 0.303, 0.005, 0.0028, 0.539, "VICE GANGS", 255 , 255, 255, 255, VICE.getFontId("Akrobat-ExtraBold"), 0)
        ...
        for x, y in pairs(VICEGangInvites) do
        ...
        if CursorInArea(0.35, 0.415, 0.415, 0.46) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if createGangName and createGangName ~= "null" and createGangName ~= "" then
                    TriggerServerEvent("VICE:CreateGang", createGangName)
                else
                    VICE.notify("~r~No gang name entered!")
                end
            end
        else
            CreateGangSelectionRed = 0
        ...
        if CursorInArea(0.435, 0.51, 0.415, 0.46) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if selectedGangInvite then
                    selectedGangInvite = VICEGangInvites[selectedGangInvite]
                    TriggerServerEvent("VICE:addUserToGang", selectedGangInvite)
                    VICEGangInvites = {}
                    a = "main"
                    VICEGangInviteIndex = 0
                    PlayerIsInGang = true
                else
                    VICE.notify("~r~No gang invite selected")
                end
            end
        else
            JoinGangSelectionRed = 0
    ...
    if a == "funds" then
        ...
        DrawAdvancedText(0.591, 0.378, 0.005, 0.0028, 0.48, "VICE GANGS - FUNDS", 255, 255, 255, 255, VICE.getFontId("Akrobat-ExtraBold"), 0)
        DrawAdvancedText(0.591, 0.464, 0.005, 0.0028, 0.5, "GANG FUNDS", 255, 255, 255, 255, VICE.getFontId("Akrobat-ExtraBold"), 0)
        DrawAdvancedText(0.591, 0.502, 0.005, 0.0028, 0.4, "£" .. GangBalance, 25, 199, 65, 255, VICE.getFontId("Akrobat-ExtraBold"), 0)
        ...
        if CursorInArea(0.3083, 0.3718, 0.5490, 0.5999) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount then
                    TriggerServerEvent("VICE:depositGangBalance", gangID, amount)
                else
                    VICE.notify("~r~No amount entered!")
                end
            end
        else
        ...
        if CursorInArea(0.4083, 0.4718, 0.5490, 0.5999) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                TriggerServerEvent("VICE:depositAllGangBalance", gangID)
            end
        else
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount then
                    if gangPermission >= 3 then
                        TriggerServerEvent("VICE:withdrawGangBalance", amount)
                    else
                        VICE.notify("~r~You don't have a high enough rank to withdraw")
                    end
                else
                    VICE.notify("~r~No amount entered!")
                end
        ...
        if CursorInArea(0.6088, 0.6739, 0.5481, 0.6018) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if gangPermission >= 3 then
                    TriggerServerEvent("VICE:withdrawAllGangBalance")
                else
                    VICE.notify("~r~You don't have a high enough rank to withdraw")
                end
            end
    ...
    if a == "contributions" then
        ...
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "VICE GANGS - CONTRIBUTIONS", 255, 255, 255, 255, VICE.getFontId("Akrobat-ExtraBold"), 0)
        ...
        if CursorInArea(0.419271, 0.482813, 0.667593, 0.699074) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if c <= 1 then
                    VICE.notify("~r~Lowest page reached")
                else
                    c = c - 1
                end
            end
        ...
        if CursorInArea(0.512500, 0.575521, 0.667593, 0.699074) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if c >= e then
                    VICE.notify("~r~Max page reached")
                else
                    c = c + 1
                end
            end
    ...
    if a == "members" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, h.theme.r, h.theme.g, h.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "VICE GANGS - MEMBERS", 255, 255, 255, 255, VICE.getFontId("Akrobat-ExtraBold"), 0)
        ...
        for x, z in pairs(b[c]) do
            ...
            if CursorInArea(0.597, 0.610, 0.3731 + 0.0287 * (x - 1), 0.4018 + 0.0287 * (x - 1)) then
                if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                    ...
                    SetResourceKvp("vice_gang_pinned", json.encode(h.pinnedPlayers))
                end
            end
        end
        if CursorInArea(0.6182, 0.6822, 0.360, 0.416) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if selectedMember and PlayerIsInGang and gangID then
                    if gangPermission >= 3 then
                        TriggerServerEvent("VICE:PromoteUser", gangID, tonumber(selectedMember))
                    else
                        VICE.notify("~r~You don't have permission to promote!")
                    end
                else
                    VICE.notify("~r~No gang member selected")
                end
            end
        else
        ...
        if CursorInArea(0.6182, 0.6822, 0.435, 0.491) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if selectedMember and PlayerIsInGang and gangID then
                    if gangPermission >= 3 then
                        if selectedMember == VICE.getUserId() then
                            VICE.notify("~r~You can't demote yourself.")
                        else
                            TriggerServerEvent("VICE:DemoteUser", gangID, selectedMember)
                        end
                    else
                        VICE.notify("~r~You don't have permission to demote!")
                    end
                else
                    VICE.notify("~r~No gang member selected")
                end
                ...
                if selectedMember then
                    if gangPermission >= 3 then
                        if YesNoConfirm() then
                            TriggerServerEvent("VICE:KickUser", gangID, selectedMember)
                        end
                    else
                        VICE.notify("~r~You don't have permission to kick!")
                    end
                else
                    VICE.notify("~r~No gang member selected")
                end
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if B then
                    if gangPermission >= 2 then
                        TriggerServerEvent("VICE:InviteUser", gangID, B)
                    else
                        VICE.notify("~r~You don't have permission to invite players")
                    end
                else
                    VICE.notify("No player name entered")
                end
            end
        else
    ...
    if a == "logs" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, h.theme.r, h.theme.g, h.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "VICE GANGS - LOGS", 255, 255, 255, 255, VICE.getFontId("Akrobat-ExtraBold"), 0)
        DrawRect(0.502, 0.518, 0.387, 0.283, 0, 0, 0, 150)
        DrawAdvancedText(0.449, 0.365, 0.005, 0.0028, 0.4, "Name", 255, 255, 255, 255, 4, 0)
        ...
        if CursorInArea(0.33, 0.395, 0.667593, 0.699074) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                TriggerServerEvent("VICE:SetGangWebhook", gangID)
            end
        else
    ...
    if a == "guide" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, h.theme.r, h.theme.g, h.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "VICE GANGS - PERMISSIONS GUIDE", 255, 255, 255, 255, VICE.getFontId("Akrobat-ExtraBold"), 0)
    ...
    if a == "settings" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, h.theme.r, h.theme.g, h.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "VICE GANGS - SETTINGS", 255, 255, 255, 255, VICE.getFontId("Akrobat-ExtraBold"), 0)
        ...
        if CursorInArea(0.3187, 0.3937, 0.3712, 0.4462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                h.blips = not h.blips
                SetResourceKvp("vice_gang_blips", tostring(h.blips))
            end
        else
        ...
        if CursorInArea(0.4197, 0.4932, 0.3712, 0.4462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                h.pings = not h.pings
                SetResourceKvp("vice_gang_pings", tostring(h.pings))
            end
        else
        ...
        if CursorInArea(0.3187, 0.3937, 0.4712, 0.5462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                h.names = not h.names
                SetResourceKvp("vice_gang_turf_alerts", tostring(h.names))
            end
        else
        ...
        if CursorInArea(0.4197, 0.4932, 0.4712, 0.5462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if newGangName and newGangName ~= "null" and newGangName ~= "" and gangID then
                    TriggerServerEvent("VICE:RenameGang", gangID, newGangName)
                    -- VICE.notify("~r~Contact a dev!")
                else
                    VICE.notify("~r~No gang name entered!")
                end
            end
        ...
        if CursorInArea(0.3187, 0.3937, 0.5712, 0.6462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if YesNoConfirm() then
                    TriggerServerEvent("VICE:LeaveGang", gangID)
                    setCursor(0)
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if gangPermission >= 4 then
                    if YesNoConfirm() == true and gangID then
                        TriggerServerEvent("VICE:DeleteGang", gangID)
                    end
                else
                    VICE.notify("~r~You don't have permission to disband!")
                end
            end
        else
        ...
        if CursorInArea(0.31, 0.39, 0.6712, 0.7064) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                SetResourceKvp("vice_gang_healthui", tostring(h.healthui))
            end
        else
        ...
        if CursorInArea(0.41, 0.49, 0.6712, 0.7064) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if gangPermission >= 4 then
                    TriggerServerEvent("VICE:setGangFit", gangID)
                else
                    VICE.notify("~r~You don't have permission to set gang fit!")
                end
            end
        else
        ...
        if CursorInArea(0.595, 0.645, 0.6155, 0.6405) then
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                SetResourceKvp("vice_gang_colour", l)
                TriggerServerEvent("VICE:setPersonalGangBlipColour", l)
            end
        end
        DrawAdvancedText(0.775, 0.693, 0.005, 0.0028, 0.4, "Back", 255, 255, 255, 255, 4, 0)
    ...
    if a == "turfs" then
        ...
        DrawRect(0.501, 0.308, 0.421, 0.047, h.theme.r, h.theme.g, h.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "VICE GANGS - TURFS", 255, 255, 255, 255, VICE.getFontId("Akrobat-ExtraBold"), 0)
    ...
    if a == "security" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, h.theme.r, h.theme.g, h.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "VICE GANGS - SECURITY", 255, 255, 255, 255, VICE.getFontId("Akrobat-ExtraBold"), 0)
        ...
        if CursorInArea(0.31, 0.65, 0.56, 0.61) then
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if gangPermission >= 4 then
                    TriggerServerEvent("VICE:LockGangFunds", gangID)
                else
    ...
    if a == "theme" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, h.theme.r, h.theme.g, h.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "VICE GANGS - THEME", 255, 255, 255, 255, VICE.getFontId("Akrobat-ExtraBold"), 0)
        DrawAdvancedText(
            0.7,
            0.360,
            0.005,
        ...
        if CursorInArea(0.5187, 0.5828, 0.4138, 0.4694) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                tVICE.CopyToClipboard(h.theme.r .. "," .. h.theme.g .. "," .. h.theme.b)
        ...
        if CursorInArea(0.5187, 0.5828, 0.5407, 0.5962) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                VICE.gangMenuTheme(math.random(1, 255), math.random(1, 255), math.random(1, 255))
            end
        else
                    ...
                    function(L)
                        if L ~= "" then
                            ...
                            if M[1] and M[2] and M[3] then
                                VICE.gangMenuTheme(tonumber(M[1]), tonumber(M[2]), tonumber(M[3]))
                            else
                                VICE.notify("~r~Invalid value")
                            end
                        else
                            VICE.notify("~r~Invalid value")
                        end
    ...
    if a == "main" then
        ...
        DrawAdvancedText(0.595, 0.395, 0.005, 0.0028, 0.51, "VICE GANGS", 255, 255, 255, 255, VICE.getFontId("Akrobat-ExtraBold"), 0)
...
VICE.createThreadOnTick(b5)
Citizen.CreateThread(
    function()
        local N = json.decode(GetResourceKvpString("vice_gang_pinned") or "{}") or {}
        h = {
            blips = GetResourceKvpString("vice_gang_blips") == "true",
            pings = GetResourceKvpString("vice_gang_pings") == "true",
            names = GetResourceKvpString("vice_gang_turf_alerts") == "true",
            healthui = GetResourceKvpString("vice_gang_healthui") == "true",
            pinnedPlayers = {}
        }
        for O in pairs(N) do
            h.pinnedPlayers[tonumber(O)] = true
        end
        h.theme = json.decode(GetResourceKvpString("vice_gang_theme")) or {r = 18, g = 82, b = 228}
        while true do
...
RegisterNetEvent(
    "VICE:setGangMemberColour",
    function(P, K)
        for h, k in pairs(VICEGangMembers) do
            if k[2] == P then
                VICEGangMembers[h].colour = K
            end
        end
    end
)
function VICE.gangMenuTheme(Q, R, S)
    if Q and R and S then
        h.theme = {r = Q, g = R, b = S}
        SetResourceKvp("vice_gang_theme", json.encode(h.theme))
    else
        VICE.notify("~r~Invalid value")
    end
end
function VICE.hasGangNamesEnabled()
    local M = m()
    if M and h then
        return h.names
    else
...
function VICE.isPlayerInSelectedGang(O)
    local M = m()
    if M then
        local P = VICE.clientGetUserIdFromSource(O)
        if P and VICE.getJobType(P) == "" then
            for T, U in pairs(VICEGangMembers) do
                if P == U[2] then
...
function VICE.hasGangBlipsEnabled()
...
RegisterCommand(
    "drawmarker",
    function()
        if h.pings and not VICE.isEmergencyService() and not globalHideUi and not VICE.inEvent() then
            local X = GetGameplayCamCoord()
            local Y = VICE.rotationToDirection(GetGameplayCamRot(2))
            ...
            if _ ~= vector3(0.0, 0.0, 0.0) then
                TriggerServerEvent("VICE:sendGangMarker", gangID, _)
            end
        end
    end
)
local function a0(a1)
    if VICE.getGangPingMarkerIndex() == 2 and not VICE.isEmergencyService() and not VICE.inEvent() then
...
local function a6(a1)
    if VICE.displayPingsOnMinimap() then
...
RegisterNetEvent("VICE:drawGangMarker")
AddEventHandler(
    "VICE:drawGangMarker",
    function(a7, a8)
        if h.pings and not VICE.isEmergencyService() and not globalHideUi and not VICE.inEvent() then
            ...
            if a9 < 1000.0 then
                ...
                if a8 then
                    ...
                    if VICE.getGangPingSound() > 1 then
                        SendNUIMessage(
                            {
                                transactionType = "gangping" .. VICE.getGangPingSound() - 1,
                                volume = VICE.getGangPingVolume() * 0.1
                            }
                        )
                    end
                else
            ...
            if h and h.pings and PlayerIsInGang then
                for T, U in pairs(V) do
                    if V[T] then
                        ...
                        if VICE.getGangPingMarkerIndex() == 3 then
                            VICE.DrawSprite3d(
                                {
                        ...
                        VICE.DrawText3D(U.coords, U.creator .. "\n" .. tostring(math.floor(U.dist)) .. "m", 0.2)
                    end
                end
            end
            Citizen.Wait(0)
...
local ab = {}
RegisterNetEvent("VICE:sendGangHPStats")
AddEventHandler(
    "VICE:sendGangHPStats",
    function(ac)
        ab = ac
    end
)
...
local function al()
    ...
    if h and h.healthui and PlayerIsInGang then
        if M and not VICE.isEmergencyService() and not globalHideUi and not VICE.inEvent() then
            local an = 0
            local ao = VICE.getShowHealthPercentageFlag()
            if VICEGangMembers then
                for ap, aq in pairs(VICEGangMembers) do
                    name, id, rank, lastseen, playtime = table.unpack(aq)
                    if lastseen == "~g~Online" and VICE.getJobType(id) == "" and h.pinnedPlayers[id] then
                        local ar = true
                        local as = nil
                        local at = nil
                        local ac = ab[id]
                        ...
                        local au = VICE.getTempFromPerm(id)
                        if au then
    ...
    if i and i == M then
        if am <= 0 then
            TriggerServerEvent("VICE:getGangHealthTable", nil)
            i = nil
        end
    else
        if am > 0 then
            TriggerServerEvent("VICE:getGangHealthTable", gangID)
            i = M
        end
    end
end
VICE.createThreadOnTick(al)
function VICE.setGangUIXPos(aB)
    local aC = tonumber(string.match(aB, "%d+")) / 100
    ad = 0.008 * aC
end
function VICE.setGangUIYPos(aB)
    local aC = tonumber(string.match(aB, "%d+")) / 100
    ae = 0.35 * aC
end