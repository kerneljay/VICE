...
local b = {}
local c = 0
function VICE.isInRadioChannel()
    return c ~= 0
end
local function d()
...
local function m(j, n)
    SendNUIMessage({
        ...
        playerSrc = j,
        isTalking = n,
        color = n and "#FFA500" or "#0075C2", -- Orange when talking, blue when not
        style = n and "talking" or "idle" -- Add style indicator for UI
    })
end
...
AddEventHandler(
    ...
    function(n)
        ...
        if n then
            ...
            TriggerEvent("pma-voice:setTalkingOnRadio", GetPlayerServerId(PlayerId()), true)
        else
            exports["pma-voice"]:clearProximityOverride()
            TriggerEvent("pma-voice:setTalkingOnRadio", GetPlayerServerId(PlayerId()), false)
        end
    end
)
RMenu.Add(
    "radios",
    "mainmenu",
    RageUI.CreateMenu("Radios", "~b~M~w~ain Menu", VICE.getRageUIMenuWidth(), VICE.getRageUIMenuHeight())
)
RMenu.Add(
    "radios",
    "advancedeffects",
...
RageUI.CreateWhile(
    ...
    function()
        RageUI.IsVisible(
            ...
            function()
                ...
                RageUI.Button(
                    ...
                    RMenu:Get("radios", "advancedeffects")
                )
            end,
            function()
            end
        ...
        RageUI.IsVisible(
            ...
            function()
                RageUI.ButtonWithStyle(
                    "Frequency Range In (Low)",
                    "",
                    {RightLabel = tostring(E["freq_low"])},
                    true,
                    function(S, T, U)
                        if U then
                            tVICE.clientPrompt(
                ...
                RageUI.ButtonWithStyle(
                    "Frequency Range In (High)",
                    "",
                    {RightLabel = tostring(E["freq_hi"])},
                    true,
                    function(S, T, U)
                        if U then
                            tVICE.clientPrompt(
                ...
                RageUI.ButtonWithStyle(
                    ...
                    "",
                    {RightLabel = tostring(E["rm_mod_freq"])},
                    true,
                    function(S, T, U)
                        if U then
                            tVICE.clientPrompt(
                ...
                RageUI.ButtonWithStyle(
                    "Frequency Range Out (Low)",
                    "",
                    {RightLabel = tostring(E["o_freq_lo"])},
                    true,
                    function(S, T, U)
                        if U then
                            tVICE.clientPrompt(
                ...
                RageUI.ButtonWithStyle(
                    "Frequency Range Out (High)",
                    "",
                    {RightLabel = tostring(E["o_freq_hi"])},
                    true,
                    function(S, T, U)
                        if U then
                            tVICE.clientPrompt(
...
RegisterCommand(
    ...
    function()
        ...
        TriggerEvent("VICE:showRadioWheel", X)
    end,
    false
)
...
RegisterCommand(
    ...
    function()
        if c ~= 0 then
            if exports["pma-voice"]:getRadioVolume() == 0 then
                exports["pma-voice"]:setRadioVolume(u("radios_volume", 10) * 10)
                TriggerServerEvent("VICE:radiosSetIsMuted", false)
            else
                exports["pma-voice"]:setRadioVolume(0)
                TriggerServerEvent("VICE:radiosSetIsMuted", true)
            end
        end
    end,
    false
)
RegisterKeyMapping("toggleradiomute", "Mute Selected Radio", "KEYBOARD", "")
RegisterNetEvent(
    "VICE:radiosSetPlayerIsMuted",
    function(q, j, p)
        local s = b[q]
        if not s then
            return
...
AddEventHandler(
    "VICE:openRadioConfig",
    function()
        RageUI.Visible(RMenu:Get("radios", "mainmenu"), true)
    end
)
function VICE.radioUIVisible(Y)
    e(Y)
end