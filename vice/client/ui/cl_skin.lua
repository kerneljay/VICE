...
function _(i, ...)
    if Locales[SkinConfig.Locale] then
        if Locales[SkinConfig.Locale][i] then
...
RegisterNUICallback(
    ...
    function(n)
        ...
        SetPedPropIndex(PlayerPedId(), 6, n.componentID, 0, 2)
    end
)
RegisterNUICallback(
    ...
    function(n)
        ...
        SetPedPropIndex(PlayerPedId(), 6, n.componentID, n.textureID, 2)
    end
)
RegisterNUICallback(
    ...
    function(n)
        n.componentID = math.floor(n.componentID + 0)
        SetPedPropIndex(PlayerPedId(), 7, n.componentID, 0, 2)
    end
)
RegisterNUICallback(
    ...
    function(n)
        n.componentID = math.floor(n.componentID - 1)
        n.textureID = math.floor(n.textureID + 0)
        SetPedPropIndex(PlayerPedId(), 7, n.componentID, n.textureID, 2)
    end
)
RegisterNUICallback(
    "updateSkin",
    function(n)
        ...
        local rawFacepaint = tonumber(n.facepaints) or 0
        if rawFacepaint > 0 then
            facepaints = rawFacepaint + 15
        else
            facepaints = 0
        end
        facepaintscolour = tonumber(n.facepaintscolour) or 0
        if v == true then
            CloseSkinCreator()
        else
            local r = nil
            ...
            TriggerServerEvent("VICE:saveFaceData", faceSaveData)
        end
    end
)
RegisterNUICallback(
    "rotateleftheading",
    function(n)
        local u = GetEntityHeading(PlayerPedId())
        SetEntityHeading(PlayerPedId(), u - 10)
    end
)
RegisterNUICallback(
    "rotaterightheading",
...
RegisterNUICallback(
    ...
    function(n)
        c = n.zoom
        local w, x, y = table.unpack(GetEntityCoords(PlayerPedId()))
