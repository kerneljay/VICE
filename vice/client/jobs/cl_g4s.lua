...
local function B()
    ...
    local D = 500 * math.ceil(#(VICE.getPlayerCoords() - GetEntityCoords(b)) / 10)
    if j then
...
RMenu.Add(
    "g4s",
    "shiftHandler",
    RageUI.CreateMenu("", "~s~S~w~ecurity Company", VICE.getRageUIMenuWidth(), VICE.getRageUIMenuHeight(), "menus", "g4sjob")
)
...
tVICE.addMarker(d.x, d.y, d.z, 1.2, 1.2, 1.2, 0, 255, 125, 125, 50, 0, true, true)
local F = tVICE.addBlip(d.x, d.y, d.z, 457, 18, "G4S Depot", 1.0, false)
local G = function()
...
VICE.createArea("g4sDepot_", d, 1.5, 6, G, H, I)
Citizen.CreateThread(
    function()
        while true do
            local C = VICE.getPlayerPed()
            local J = VICE.getPlayerCoords()
            if a then
                if not u then
                    displayJob()
                end
    ...
    function()
        while true do
            if not y.hackingEngaged and not a and not y.awaitingResponse then
                if table.count(s) > 0 and VICE.getPlayerVehicle() == 0 then
                    for K, L in pairs(s) do
                        ...
                        if table.has(p, O) or not DoesEntityExist(N) then
                            ...
                            if IsVehicleStopped(N) then
                                local P = #(VICE.getPlayerCoords() - GetEntityCoords(N))
                                ...
                                if P <= 11.0 then
                                    ...
                                    if IsControlJustPressed(1, 18) then
                                        ...
                                        y.entity = N
                                        y.src = O
                                        TriggerServerEvent("VICE:requestHackingChanceServer", M, O)
...
function toggleCashbox()
    if A.active then
        ...
        local o = VICE.loadModel("cashbox")
        local C = VICE.getPlayerPed()
...
RegisterNetEvent(
    ...
    function(b, W, X, Y)
        if Y then
            ...
            local o = VICE.loadModel("prop_cs_tablet")
            v = CreateObject(o, 0, 0, 0, true, true, true)
            AttachEntityToEntity(
                v,
                VICE.getPlayerPed(),
                GetPedBoneIndex(VICE.getPlayerPed(), 57005),
                0.17,
                0.10,
                -0.13,
                24.0,
            ...
            if HasScaleformMovieLoaded(R) then
                ...
                if IsScaleformMovieMethodReturnValueReady(S) then
                    ...
                    elseif ProgramID == 87 then
                        ...
                        TriggerServerEvent("VICE:updateHackingOutcome", y.netId, y.src, U, false)
                        DeleteObject(v)
                        break
                    ...
                    elseif ProgramID == 86 then
                        ...
                        TriggerServerEvent("VICE:updateHackingOutcome", y.netId, y.src, U, true)
                        DeleteObject(v)
                        break
    ...
    function()
        if RageUI.Visible(RMenu:Get("g4s", "main")) then
            RageUI.DrawContent(
                ...
                function()
                    if not j then
                        if VICE.getPlayerVehicle() == 0 and w < 4 then
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(Z, _, a0)
                                if a0 then
                                    TriggerServerEvent("VICE:toggleShiftG4S", false)
                                    a = false
                                end
                            end
                        )
                        ...
                        if not h then
                            RageUI.ButtonWithStyle(
                                ...
                                function(Z, _, a0)
                                    if a0 then
                                        TriggerServerEvent("VICE:activatePanicServer", c)
                                        imageNotification(
                                            "notification_images",
                                            "g4slogo",
                    ...
                    if not a then
                        RageUI.ButtonWithStyle(
                            ...
                            function(Z, _, a0)
                                if a0 then
                                    RageUI.Visible(RMenu:Get("g4s", "shiftHandler"), false)
                                    TriggerServerEvent("VICE:toggleShiftG4S", true)
                                end
                            end
                        )
                    else
...
function newG4sVehicle()
    ...
    if DoesEntityExist(b) and not IsVehicleDriveable(b, true) then
        ...
        b = VICE.spawnVehicle(n, a1.x, a1.y, a1.z, GetEntityHeading(C), true, true)
        c = VehToNet(b)
        TriggerServerEvent("VICE:updateCurrentVehicle", c)
        w = w + 1
    end
end
function securityCheck(C)
    local a2 = VICE.getPlayerVehicle()
    if a2 ~= 0 and a2 and b and b ~= 0 then
        if a2 ~= b then
            r = true
            imageNotification(
                "notification_images",
            ...
            SetTimeout(
                15000,
                function()
                    a2 = GetVehiclePedIsIn(VICE.getPlayerPed(), false)
                    if a2 ~= 0 then
                        if a2 ~= b then
                            TriggerServerEvent("VICE:toggleShiftG4S", false)
                            tVICE.removeBlip(k)
...
RegisterNetEvent(
    "VICE:resetPanicStatus",
    function()
        h = false
    end
)
RegisterNetEvent(
    "VICE:updateJobInformation",
    function(a3)
        if l ~= 0 then
            tVICE.removeBlip(l)
        end
        if a3.jobActive then
            ...
            Wait(3000)
            TriggerServerEvent("VICE:acceptJob")
        end
    end
)
...
RegisterNetEvent(
    ...
    function(a4)
        ...
        local J = VICE.getPlayerCoords()
        a = true
        VICE.disableCustomizationSave(a)
        b = VICE.spawnVehicle(n, z.x, z.y, z.z, z.heading, true, true)
        c = VehToNet(b)
        TaskWarpPedIntoVehicle(C, b, -1)
        g = a4
        local a5 = {"Jack", "SEAN", "Serio", "mkay", "VICE"}
        ...
        x = json.encode(tVICE.getCustomization())
        if a8 then
            VICE.loadCustomisationPreset("G4SMale")
        else
            VICE.loadCustomisationPreset("G4SFemale")
        end
        Wait(2000)
        ...
        Wait(5000)
        TaskVehicleTempAction(VICE.getPlayerPed(), b, 27, 10.0)
        Wait(2000)
...
RegisterNetEvent(
    ...
    function()
        ...
        local C = VICE.getPlayerPed()
        tVICE.setCustomization(json.decode(x))
        if A.active then
            toggleCashbox()
        end
        if k ~= 0 then
...
RegisterNetEvent(
    ...
    function()
        ...
        while not f do
            ...
            local P = #(VICE.getPlayerCoords() - ae)
            if P <= 1.0 then
                ad = true
        ...
        TriggerServerEvent("VICE:updateMoneyInVehicle")
        t = false
    end
)
RegisterNetEvent(
    "VICE:requestMoneyOutVehicle",
    function()
        local ac = VICE.getPlayerPed()
        while IsPedInAnyVehicle(ac, true) do
            Wait(400)
        ...
        w = 1
        TriggerServerEvent("VICE:updateMoneyOutVehicle")
    end
)
...
RegisterNetEvent(
    "VICE:activeVansUpdate",
    function(ax, O, aC)
        if not a then
            local b = VICE.getObjectId(ax, "VICE:activeVansUpdate 1")
            if b ~= 0 and b and DoesEntityExist(b) then
                if aC then
                    s[ax] = nil
                else
                    if not table.has(s, ax) then
                        local b = VICE.getObjectId(ax, "VICE:activeVansUpdate")
                        s[ax] = {O, b, ax}
                    end
                end
            end
