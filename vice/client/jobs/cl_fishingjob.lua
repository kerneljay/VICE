            ...
            ontick_fishing = function(y)
                if globalFishingOnDuty then
                    if
                     ...
                     then
                        TriggerServerEvent("VICE:fishingJobChecks", y.type)
                    end
                    if IsControlJustPressed(0, 47) then
                        local z = GetVehiclePedIsIn(PlayerPedId(), true)
                        VICE.notify("~y~Boat anchor set to " .. tostring(not x))
...
function func_fishingStatus(y)
    ...
    elseif g and c then
        if IsControlJustPressed(0, 102) then
            ...
            if d >= e and d <= f then
                TriggerServerEvent("VICE:fishingCaughtFish", d)
            else
                TriggerServerEvent("VICE:fishingCaughtFish", d)
...
VICE.createThreadOnTick(func_fishingStatus)
RegisterNetEvent(
    "VICE:fishingJobSetCasted",
    function()
        g = true
...
VICE.createThreadOnTick(func_createBar)
