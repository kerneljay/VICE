        ...
        if NetworkIsPlayerActive(player) then
            ...
            if IsPedFatallyInjured(ped) and not isDead then
				...
				if killerentitytype == 1 then
					...
					if IsPedInAnyVehicle(killer, false) == 1 and VICE.GetPedVehicleSeat(killer) then
						killerinvehicle = true
						killervehiclename = GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsUsing(killer)))
                        killervehicleseat = VICE.GetPedVehicleSeat(killer)
					else killerinvehicle = false
					end
				end

				local killerid = GetPlayerByEntityID(killer)
				if killer ~= ped and killerid and NetworkIsPlayerActive(killerid) then killerid = GetPlayerServerId(killerid)
				else killerid = -1
				end
...
function VICE.GetPedVehicleSeat(ped)
