...
Citizen.CreateThread(function()
    while true do 
        local e=VICE.getPlayerVehicle()
        if e~=0 and GetPedInVehicleSeat(e,-1)==VICE.getPlayerPed()then 
            local f=GetVehicleEngineHealth(e)
            if f<=b and a[e]==nil then 
                local g=GetVehicleClass(e)
                if g~=8 and g~=16 and g~=15 and g~=13 then 
                    TriggerEvent("vice:PlaySound", "airbag")
                    createAirbags(e)
                end 
            else 
                if a[e]==nil then 
                    d=GetPedArmour(VICE.getPlayerPed())
                end 
            end 
        else 
            d=0 
...
function createAirbags(e)
    ...
    if d>GetPedArmour(VICE.getPlayerPed())then 
        tVICE.setArmour(d)
    end
    d=0
