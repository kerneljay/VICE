...
function tVICE.isPlayerInDrone()
    return c.active
end
local function d()
    ...
    if c.cameraEnabled then
        ...
        local f = 500 * math.ceil(#(VICE.getPlayerCoords() - GetEntityCoords(c.vehicleHandle)) / 10)
        ...
        local g = VICE.loadModel("prop_cs_tablet")
        c.tablet = CreateObject(g, VICE.getPlayerCoords(), true)
        ...
        VICE.loadAnimDict("amb@code_human_in_bus_passenger_idles@female@tablet@idle_a")
        TaskPlayAnim(
            e,
...
RegisterNetEvent(
    ...
    function()
        if not c.active then
            if VICE.getPlayerVehicle() == 0 and not tVICE.isInComa() then
                TriggerServerEvent("VICE:performDroneLog", "Powered on their DJI Inspire Drone")
                newDrone()
            else
                VICE.notify("You are not able to use a drone right now.")
            end
        else
            TriggerServerEvent("VICE:performDroneLog", "Powered off their DJI Inspire Drone")
            deleteDrone()
        end
    end
)
        ...
        while true do
            if c.active then
                ...
                if not NetworkHasControlOfEntity(c.driverHandle) or not NetworkHasControlOfEntity(c.vehicleHandle) then
                    VICE.notify("You have ~b~lost control ~w~of the drone, after going out of range")
                    deleteDrone()
                end
                ...
                if IsDisabledControlJustPressed(0, 121) then
                    ...
                    TaskHeliMission(
                        ...
                        b,
                        10.0,
                        GetEntityHeading(c.driverHandle),
                        -1,
                        -1,
                        -1,
                        32
...
Citizen.CreateThread(
    function()
        while true do
            local p = VICE.getPlayerVehicle()
            if p ~= 0 then
                local q = GetEntityModel(p)
                if q == GetHashKey(a) then
                    o = o + 1
                    if o > 3 then
                        VICE.notify("~r~Access Denied~w~: Engaging defences in 3, 2, 1...")
                        Wait(500)
                        local r = VICE.getPlayerCoords()
                        AddOwnedExplosion(VICE.getPlayerPed(), r.x, r.y, r.z, 4, 17.0, true, false, 10.0)
                        ...
                        if VICE.getPlayerVehicle() and DoesEntityExist(VICE.getPlayerVehicle()) then
                            DeleteEntity(VICE.getPlayerVehicle())
                        end
                    end
                end
            end
...
function newDrone()
    local g = VICE.loadModel(a)
    local v = VICE.loadModel("s_m_m_bouncer_01")
    local e = VICE.getPlayerPed()
    local r = GetOffsetFromEntityInWorldCoords(e, 0.0, 4.0, 0.0)
    local w = GetEntityHeading(e)
    c.vehicleHandle = VICE.spawnVehicle(a, r, w, true)
    SetModelAsNoLongerNeeded(g)
    SetEntityNoCollisionEntity(VICE.getPlayerPed(), c.vehicleHandle, 1)
    Wait(1000)
    ...
    c.active = true
    VICE.notify("Drone ~b~created~w~.")
    Wait(1000)
    if globalNHSOnDuty then
        VICE.notify("~y~Reminder~w~: These drones are reserved for usage by HEMS, Specialised Off Road Unit")
        VICE.notify("and Senior Pilots only. If you are not in these divisions, you ~r~should not ~r~be using this.")
    end
    VICE.notify("To remove your drone, use ~b~/drone")
end
...
local function B()
    ...
    if p == 0 then
        if z then
            if #(z - VICE.getPlayerCoords()) > 20.0 then
                local C = PlayerPedId()
    ...
    if #(z - D) > 20.0 then
        ...
        if IsDisabledControlJustPressed(0, 23) or IsDisabledControlJustPressed(0, 75) then
            VICE.notify("You must be near the ~y~start position~w~ to exit.")
        end
    end
end
VICE.createThreadOnTick(B)
