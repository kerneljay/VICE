...
local function q()
    if IsControlJustReleased(0, 299) then
        if NetworkHasControlOfEntity(c) then
            TriggerServerEvent("VICE:toggleAmbulanceDoors", VICE.getNetId(c, "Stretchers"))
        else
...
local function C(D, E)
    ...
    if not y(D) or b then
        ...
        VICE.loadAnimDict(a.positions[e].animDict)
        DetachEntity(x, false, false)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if d and d ~= 0 then
            ...
            if not b and not IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and VICE.getPlayerVehicle() == 0 then
            ...
            if IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and c == 0 then
                ...
                if H ~= -1 and globalNHSOnDuty then
                    ...
                    if IsControlJustPressed(0, 183) then
                        ...
                        if L > 0 and M ~= 0 then
                            TriggerServerEvent("VICE:forcePlayerOnToStretcher", L, M)
                        end
                    end
                end
                r(K)
            ...
            if IsControlJustReleased(0, 51) and not IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and not b then
                if DecorGetInt(d, "inside_ambulance_netid") ~= 0 then
                    local O = VICE.getObjectId(DecorGetInt(d, "inside_ambulance_netid"), "Stretcher Thread")
...
function pushStretcher(x, d)
    Citizen.CreateThread(function()
        ...
        TriggerServerEvent("VICE:stretcherAttachPlayer", P)
        ...
        VICE.loadAnimDict("anim@heists@box_carry@")
        local Q = 0
        while IsEntityAttachedToEntity(d, x) do
            ...
            if DecorGetInt(d, "inside_ambulance_netid") ~= 0 then
                local O = VICE.getObjectId(DecorGetInt(d, "inside_ambulance_netid"), "pushStretcher(ped, nearbyStretcher)")
                if DecorGetBool(O, "has_stretcher_inside") ~= false then
                    if not NetworkHasControlOfEntity(O) then
                        if GetGameTimer() - Q > 200 then
                            TriggerServerEvent("VICE:updateHasStretcherInsideDecor",VICE.getNetId(O, "Stretchers"),false)
                            Q = GetGameTimer()
                        end
                    else
            ...
            if c and c ~= 0 then
                ...
                if IsControlJustReleased(0, 300) and (GetVehicleDoorAngleRatio(c, 4) ~= 0 or IsThisModelAHeli(GetEntityModel(c))) and not R then
                    ...
                    if not NetworkHasControlOfEntity(c) then
                        TriggerServerEvent("VICE:updateHasStretcherInsideDecor", VICE.getNetId(c, "Stretcher"), true)
                    else
                        DecorSetBool(c, "has_stretcher_inside", true)
                    end
                    if not NetworkHasControlOfEntity(d) then
                        TriggerServerEvent("VICE:updateStretcherLocation",VICE.getNetId(d, "Stretchers"),VICE.getNetId(c, "Stretchers"))
                    else
                        DecorSetInt(d, "inside_ambulance_netid", VICE.getNetId(c, "Stretchers"))
                    end
...
RegisterNetEvent("VICE:stretcherAttachPlayer",function(P, S)
...
RegisterCommand("stretcher",function()
    if globalNHSOnDuty then
        ...
        local g = VICE.loadModel("trump-stretcher")
        local D = VICE.spawnVehicle(g, v.x, v.y, v.z, GetEntityHeading(x), false, true, true)
...
RegisterCommand("removestretcher",function()
    if globalNHSOnDuty then
        ...
        if V then
            TriggerServerEvent("VICE:removeStretcher", VICE.getNetId(V, "Stretchers"))
        end
    end
end,false)
RegisterNetEvent("VICE:setStretcherInside",function(P, W)
    local D = VICE.getObjectId(P, "VICE:setStretcherInside")
    DecorSetInt(D, "inside_ambulance_netid", W)
end)
RegisterNetEvent("VICE:setHasStretcherInsideDecor",function(X, Y)
    local c = VICE.getObjectId(X, "VICE:setHasStretcherInsideDecor")
    DecorSetBool(c, "has_stretcher_inside", Y)
end)
RegisterNetEvent("VICE:toggleAmbulanceDoorStatus",function(X)
    local c = VICE.getObjectId(X, "VICE:toggleAmbulanceDoorStatus")
...
function VICE.isUsingStretcher()
    return b
end
RegisterNetEvent("VICE:forcePlayerOnToStretcher",function(P)
    if not NetworkDoesNetworkIdExist(P) then
        return
    end
    local D = NetworkGetEntityFromNetworkId(P)
