...
local m = -20.0
local n = 0.01
RegisterNetEvent("VICE:doAcid",function()
    local p = "mp_player_inteat@burger"
    local q = "mp_player_int_eat_burger"
    local r = VICE.loadModel("a_c_hen")
    RequestAnimDict(p)
    while not HasAnimDictLoaded(p) do
        Wait(0)
    end
    TaskPlayAnim(VICE.getPlayerPed(), p, q, 2.0, 2.0, -1, 51, 0, false, false, false)
    RemoveAnimDict(p)
...
DoAcid = function(x)
    ...
    while GetGameTimer() - A < x do
        local D = GetEntityCoords(VICE.getPlayerPed())
        ...
        if E > 1.0 then
            ...
            if z == 5 then
                z = 0
                local F = C - D
                local G = GetEntityVelocity(VICE.getPlayerPed())
                SetEntityCoordsNoOffset(VICE.getPlayerPed(), D.x + F.x, D.y + F.y, D.z)
                ForcePedMotionState(VICE.getPlayerPed(), -1115154469, 1, 1, 0)
                SetEntityVelocity(VICE.getPlayerPed(), G.x, G.y, G.z)
            end
            C = GetEntityCoords(VICE.getPlayerPed())
        end
        DrawToons()
        DrawCubes()
        if e and not f then
...
InitPed = function()
    ...
    while GetScreenCoordFromWorldCoord(L.x, L.y, L.z) and not IsPointOnRoad(L.x, L.y, L.z) do
        J = math.random(0, 359)
        K = math.random(50, 80)
        L = GetEntityCoords(VICE.getPlayerPed()) + PointOnSphere(0.0, J)
        Citizen.Wait(0)
    end
...
TrackEnt = function()
    while true do
        local E = GetVecDist(GetEntityCoords(VICE.getPlayerPed()), GetEntityCoords(g))
        if E > 5.0 then
            TaskGoToEntity(g, VICE.getPlayerPed(), -1, 4.0, 100.0, 1073741824, 0)
            Wait(1000)
        else
            ...
            if not d or GetGameTimer() - d > 1000 then
                d = GetGameTimer()
                TaskTurnPedToFaceEntity(g, VICE.getPlayerPed(), -1)
            end
...
DrawCubes = function()
    local X = GetEntityCoords(VICE.getPlayerPed())
    local T = 10
    for U, V in pairs(b) do
        local Y = 0.1
        local Z = 0.1
...
local ae = false
local af = false
local viceBlueRushActive = false
RegisterNetEvent("VICE:cocaineEffect",function()
    if ae then
        af = true
        local t = 0
        while t < 10 do
            tVICE.varyHealth(-10)
            VICE.notify("~y~I don't feel too good..")
            Wait(5000)
            t = t + 1
        end
...
RegisterNetEvent("VICE:heroinEffect",function()
...
RegisterNetEvent("VICE:blueRushEffect",function()
    if viceBlueRushActive then
        VICE.notify("~r~Joint effect is already active.")
        return
    end
    ...
    CreateThread(function()
        ...
        while viceBlueRushActive and GetGameTimer() < endTime do
            local alpha = math.floor(35 + ((math.sin(GetGameTimer() / 175.0) + 1.0) * 20))
            DrawRect(0.5, 0.5, 1.0, 1.0, 25, 95, 255, alpha)
            Wait(0)
        end
    end)
