...
local function p()
    c = true
    local q = VICE.loadModel("BA_PROP_CLUB_EMIS_RIG_10")
...
AddEventHandler("VICE:onClientSpawn",function(P, Q)
    if Q then
        ...
        VICE.createArea("nightclub_entrance", a, 3.0, 6, R, T, U, {})
        local R = function(S)
        end
        local T = function(S)
        end
        ...
        VICE.createArea("nightclub_exit", b, 3.0, 6, R, T, U, {})
    end
end)
Citizen.CreateThread(function()
...
Citizen.CreateThread(function()
    local V = AddBlipForCoord(-1687.7420654297,-921.15362548828,23.82737159729)
    SetBlipSprite(V, 136)
    SetBlipColour(V, 1)
    SetBlipAsShortRange(V, true)
    SetBlipScale(V, 0.75)
