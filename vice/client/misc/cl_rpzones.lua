...
local function f()
    ...
    g.position = VICE.getPlayerCoords()
    g.radius = g.radius + 0.0
    g.vehicles.speed = g.vehicles.speed + 0.0
    TriggerServerEvent("VICE:createRPZone", g)
end
...
local function k()
    ...
    if l then
        TriggerServerEvent("VICE:removeRPZone", l)
    end
end
local p = false
RMenu.Add(
    "rpzones",
    "mainmenu",
    RageUI.CreateMenu("", "~s~M~w~ain Menu", VICE.getRageUIMenuWidth(), VICE.getRageUIMenuHeight(), "menus", "adminmenu")
)
RMenu.Add(
    "rpzones",
    "weapons",
...
local function w(g, x)
    ...
    if not g.weapons.melee and y == `GROUP_MELEE` then
        VICE.setWeapon(j, "WEAPON_UNARMED")
    elseif not g.weapons.throwable and y == `GROUP_THROWN` then
        VICE.setWeapon(j, "WEAPON_UNARMED")
    elseif not g.weapons.pistol and y == `GROUP_PISTOL` then
        VICE.setWeapon(j, "WEAPON_UNARMED")
    elseif not g.weapons.shotgun and y == `GROUP_SHOTGUN` then
        VICE.setWeapon(j, "WEAPON_UNARMED")
    elseif not g.weapons.mg and y == `GROUP_MG` then
        VICE.setWeapon(j, "WEAPON_UNARMED")
    elseif not g.weapons.smg and y == `GROUP_SMG` then
        VICE.setWeapon(j, "WEAPON_UNARMED")
    elseif not g.weapons.rifle and y == `GROUP_RIFLE` then
        VICE.setWeapon(j, "WEAPON_UNARMED")
    elseif not g.weapons.sniper and y == `GROUP_SNIPER` then
        VICE.setWeapon(j, "WEAPON_UNARMED")
    elseif not g.weapons.heavy and y == `GROUP_HEAVY` then
        VICE.setWeapon(j, "WEAPON_UNARMED")
    end
end
local function z(A)
...
local function I(g)
    local n, x = GetCurrentPedWeapon(VICE.getPlayerPed())
    if x ~= 0 and x ~= `WEAPON_STAFFGUN` then
        w(g, x)
    end
    local A, J = VICE.getPlayerVehicle()
    if A ~= 0 and J then
        C(g, A)
    end
...
VICE.createThreadOnTick(K)
function VICE.GetRPZoneInfo()
    local M = VICE.getPlayerCoords()
    for n, g in ipairs(d) do
        if #(M - g.position) < g.radius then
            return g
