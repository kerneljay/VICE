...
Citizen.CreateThread(function()
	while true do
		...
		if GetVehiclePedIsTryingToEnter(m) ~= 0 or d then
			VICE.setWeapon(m, "WEAPON_UNARMED", true)
		end
		if b then
					...
					if o ~= `WEAPON_UNARMED` then
						if table.has(i, o) then
							...
							if p ~= `WEAPON_UNARMED` and not table.has(i, p) then
								...
								a = false
								if o == "WEAPON_PDGLOCK20VA5" and GetPedDrawableVariation(n, 7) == 187 then
									VICE.loadCustomisationPreset("TaserInHolster")
								end
								if not d then
									a = false
									b = false
						...
						if table.has(i, f) then
							...
							if p ~= `WEAPON_UNARMED` and not table.has(i, p) then
								...
								if h and GetPedDrawableVariation(n, 7) == 161 then
									VICE.loadCustomisationPreset("TaserOutHolster")
								end
							end
						else
							loadAnimDict("reaction@intimidation@1h")
							...
							RemoveAnimDict("reaction@intimidation@1h")
							b = true
							VICE.setWeapon(n, GetHashKey(f), true)
							Citizen.Wait(1300)
							VICE.setWeapon(n, "WEAPON_UNARMED", true)
							Citizen.Wait(700)
							ClearPedTasks(n)
							a = true
							b = false
