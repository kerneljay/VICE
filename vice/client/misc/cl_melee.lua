...
RegisterCommand("togglemelee",function()
    if VICE.getUserId()==1 or VICE.getUserId()==2 or VICE.getUserId()==3 or VICE.isDev() then
        c = not c
        VICE.notify('Melee Combat: '..(c and "~g~Enabled" or "Disabled"))
    end 
end)
function VICE.enablePunching(d)
    c = d
end
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if h == `WEAPON_UNARMED` or h == `WEAPON_STAFFGUN` or GetWeapontypeGroup(h) == `GROUP_MELEE` or c then
...
AddEventHandler("VICE:onClientSpawn",function(h, i)
    if i then
        ...
        for n, o in pairs(a) do
            VICE.createArea("boxing_" .. n, o.pos, o.radius, 6, j, l, m, {})
        end
    end
end)
...
AddEventHandler("VICE:onClientSpawn",function(h, i)
    if i then
        ...
        for n, u in pairs(t) do
            tVICE.addMarker(u.x, u.y, u.z, 1.0, 1.0, 1.0, 0, 0, 255, 100, 50, 27, false, false, true)
            VICE.createArea(n .. "_boxingGloves", u, 1.5, 6, j, l, m, {})
        end
    end
end)
local v = {}
function putGloves()
    ...
    local x = VICE.loadModel("prop_boxing_glove_01")
...
--         Wait(0)
--         local h = GetSelectedPedWeapon(PlayerPedId())
--         if h ~= `WEAPON_STAFFGUN` and not c then
