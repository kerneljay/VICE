...
Citizen.CreateThread(function()
    ...
    while true do
        if a.active and a.id ~= 0 then
            ...
            if not a.inVehicle then
                if b[a.id] and b[a.id].animDict and b[a.id].animName then
            ...
            if h ~= `WEAPON_UNARMED` and (not b[a.id].allowPistol or not table.has(c, h)) then
                VICE.setWeapon(g, "WEAPON_UNARMED", true)
            end
...
RMenu.Add("policeshields","main",RageUI.CreateMenu("~b~P~w~olice Shields", "Shield Management", VICE.getRageUIMenuWidth(), VICE.getRageUIMenuHeight()))
...
RegisterNetEvent("VICE:toggleShieldMenu",function(l)
...
function removeShield(m)
    if DoesEntityExist(a.prop) then
        ...
        if m then
            VICE.notify("~g~Success~w~: Shield removed")
        end
    end
end
function createShield(n)
    ...
    local o = false
    local g = VICE.getPlayerPed()
    local p = VICE.getPlayerCoords()
    if a.active then
        removeShield(false)
    end
    a.id = n
    local q = VICE.loadModel(b[n].model)
    ...
    VICE.loadAnimDict(b[n].animDict)
