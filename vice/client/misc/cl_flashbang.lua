...
RegisterNetEvent("VICE:flashbangExplode",function(coords)
    if #(coords - VICE.getPlayerCoords()) <= b then
...
function explodeFlashbang(coords)
    ...
    TaskPlayAnim(VICE.getPlayerPed(),"anim@heists@ornate_bank@thermal_charge","cover_eyes_intro",-8.0,-8.0,1000,50,8.0,0,0,0)
...
function func_checkForFlashbang()
    if not a then
        if GetSelectedPedWeapon(VICE.getPlayerPed()) == `WEAPON_FLASHBANG` then
            a = true
        else
            a = false
        end
    else
        if IsPedShooting(VICE.getPlayerPed()) and not c then
            c = true
            Citizen.CreateThread(function()
                ...
                if e ~= 0 then
                    ...
                    AddExplosion(coords, 25, 0.0, 1.0, nil, true, true)
                    TriggerServerEvent("VICE:flashbangThrown", f)
                end
            end)
...
VICE.createThreadOnTick(func_checkForFlashbang)