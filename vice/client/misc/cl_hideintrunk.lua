...
local j = false
function tVICE.isPlayerHidingInBoot()
    return j
end
local function k(vehicle, l)
    if tVICE.isHandcuffed() or tVICE.isTazed() then
        return false
    end
    ...
    if l and #(VICE.getPlayerCoords() - l) > 1.0 then
        return false
    end
    return true
end
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            local r = VICE.getClosestVehicle(7.0)
            local s = GetVehicleClass(r)
            local t = GetVehicleDoorLockStatus(r)
            if q == 0 and a[s] and GetEntityHealth(VICE.getPlayerPed()) > 102 and not noclipActive then
                if r and r ~= 0 then
                    o = 0
                    local u = GetEntityBoneIndexByName(r, "boot")
                    local v = GetWorldPositionOfEntityBone(r, u)
                    local w = #(v - VICE.getPlayerCoords())
                    if w < 2 then
                        ...
                        if IsDisabledControlJustReleased(1, 137) and k(vehicle) then
                            if not b(vehicle) then
                                local l = VICE.getPlayerCoords()
                                tVICE.startCircularProgressBar("",2000,nil,function()
                                end)
                                if k(vehicle, l) then
                                    if t <= 1 then
                                        tVICE.setCanAnim(false)
                                        VICE.setWeapon(p, "WEAPON_UNARMED", true)
                                        j = true
                                        ...
                                        Wait(100)
                                        VICE.loadAnimDict("timetable@floyd@cryingonbed@base")
                                        ...
                                        while z and j do
                                            ...
                                            if IsDisabledControlPressed(0, 177) then
                                                if GetVehicleDoorLockStatus(r) <= 1 then
                                                    ...
                                                    VICE.notify("~r~Vehicle is locked, cannot get in boot.")
                                                end
                                            end
                                        ...
                                        VICE.notify("~r~Vehicle is locked, cannot get out of boot.")
                                    end
                                else
                                    VICE.notify("~r~You and the vehicle must remain stationary to get in.")
                                end
                            else
                                VICE.notify("~r~Someone is already in this boot.")
                            end
                        end
                    else
                        ClearHelp(true)
...
RegisterNetEvent("VICE:removeHiddenInBoot",function()
    j = false
end)