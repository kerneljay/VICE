...
Citizen.CreateThread(function()
    for c, d in pairs(b.weapons) do
        if not a.weapons[d.hash] then
            ...
            elseif d.class == "Mosin" then
                a.weapons[d.hash] = {bone = 24818,offset = vector3(-0.12, -0.12, -0.13),rotation = vector3(100.0, -3.0, 5.0),model = GetHashKey(d.model),type = "Mosin"}
            end
        end
    end
end)
AddEventHandler("VICE:setDiagonalWeapons",function()
    if not LocalPlayer.state.weaponsDiagonal then
        LocalPlayer.state:set("weaponsDiagonal", true, true)
    end
end)
AddEventHandler("VICE:setVerticalWeapons",function()
    if LocalPlayer.state.weaponsDiagonal then
        LocalPlayer.state:set("weaponsDiagonal", nil, true)
    end
end)
AddEventHandler("VICE:setFrontAR",function()
    if not LocalPlayer.state.frontAR then
        LocalPlayer.state:set("frontAR", true, true)
    end
end)
AddEventHandler("VICE:setBackAR",function()
...
local function g()
    ...
    local i = tVICE.getWeapons()
    local j = false
    local k = globalOnPoliceDuty and VICE.getPlayerVehicle() ~= 0
    for l in pairs(i) do
        local m = GetHashKey(l)
        local n = a.weapons[m]
        if n then
            local o = (not k or n.type ~= "Heavy") and (VICE.isEmergencyService() or not isInGreenzone) and not VICE.isPlayerInAnimalForm()
