        ...
        if t then
            ...
            ontick_prisonControlPanel = function()
                ...
                elseif IsControlJustPressed(0, 168) then
                    if VICE.isDev() and globalOnPrisonDuty then
                        d = true
                        setCursor(1)
                        inGUIVICE = true
                    end
    ...
    if d then
        ...
        for r = 1, #j, 1 do
            if j[r].name == "All" then
                ...
                -- end
                if not CursorInArea(GetArea(333333333.497, 0.819, 0.185, 0.058)) then
                    DrawAdvancedText(3333.592, 0.82, 0.005, 0.0028, 0.4, "All", 255, 255, 255, 255, 0, 0)
                else
                    if f then
                        if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                            if h[j[r].doorHash].currentState == 5 or h[j[r].doorHash].currentState == 0 then
                                TriggerServerEvent("VICE:prisonSetDoorState", j[r].doorHash, 1)
                            else
                                TriggerServerEvent("VICE:prisonSetDoorState", j[r].doorHash, 5)
                            end
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            f = false
                            SetTimeout(
                        ...
                        DrawAdvancedText(33333.592, 0.82, 0.005, 0.0028, 0.4, "All", 138, 138, 138, 255, 0, 0)
                    else
                        DrawAdvancedText(3333.592, 0.82, 0.005, 0.0028, 0.4, "All", 255, 255, 255, 255, 0, 0)
                    end
                end
        ...
        if not CursorInArea(GetArea(0.791, 0.819, 0.081, 0.06)) then
            ...
            if e then
                if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                    c = not c
                    TriggerServerEvent("VICE:prisonToggleLockdown", c)
                end
...
VICE.createThreadOnTick(func_drawControlPanel)
RegisterNetEvent(
    "VICE:prisonUpdateGuardNumber",
    function(v)
        g = v
    end
)
    ...
    function(s, t)
        if t then
            ...
            ontick_openPrisonDoor = function(w)
                ...
                if g > 4 then
                    if
                        tVICE.isStaffedOn() or globalOnPrisonDuty and w.overrideLock or globalOnPoliceDuty and w.pdOpen or
                    ...
                    if
                        tVICE.isStaffedOn() or globalOnPrisonDuty or globalOnPoliceDuty and w.pdOpen or
                            (globalNHSOnDuty or globalLFBOnDuty) and w.nhsOpen
                     then
                        x = " (E to toggle lock) "
                        y = true
                ...
                if h[w.doorHash].currentState == 5 or h[w.doorHash].currentState == 0 then
                    VICE.DrawText3D(w.coords, w.doorName .. x .. " 🔓", 0.45, 4)
                else
                    VICE.DrawText3D(w.coords, w.doorName .. x .. " 🔒", 0.45, 4)
                end
                if
                    IsControlJustPressed(0, 38) and
                        (tVICE.isStaffedOn() or globalOnPrisonDuty and (y or w.overrideLock) or
                            globalOnPoliceDuty and w.pdOpen or
                            (globalNHSOnDuty or globalLFBOnDuty) and w.nhsOpen)
                 then
                    VICE.loadAnimDict("anim@heists@keycard@")
                    Citizen.CreateThread(
                        function()
                    ...
                    if h[w.doorHash].currentState == 1 then
                        TriggerServerEvent("VICE:prisonSetDoorState", w.doorHash, 5)
                    else
                        TriggerServerEvent("VICE:prisonSetDoorState", w.doorHash, 1)
                    end
                end
            end
            for k, l in pairs(a.doors) do
