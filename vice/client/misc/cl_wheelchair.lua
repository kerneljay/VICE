RegisterCommand("wheelchair",function()
    if globalNHSOnDuty then
        local a = VICE.loadModel("prop_wheelchair_01")
        local b = CreateObject(a, GetEntityCoords(VICE.getPlayerPed()), true)
        SetModelAsNoLongerNeeded(a)
    end
end,false)
RegisterCommand("removewheelchair",function()
    if globalNHSOnDuty then
        local b = GetClosestObjectOfType(GetEntityCoords(VICE.getPlayerPed()), 10.0, `prop_wheelchair_01`)
...
Citizen.CreateThread(function()
    while true do
        local c = 500
        local f = VICE.getPlayerVehicle()
        if f == 0 then
            local d = VICE.getPlayerPed()
            local e = GetEntityCoords(d)
            local g = GetClosestObjectOfType(e, 3.0, `prop_wheelchair_01`, false)
...
function SitOnWheelchair(x)
    ...
    AttachEntityToEntity(VICE.getPlayerPed(), x, 0, 0, 0.0, 0.4, 0.0, 0.0, 180.0, 0.0, false, false, false, false, 2)
    local z = GetEntityHeading(x)
    while IsEntityAttachedToEntity(VICE.getPlayerPed(), x) do
        Citizen.Wait(5)
        if IsPedDeadOrDying(VICE.getPlayerPed()) then
            DetachEntity(VICE.getPlayerPed(), true, true)
        end
        if not IsEntityPlayingAnim(VICE.getPlayerPed(), "missfinale_c2leadinoutfin_c_int", "_leadin_loop2_lester", 3) then
            TaskPlayAnim(VICE.getPlayerPed(),"missfinale_c2leadinoutfin_c_int","_leadin_loop2_lester",8.0,8.0,-1,69,1,false,false,false)
        end
        local A = false
        ...
        if IsControlJustPressed(0, 73) or IsControlJustPressed(0, 177) then
            ...
            SetEntityCoords(VICE.getPlayerPed(), B, C, D)
            A = true
        end
        if A and not NetworkHasControlOfEntity(x) and GetGameTimer() - w >= 200 then
            local E = NetworkGetNetworkIdFromEntity(x)
            if E > 0 then
                TriggerServerEvent("VICE:syncWheelchairPosition", E, GetEntityCoords(x, true), GetEntityHeading(x))
                w = GetGameTimer()
            end
        end
    end
...
function PickUpWheelchair(x)
    ...
    TriggerServerEvent("VICE:wheelchairAttachPlayer", F)
    while not NetworkHasControlOfEntity(x) or not IsEntityAttachedToEntity(x, VICE.getPlayerPed()) do
        Wait(0)
    end
    LoadAnim("anim@heists@box_carry@")
    Wait(0)
    while IsEntityAttachedToEntity(x, VICE.getPlayerPed()) do
        Citizen.Wait(5)
        if not IsEntityPlayingAnim(VICE.getPlayerPed(), "anim@heists@box_carry@", "idle", 3) then
            TaskPlayAnim(VICE.getPlayerPed(), "anim@heists@box_carry@", "idle", 8.0, 8.0, -1, 50, 0, false, false, false)
        end
        if IsPedDeadOrDying(VICE.getPlayerPed()) then
...
RegisterNetEvent("VICE:wheelchairAttachPlayer",function(F, G)
