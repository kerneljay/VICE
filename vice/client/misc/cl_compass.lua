...
function tVICE.isCompassEnabled()
    return showCompass
end
function tVICE.setCompassEnabled(f)
    showCompass = f
    SetResourceKvp("vice_compass", tostring(f))
end
local function degreesToCompassDirection(degrees)
...
Citizen.CreateThread(
    function()
        local g = GetResourceKvpString("vice_compass") or "false"
        if g == "false" then
            showCompass = false
        else
            showCompass = true
...
local function h(i, j, k, l)
    VICE.DrawText(j, k, i, l.size, 0, 0, {l.colour.r, l.colour.g, l.colour.b, l.colour.a}, true)
end
local gangPingPositions = {} 
...
local function m()
    if showCompass and not globalHideUi then
        ...
        if a.compass.followGameplayCam then
            ...
            o = 360.0 - GetEntityHeading(VICE.getPlayerPed())
        end
        ...
        if showCompass then
            ...
            for _, pingData in pairs(Z) do
                local gangPingPos = pingData.coords
                local direction = (math.deg(math.atan2(gangPingPos.y - playerPos.y, gangPingPos.x - playerPos.x)) + 360) % 360
                local compassDirection = degreesToCompassDirection(direction)

                local isGangPing = false

                if isGangPing then
                    local gangPingDirection = (direction - o + 360.0) % 360.0
                    local gangPingPosition = a.compass.position.x + a.compass.width * (gangPingDirection / a.compass.fov)

                    gangPingPositions[_] = gangPingPosition
                else
                    h(
                        compassDirection,
                        gangPingPosition, 
            ...
            for _, gangPingPosition in pairs(gangPingPositions) do
                DrawRect(
                    gangPingPosition,
                    a.compass.position.y + a.compass.needle.textOffset,
                    a.compass.needle.textSize,
                    a.compass.needle.textSize,
                    255,
                    0,
                    0,
