RMenu.Add("policedog","main",RageUI.CreateMenu("Dog Support Unit", "Dog Management", VICE.getRageUIMenuWidth(), VICE.getRageUIMenuHeight()))
AddEventHandler("VICE:setPoliceOnDuty",function(a)
    if a then
        RegisterKeyMapping("k9attack", "Police Dog Attack", "keyboard", "INSERT")
    end
end)
AddEventHandler("VICE:setPrisonGuardOnDuty",function(b)
    if b then
        RegisterKeyMapping("k9attack", "Police Dog Attack", "keyboard", "INSERT")
    end
end)
...
local i = "K9"
local j = "K9TARGET"
RegisterNetEvent("VICE:policeDogMenu",function()
    ...
    if globalOnPrisonDuty and VICE.hasHmpCallsign() then
        RMenu:Get("vicescenemenu", "main"):SetSubtitle("HMP~w~: " .. VICE.getHmpCallsign() .. " - " .. VICE.getHmpRank() .. " - " .. VICE.getHmpName())
    end
    RageUI.Visible(RMenu:Get("policedog", "main"), true)
end)
RegisterNetEvent("VICE:sendClientRagdoll",function()
    SetPedToRagdoll(VICE.getPlayerPed(), 12000, 12000, 0, 0, 0, 0)
    showNotification("~y~~h~Alert~h~~s~: " .. "You have been bitten by a police dog.")
end)
RegisterNetEvent("VICE:policeDogAttack",function()
...
local m = {breed = 1}
local function n()
    local o = GetResourceKvpString("vice_policedog_customisations")
    if o and o ~= "" and o ~= "null" then
...
local function r(s)
    ...
    local q = k[m.breed]
    p[q] = s
    SetResourceKvp("vice_policedog_customisations", json.encode(p))
end
...
function tpDog()
    local z = GetEntityCoords(VICE.getPlayerPed(), true)
                                    ...
                                    if M < 2.0 then
                                        ...
                                        for O, P in pairs(N) do
                                            if I == GetPlayerPed(P) then
                                                ...
                                                TriggerServerEvent("VICE:performDogLog","Instructed dog to attack ID: " .. Q)
                                                TriggerServerEvent("VICE:serverDogAttack", Q)
                                                H = true
                                                Wait(1000)
    ...
    while f.currentAction == d.SearchNearby and DoesEntityExist(f.handle) do
        ...
        for W, X in pairs(GetActivePlayers()) do
            ...
            if GetVehiclePedIsUsing(Y) == 0 and IsEntityVisible(Y) and #(GetEntityCoords(Y, true) - L) < 5.0 then
                ...
                local _ = VICE.clientGetUserIdFromSource(Z)
                if VICE.getJobType(_) == "" then
        ...
        if #V > 0 then
            ...
            TriggerServerEvent("VICE:policeDogSniffPlayer", a0.source)
        end
        Citizen.Wait(5000)
    end
end
...
function newDog(q, a4)
    local a5 = VICE.getPlayerPed()
    if not IsPedInAnyPoliceVehicle(a5) then
        local a6 = VICE.loadModel(q)
        ...
        if DoesEntityExist(f.handle) then
            ...
            TriggerServerEvent("VICE:performDogLog", "Created a new police dog")
        end
    end
end
function deleteDog()
    ...
    f.active = false
    f.handle = 0
    TriggerServerEvent("VICE:performDogLog", "Deleted dog")
end
function dogFollow()
    if not f.active then
        return
    ...
    if f.currentAction ~= d.Follow then
        ClearPedTasks(f.handle)
        TaskFollowToOffsetOfEntity(f.handle, VICE.getPlayerPed(), 0.0, 0.0, 0.0, 7.0, -1, 10.0, true)
        f.currentAction = d.Follow
    else
        ClearPedTasks(f.handle)
        f.currentAction = d.Follow
...
function showNotification(ab, A)
    ...
    elseif ab == e.Error then
        AddTextComponentSubstringPlayerName("~h~Error~h~~s~: " .. A)
    ...
    elseif ab == e.Info then
        AddTextComponentSubstringPlayerName("~h~Info~h~~s~: " .. A)
    else
        AddTextComponentSubstringPlayerName(A)
    end
...
RegisterNetEvent("VICE:policeDogIndicate",function(ad)
    ...
    notify("Your dog starts indicating.")
    ClearPedTasks(f.handle)
    local ag = 0
