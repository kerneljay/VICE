...
local function i()
    ...
    if j == 0 then
        a = true
        SetResourceKvpInt("vice_quests_completedall", 1)
        TriggerServerEvent("VICE:claimQuestReward")
    end
    return a
end
local function l()
    hasCompletedAllQuestsInt = GetResourceKvpInt("vice_quests_completedall")
    if not hasCompletedAllQuestsInt then
        SetResourceKvpInt("vice_quests_completedall", 0)
    end
    if hasCompletedAllQuestsInt == 0 then
        for m = 1, 50 do
            local n = GetResourceKvpInt("vice_quests_q" .. m .. "_status")
            if not n then
                SetResourceKvpInt("vice_quests_q" .. m .. "_status", 0)
                n = 0
            end
            if n == 0 then
                c[m] = true
...
local function o(p)
    c[p] = nil
    SetResourceKvpInt("vice_quests_q" .. p .. "_status", 1)
    TriggerServerEvent("VICE:setQuestCompleted")
    i()
end
local function q()
    local j = 50
...
Citizen.CreateThread(function()
    ...
    for w, k in pairs(c) do
        ...
        VICE.createArea("quests_prop_id_" .. w,undiscoveredQ.pos,100.0,6,x,z,A,{undiscoveredQID = w, undiscoveredQ = undiscoveredQ})
        local B = function()
        end
        local C = function()
        end
        local A = function(y)
            ...
            if IsControlJustReleased(0, 38) then
                ...
                if q() ~= 50 then
                    ...
                    DoScreenFadeIn(2000)
                    tVICE.removeArea("quests_prop_id_" .. y.undiscoveredQID)
                    tVICE.removeArea("quests_text_id_" .. y.undiscoveredQID)
                else
                    r("~g~+14 Days VICE Club Platinum", "You collected them all!", 600)
                    PlaySoundFrontend(-1, "Hack_Success", "DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS", 1)
                    ShakeGameplayCam("FAMILY5_DRUG_TRIP_SHAKE", 1.0)
        ...
        VICE.createArea("quests_text_id_" .. w,undiscoveredQ.pos,2.0,6,B,C,A,{undiscoveredQ = undiscoveredQ, undiscoveredQID = w})
    end
end)
Citizen.CreateThread(function()
    Wait(5000)
