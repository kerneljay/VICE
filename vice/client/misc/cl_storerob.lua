...
local r = {
    ...
    ["new_store"] = {
        shopNpcModel = "mp_m_shopkeep_01",
        shopNpcPosition = vector3(242.13255310059,-897.93481445312,29.623216629028),
        shopNpcHeading = 90.0,
        shopNpcHandler = 0,
        prop_safe = "v_ilev_gangsafe",
        prop_door = "v_ilev_gangsafedoor",
        safePosition = vector3(244.28575134277,-905.9794921875,29.623216629028),
        safeHeading = 90.0,
        money_prop = "bkr_prop_moneypack_03a",
        money_prop2 = "bkr_prop_moneypack_03a",
        money_prop3 = "prop_poly_bag_money",
        money_prop4 = "prop_cash_case_01",
        moneyPos = vector3(244.28575134277,-905.9794921875,29.623216629028),
        moneyPos2 = vector3(244.28575134277,-905.9794921875,29.623216629028) + vector3(0.0, 0.0, 0.15),
        moneyPos3 = vector3(244.28575134277,-905.9794921875,29.623216629028),
        moneyPos4 = vector3(244.28575134277,-905.9794921875,29.623216629028),
        moneyHeading = 90.0,
        moneyHeading2 = 90.0,
        moneyHeading3 = 90.0,
        moneyHeading4 = 90.0,
...
AddEventHandler(
    ...
    function(t, u)
        if u then
            TriggerServerEvent("VICE:getStoreRobBlips")
            while p == nil do
                Wait(0)
            end
            while true do
...
function playStorePedCashRegisterAnimation(x, w, v)
    if not IsPedDeadOrDying(x) then
        if IsEntityPlayingAnim(x, "mp_am_hold_up", "holdup_victim_20s", 3) then
            ...
            if not IsPedDeadOrDying(x) then
                local z = GetClosestObjectOfType(GetEntityCoords(x), 5.0, `prop_till_01`)
                ...
                local A = VICE.loadModel("prop_poly_bag_01")
                        ...
                        function()
                            while true do
                                ...
                                if DoesEntityExist(B) then
                                    if #(GetEntityCoords(VICE.getPlayerPed()) - GetEntityCoords(B)) <= 1.5 then
                                        PlaySoundFrontend(
                                            -1,
                                            "ROBBERY_MONEY_TOTAL",
                                        ...
                                        TriggerServerEvent("VICE:pickupCashFromApu", v)
                                        DeleteObject(B)
                                        break
                                    end
                                else
...
Citizen.CreateThread(
    function()
        while true do
            local t = GetEntityCoords(VICE.getPlayerPed(), true)
            if r then
                for v, w in pairs(r) do
...
function refreshStoreBlips(u)
    ...
    for D, E in pairs(p) do
        ...
        if E.beingrobbed == true then
            tempBlip = tVICE.addBlip(F.x, F.y, F.z, 52, 1, "Robbable Store [BEING ROBBED]")
        else
            tempBlip = tVICE.addBlip(F.x, F.y, F.z, 52, 2, "Robbable Store")
        end
...
Citizen.CreateThread(
    function()
        for v, w in pairs(r) do
            local G = VICE.loadModel(w.shopNpcModel)
            local H =
                CreatePed(
                26,
                G,
            ...
            SetPedFleeAttributes(H, 0, 0)
            local I = VICE.loadModel(w.prop_safe)
            local J = VICE.loadModel(w.prop_door)
            local K = VICE.loadModel(w.money_prop)
            local L = VICE.loadModel(w.money_prop2)
            local M = VICE.loadModel(w.money_prop3)
            local N = VICE.loadModel(w.money_prop4)
        ...
        while true do
            local U = GetEntityCoords(VICE.getPlayerPed())
            for v, w in pairs(r) do
                             ...
                             then
                                if not q then
                                    if GetSelectedPedWeapon(VICE.getPlayerPed()) == `WEAPON_UNARMED` then
                                        VICE.notify("~r~You need a weapon in your hands to rob this store!")
                                    else
                                        TriggerServerEvent("VICE:initiateStoreRobbery", v)
...
RegisterNetEvent(
    "VICE:beginStoreRobbingAnimations",
    function(v)
...
RegisterNetEvent(
    ...
    function(Y, v)
        ...
        while q do
            local U = GetEntityCoords(VICE.getPlayerPed())
            if #(Z - U) > 20 then
                TriggerServerEvent("VICE:forceEndRobbery", v)
...
RegisterNetEvent(
    ...
    function(v)
        ...
        local G = VICE.loadModel(r[v].shopNpcModel)
        local H =
            CreatePed(
            26,
            G,
                ...
                if w.distanceToPlayer < 2.0 then
                    if safeCrackingState == "complete" then
                        ...
                        if IsControlJustReleased(1, 51) then
                            ...
                            TriggerServerEvent("VICE:completeSafeCracking", v)
                            r[v].robberyInProgress = false
                            safeCrackingState = "setup"
                            Wait(5000)
                            TriggerServerEvent("VICE:syncCloseSafeDoor", v)
                        end
                    elseif w.robberyInProgress then
                        holdup_DisplayHelpText2("Press ~INPUT_CONTEXT~ to start cracking the safe!")
                        if IsControlJustReleased(1, 51) then
                            VICE.notify("~g~Started cracking safe..")
                            LoadResources()
...
function RunMiniGame(v, ab, ac)
    ...
    while safeCrackingInProgress do
        ...
        elseif safeCrackingState == "cracking" then
            ...
            local ae = GetEntityHealth(VICE.getPlayerPed())
            if ae <= 102 or tVICE.isHandcuffed() then
...
function GetSafeDoorAnimOffsetPosition(ah, ai, aj)
    ...
    return vector3(ah.x + ao, ah.y + ap, GetEntityCoords(VICE.getPlayerPed()).z)
end
function PlaySafeCrackIntroAnim(aq, h)
    local ar = "mini@safe_cracking"
    local as = "step_into"
    loadAnimDict(ar)
    TaskPlayAnimAdvanced(VICE.getPlayerPed(), ar, as, aq.x, aq.y, aq.z, 0.0, 0.0, h, 8.0, 8.0, -1, 2, 0.7, 0, 0)
    Wait(0)
    Wait(1000)
    safeCrackingState = "cracking"
end
function HandleSafeDialMovement()
    local at = VICE.getPlayerPed()
    local ar = "mini@safe_cracking"
    local au = ""
    ...
    loadAnimDict(ar)
    if not IsEntityPlayingAnim(VICE.getPlayerPed(), ar, au, 3) then
        TaskPlayAnimAdvanced(VICE.getPlayerPed(), ar, au, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    end
end
function HandleIncorrectMovement(ad, b)
    ...
    loadAnimDict(ar)
    TaskPlayAnimAdvanced(VICE.getPlayerPed(), ar, ax, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    d = "rotation.idle"
end
...
function EndMiniGame(v, ab)
    ...
    Wait(2500)
    TriggerServerEvent("VICE:syncOpenSafeDoor", v)
    safeCrackingState = "setup"
