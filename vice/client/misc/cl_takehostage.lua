    ...
    if not a.InProgress and u then
        ...
        if i then
            ...
            if w ~= -1 then
                ...
                if x then
                    VICE.setWeapon(PlayerPedId(), foundWeapon, true)
                    a.InProgress = true
                    a.startTime = GetGameTimer()
                    a.targetSrc = w
                    TriggerServerEvent("VICE:takeHostageSync", w)
                    a.type = "agressor"
                else
...
RegisterNetEvent("VICE:takeHostageSyncTarget",function(y)
...
RegisterNetEvent("VICE:takeHostageReleaseHostage",function()
    a.InProgress = false
    a.type = ""
    DetachEntity(PlayerPedId(), true, false)
    q("reaction@shove")
...
-- RegisterNetEvent("VICE:takeHostageKillHostage",function()
--     a.InProgress = false
--     a.type = ""
...
RegisterNetEvent("VICE:takeHostageCl_stop",function()
    a.InProgress = false
    a.type = ""
    ClearPedSecondaryTask(PlayerPedId())
    DetachEntity(PlayerPedId(), true, false)
    ...
    while true do
        if a.type == "agressor" then
            ...
            if IsEntityDead(PlayerPedId()) then
                ...
                RemoveAnimDict("reaction@shove")
                TriggerServerEvent("VICE:takeHostageReleaseHostage", a.targetSrc)
            end
            ...
            elseif IsDisabledControlJustPressed(0, 74) and GetGameTimer() - a.startTime > 4000 then
                ...
                RemoveAnimDict("anim@gangops@hostage@")
                TriggerServerEvent("VICE:takeHostageKillHostage", a.targetSrc)
                TriggerServerEvent("VICE:takeHostageStop", a.targetSrc)
                Wait(100)
...
function VICE.takeHostageInProgress()
    return a.InProgress
end
