...
local g = 0
local h = false
RegisterNetEvent("VICE:setPoliceOnDuty",
    function(i)
...
function VICE.isPedBeingTackled()
    return h
end
RegisterNetEvent(
    "VICE:playTackle",
    ...
    for p, q in ipairs(GetActivePlayers()) do
        if q ~= PlayerId() then
            ...
            if t ~= "metpd" and t ~= "hmp" and t ~= "npas" then
                ...
                local v = #(u - VICE.getPlayerCoords())
                if v < n then
                    n = v
                    o = r
                end
...
function func_tackleManagement()
    if h then
        SetPedToRagdoll(VICE.getPlayerPed(), 1000, 1000, 0, 0, 0, 0)
    end
    if globalOnPoliceDuty or w() or globalUKBFOnDuty or tVICE.isStaffedOn() or globalOnPrisonDuty and globalPlayerInPrisonZone then
        if IsControlPressed(0, a["LEFTSHIFT"]) and IsControlPressed(0, a["G"]) then
            if not b and GetGameTimer() - g > 10 * 1000 and GetEntityHealth(PlayerPedId()) > 102 and not VICE.isKnockedOut() then
                local x = m()
                if x then
                    if not b and not c and not IsPedInAnyVehicle(VICE.getPlayerPed()) and not IsPedInAnyVehicle(GetPlayerPed(x)) then
                        b = true
                        g = GetGameTimer()
                        TriggerServerEvent("VICE:tryTackle", x)
                        DisablePlayerFiring(VICE.getPlayerPed(), true)
                        DisableControlAction(0, 22, true)
                        DisableControlAction(0, 25, true)
                        DisableControlAction(0, 154, true)
...
VICE.createThreadOnTick(func_tackleManagement)
