...
local h=vector3(412.78302001953,9.3809432983398,91.935417175293)
RMenu.Add('VICEChips','cashier',RageUI.CreateMenu("","",VICE.getRageUIMenuWidth(),VICE.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('VICEChips','cashier'):SetSubtitle("Chips")
RMenu.Add('casino_enter','casino',RageUI.CreateMenu("","",VICE.getRageUIMenuWidth(),VICE.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('casino_enter','casino'):SetSubtitle("Enter")
RMenu.Add('casino_exit','casino',RageUI.CreateMenu("","",VICE.getRageUIMenuWidth(),VICE.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
RMenu:Get('casino_exit','casino'):SetSubtitle("Exit")
RMenu.Add('casino_rooftop_exit','casino',RageUI.CreateMenu("","",VICE.getRageUIMenuWidth(),VICE.getRageUIMenuHeight(),"shopui_title_casino","shopui_title_casino"))
...
Citizen.CreateThread(function()
    for i, j in pairs(h) do
        tVICE.createDynamicPed(j.model,j.pedPosition,100.0,true,"mini@strip_club@idles@bouncer@base","base",75.0,nil,function(k)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('VICEChips', 'cashier')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Buy chips","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    ...
                    if tonumber(q)then 
                        q=tonumber(q)
                        if q>0 then 
                            TriggerServerEvent("VICE:buyChips",q)
                        else 
                            VICE.notify("~r~Negative amount.")
                        end 
                    else 
            ...
            RageUI.ButtonWithStyle("Buy all chips","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    TriggerServerEvent("VICE:buyChips")
                end 
            end)
            RageUI.ButtonWithStyle("Sell chips","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    ...
                    if tonumber(q)then 
                        q=tonumber(q)
                        if q>0 then 
                            TriggerServerEvent("VICE:sellChips",q)
                        else 
                            VICE.notify("~r~Negative amount.")
                        end 
                    else 
            ...
            RageUI.ButtonWithStyle("Sell all chips","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    TriggerServerEvent("VICE:sellChips")
                end 
            end)
        end)
    end
    if RageUI.Visible(RMenu:Get('casino_exit', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Exit Diamond Casino","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    TriggerServerEvent("VICE:exitDiamondCasino")
                    tVICE.teleport(e.x,e.y,e.z)
                    if cardObjects then 
    ...
    if RageUI.Visible(RMenu:Get('casino_enter', 'casino')) then
        RageUI.DrawContent({ header = true, glare = false, instructionalButton = false}, function()
            RageUI.ButtonWithStyle("Enter Diamond Casino","",{RightLabel="→→→"},true,function(n,o,p)
                if p and VICE.getPlayerCombatTimer() == 0 then 
                    TriggerServerEvent("VICE:enterDiamondCasino")
                    tVICE.teleport(f.x,f.y,f.z)
                end 
            end)
            RageUI.ButtonWithStyle("Diamond Casino Rooftop","",{RightLabel="→→→"},true,function(n,o,p)
                if p then 
                    tVICE.teleport(g.x,g.y,g.z)
                end 
            end)
        end)
    end
...
function showCasinoChipsCashier(u)
    ...
    RageUI.Visible(RMenu:Get('VICEChips','cashier'),u)
end
...
Citizen.CreateThread(function()
    while true do 
        ...
        --DrawMarker(27,h.x,h.y,h.z-1.0,0,0,0,0,0,0,1.001,1.0001,0.5001,255,255,255,200,0,0,0,0)
        Wait(0)
    end 
end)
Citizen.CreateThread(function()
    while true do 
        local v=GetEntityCoords(VICE.getPlayerPed())
        a=#(v-e)
        b=#(v-f)
        c=#(v-g)
        Wait(100)
...
RegisterNetEvent("VICE:setDisplayChips")
AddEventHandler("VICE:setDisplayChips",function(x)
    d=getMoneyStringFormatted(tostring(x))
end)

RegisterNetEvent("VICE:chipsUpdated",function()
...
AddEventHandler("VICE:onClientSpawn",function(D, E)
    if E then
        ...
        for H,k in pairs(h)do 
            tVICE.addBlip(k.entryPosition.x,k.entryPosition.y,k.entryPosition.z,683,0,"Chips Cashier",0.7,true)
            tVICE.addMarker(k.entryPosition.x,k.entryPosition.y,k.entryPosition.z,1.0,1.0,1.0,138,43,226,70,50,27)
            VICE.createArea("chipscashier_"..H,k.entryPosition,1.5,6,C,E,F,{})
        end
        for I,J in pairs(z)do 
            VICE.createArea("casinos_"..I,J.position,J.radius,6,function()
            end,function()
            end,G,{})
        end 
    end
