    ...
    if VICE.globalOnPoliceDuty() then 
        if m[1]~=nil then 
            if tonumber(m[1])>100 then 
                d=tonumber(m[1])
                VICE.notify(string.format("~g~Maximum speed set to %smph",tonumber(m[1])))
            else 
                d=101
                VICE.notify("~r~Minimum speed you can set is 101mph!")
            end 
        end 
    else 
        VICE.notify("~r~Speed gun is not enabled!")
    end 
end)
...
VICE.createThreadOnTick(func_drawRadar)
Citizen.CreateThread(function()
    while true do 
        if VICE.globalOnPoliceDuty() and not usingDelgun then
            if GetSelectedPedWeapon(VICE.getPlayerPed())==a then 
                b=true 
            else 
                b=false 
            end
            if b and VICE.getPlayerVehicle()==0 then 
                local n=VICE.getPlayerId()
                if IsPlayerFreeAiming(n)then 
                    ...
                    if p~=0 and GetPedInVehicleSeat(p,-1)==e and not IsPedInAnyPlane(e) and not IsPedInAnyHeli(e) then 
                        ...
                        if k[f]~=nil then 
                            ...
                            VICE.notify(string.format("~h~Vehicle Flagged:~s~~n~Plate %s is flagged for:~n~%s",f,r))
                            soundPlayed=true
                        ...
                        if h>d and h>101 then 
                            ...
                            if not j[q]then 
                                ...
                                TriggerServerEvent("VICE:speedGunFinePlayer",GetPlayerServerId(NetworkGetPlayerIndexFromPed(e)),h-d)
                                Citizen.Wait(3000)
                            else 
                                VICE.notify("~r~This vehicle has been fined recently!")
                            end 
                            else i="~w~"
                        end 
                    end 
...
RegisterNetEvent("VICE:speedGunPlayerFined")
AddEventHandler("VICE:speedGunPlayerFined",function()
    PlaySoundFrontend(-1,"ScreenFlash","MissionFailedSounds",1)
    StartScreenEffect("FocusOut",0,false)
    Wait(2000)
    StopScreenEffect("FocusOut")
end)
AddEventHandler("VICE:setFlagVehicles",function(s)
    k=s 
end)