...
local function b()
    ...
    TriggerServerEvent("VICE:startThrowSmokeGrenade", c)
    Citizen.Wait(100)
    ...
    TriggerServerEvent("VICE:endThrowSmokeGrenade", c, g)
end
local function h(i)
    local j = GetSelectedPedWeapon(i.playerPed)
    if j == `WEAPON_SMOKEGRENADEVICE` or j == `WEAPON_SMOKEGRENADEVICEPD` then
...
RegisterNetEvent("VICE:displaySmokeGrenade",function(c, g)
    local l = {coords = g, isDeleted = false}
    VICE.createArea("smoke_" .. tostring(c),g,250.0,1000.0,k,m,function()end,l)
    Citizen.Wait(60000)
    tVICE.removeArea("smoke_" .. tostring(c))
    if l.handle then
        StopParticleFxLooped(l.handle, false)
        l.handle = nil
        l.isDeleted = true
