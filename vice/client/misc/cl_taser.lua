...
RegisterNetEvent("VICE:receiveBarbs",function(e)
    c = true
    d = e
end)
RegisterNetEvent("VICE:receiveActivation",function()
    if not tVICE.isStaffedOn() then
        displayTaserNotification("You are currently being ~b~activated~w~.")
        SetPedToRagdoll(VICE.getPlayerPed(), 5000, 5000, 0, 0, 0, 0)
    end
end)
RegisterNetEvent("VICE:arcSuccessful",function()
    ...
    TriggerServerEvent("VICE:playTaserSound", GetEntityCoords(VICE.getPlayerPed()), "arcsound")
    b = false
...
RegisterNetEvent("playTaserSoundClient",function(f, g)
    local h = GetEntityCoords(VICE.getPlayerPed(), true)
    local i = #(h - f)
    if i <= 15 then
        SendNUIMessage({transactionType = g})
    end
end)
RegisterNetEvent("VICE:barbsNoLonger",function()
    c = false
    d = 0
    displayTaserNotification("Your taser ~b~barbs ~w~have been ~b~disconnected~w~.")
end)
RegisterNetEvent("VICE:barbsRippedOut",function(e)
...
RegisterNetEvent("VICE:reloadTaser",function(j)
    if GetSelectedPedWeapon(VICE.getPlayerPed()) == GetHashKey(a.hash) then
        MakePedReload(VICE.getPlayerPed())
        if a.cartridges == 0 then
            a.cartridges = 2
            if a.reactivate1.active then
                TriggerServerEvent("VICE:barbsNoLongerServer", a.reactivate1.serverId)
            end
            if a.reactivate2.active then
                TriggerServerEvent("VICE:barbsNoLongerServer", a.reactivate2.serverId)
            end
            a.reactivate1 = {active = false, ped = 0, serverId = 0}
            a.reactivate2 = {active = false, ped = 0, serverId = 0}
        ...
        if k.weapon == GetHashKey(a.hash) then
            if IsControlJustPressed(1, 308) then
                if a.reactivate1.active == true then
                    ...
                    TriggerServerEvent("VICE:playTaserSound", k.playerCoords, "reactivate")
                    TriggerServerEvent("VICE:reactivatePed", a.reactivate1.serverId)
                end
            elseif IsControlJustPressed(1, 307) then
                if a.reactivate2.active == true then
                    ...
                    TriggerServerEvent("VICE:playTaserSound", k.playerCoords, "reactivate")
                    TriggerServerEvent("VICE:reactivatePed", a.reactivate2.serverId)
                end
            elseif IsControlJustPressed(1, 121) and b then
                TriggerServerEvent("VICE:arcTaser")
            end
            if a.cartridges < 1 then
                ...
                if IsPedShooting(k.playerPed) then
                    TriggerServerEvent("VICE:playTaserSound", k.playerCoords, "taser")
                    a.cartridges = a.cartridges - 1
                    local l, m = GetEntityPlayerIsFreeAimingAt(k.playerId)
                    if l and IsEntityAPed(k.playerPed) then
                        local n = VICE.getPedServerId(m)
                        TriggerServerEvent("VICE:receiveBarbsServer", n)
                        if n then
                            if a.cartridges == 1 then
...
VICE.createThreadOnTick(func_taserHandler)
...
Citizen.CreateThread(function()
    SetPedMinGroundTimeForStungun(VICE.getPlayerPed(), 15000)
    while true do
        if IsPedBeingStunned(VICE.getPlayerPed()) then
            stunGun()
        end
        Wait(100)
    end
        ...
        if c then
            while c do
                ...
                if IsControlJustPressed(1, 38) then
                    c = false
                    if d and d > 0 then
                        TriggerServerEvent("VICE:barbsRippedOutServer", d)
                    end
                    d = 0
...
function VICE.setStunExitsEarly()
    p = true
end
