...
RegisterCommand("flagf",function(o, p)
    if c ~= "" then
        ...
        if table.count(p) ~= 0 then
            q = table.concat(p, " ")
            TriggerServerEvent("VICE:flagVehicleAnpr", string.upper(c), q)
        else
            VICE.notify("Error~w~: You must provide a valid flag reason.")
        end
    end
end)
RegisterCommand("flagr",function(o, p)
    if d ~= "" then
        ...
        if table.count(p) ~= 0 then
            q = table.concat(p, " ")
            TriggerServerEvent("VICE:flagVehicleAnpr", string.upper(d), q)
        else
            VICE.notify("Error~w~: You must provide a valid flag reason.")
        end
    end
...
VICE.createThreadOnTick(func_drawFullAnpr)
function anprGetVehicle(u)
    ...
    local w = GetVehiclePedIsIn(VICE.getPlayerPed(), false)
    ...
    if B > 0 and IsEntityAVehicle(B) then
        ...
        if C then
            if m[C] then
                ...
                VICE.notify(string.format("ANPR Alert~w~: Plate ~b~%s ~w~is flagged for:~b~~n~%s", C, D))
                n = true
...
function isPedInPoliceVehicle()
    if IsPedInAnyVehicle(VICE.getPlayerPed(), false) then
        if GetVehicleClass(VICE.getPlayerVehicle()) == 18 and globalOnPoliceDuty then
            return true
        end
    end
    return false
...
function drawFrontAnprText()
    if b then
        DrawAdvancedText(t.x - 0.0335, t.y - 0.235, 0.1, 0.2, 0.5, "FRONT REG", 200, 0, 0, 255, 4, 0)
    ...
    elseif not b then
        ...
        g = e
        DrawAdvancedText(t.x - 0.0335, t.y - 0.235, 0.1, 0.2, 0.5, "FRONT REG", 255, 255, 255, 255, 4, 0)
...
function drawRearAnprText()
    if b then
        DrawAdvancedText(t.x + 0.036, t.y - 0.235, 0.1, 0.2, 0.5, "REAR REG", 200, 0, 0, 255, 4, 0)
    ...
    elseif not b then
        ...
        h = f
        DrawAdvancedText(t.x + 0.036, t.y - 0.235, 0.1, 0.2, 0.5, "REAR REG", 255, 255, 255, 255, 4, 0)
...
RegisterNetEvent("VICE:setFlagVehicles",function(H)
    m = H
end)