...
RegisterCommand(
    "getcasinotable",
    function()
        local K = GetEntityCoords(VICE.getPlayerPed())
        local L = GetClosestObjectOfType(K.x, K.y, K.z, 1.0, `vw_prop_casino_blckjack_01`, 0, 0, 0)
        if DoesEntityExist(L) then
            ...
            local M = GetClosestObjectOfType(K.x, K.y, K.z, 1.0, `vw_prop_casino_blckjack_01b`, 0, 0, 0)
...
Citizen.CreateThread(function()
    ...
    for O = 0, #G.blackjackTables, 1 do
        ...
        if randomBlackShit < 7 then
            ...
            dealerModel = `S_F_Y_Casino_01`
        end
        local P = G.blackjackTables[O]
        tVICE.createDynamicPed(dealerModel,P.dealerPos,P.dealerHeading,true,nil,nil,75.0,nil,function(Q)
            f[O + 1] = Q
            ...
            VICE.loadAnimDict(dealerAnimDict)
...
function resetDealerIdle(U)
    ...
    if DoesEntityExist(U) then
        ...
        TaskPlayAnim(VICE.getPlayerPed(),"anim_casino_b@amb@casino@games@shared@player@","idle_cardgames",1.0,1.0,-1,0)
        RemoveAnimDict(dealerAnimDict)
    end
end
local V = false
                ...
                if tonumber(W) then
                    ...
                    if W > 0 then
                        if not isChairHighRollers(F) and z > B then
                            VICE.notify("~r~Maximum bet is " ..tostring(getMoneyStringFormatted(B)) .." please purchase a membership for higher bets.")
                        else
                            z = W
                        end
                    end
            ...
            if IsControlJustPressed(0, 201) then 
                if tonumber(z) >= 0 then
                    ...
                    elseif isChairHighRollers(F) and z < B then
                        VICE.notify("~r~Minimum bet is " .. tostring(getMoneyStringFormatted(B)))
                    elseif z > 500000000 then
                        VICE.notify("~r~Maximum bet is £500,000,000")
                    else
    ...
    while true do
        if sittingAtBlackjackTable and A then
            ...
            if IsControlJustPressed(0, 202) and not i then
                ...
                TaskPlayAnim(VICE.getPlayerPed(), blackjackAnimDictToLoad, "sit_exit_left", 1.0, 1.0, 2500, 0)
...
RMenu.Add('viceblackjack', 'instructions', RageUI.CreateMenu("", "test",1300, 50,"casinoui_cards_blackjack", "casinoui_cards_blackjack"))
RMenu:Get('viceblackjack', 'instructions'):SetSubtitle("Blackjack Table")
RMenu.Add('viceblackjack_high', 'instructions', RageUI.CreateMenu("", "test",1300, 50,"casinoui_cards_blackjack_high", "casinoui_cards_blackjack_high"))
RMenu:Get('viceblackjack_high', 'instructions'):SetSubtitle("High Rollers Table")


RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('viceblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare=  false, instructionalButton = true }, function()           
            ...
            end, RMenu:Get('viceblackjack', 'instructions'))            
        end, function()
        end)
    end
    if RageUI.Visible(RMenu:Get('viceblackjack_high', 'instructions')) then
        RageUI.DrawContent({ header = true, glare=  false, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "The aim of Blackjack is to beat the Dealer's hand without going over 21. This game uses four 52           card decks, which are shuffled at the start of every hand.                                                                                    The dealer will stand on soft 17.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
            end, RMenu:Get('viceblackjack_high', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
function showHowToBlackjack(flag)
    ...
    if not isChairHighRollers(a) then
        RageUI.Visible(RMenu:Get('viceblackjack', 'instructions'), flag)
    else 
        RageUI.Visible(RMenu:Get('viceblackjack_high', 'instructions'), flag)
    end
end
...
function goToBlackjackSeat(an)
    ...
    c = an
    fVar3 = blackjack_func_217(VICE.getPlayerPed(), blackjack_func_218(c, 0), 1)
    fVar4 = blackjack_func_217(VICE.getPlayerPed(), blackjack_func_218(c, 1), 1)
    fVar5 = blackjack_func_217(VICE.getPlayerPed(), blackjack_func_218(c, 2), 1)
    ...
    TaskGoStraightToCoord(VICE.getPlayerPed(), ao.x, ao.y, ao.z, 1.0, 5000, ap, 0.01)
...
function betChipsForNextHand(aH, av, aI, ah, aJ, aK)
    ...
    if cardObjects[tostring(ah) .. "chips"] then
...
function requestCard()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(VICE.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@blackjack@player","request_card",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
...
function putBetOnTable()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(VICE.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@blackjack@player",getAnimNameFromBet(100),4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(3000,function()
        E = true
    end)
...
function angryIBust()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(VICE.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_terrible_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
...
function angryILost()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(VICE.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_bad_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
...
function annoyedIPushed()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(VICE.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_impartial_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
...
function happyIWon()
    ...
    local aW = blackjack_func_215(F)
    local aX = NetworkCreateSynchronisedScene(aV.x, aV.y, aV.z, aW.x, aW.y, aW.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(VICE.getPlayerPed(),aX,"anim_casino_b@amb@casino@games@shared@player@","reaction_good_var_01",4.0,-2.0,13,16,1148846080,0)
    NetworkStartSynchronisedScene(aX)
    SetTimeout(5000,function()
        E = true
    end)
...
function setBlackjackDealerPedVoiceGroup(bo, U)
    if bo == 0 then
        SetPedVoiceGroup(U, `S_M_Y_Casino_01_WHITE_01`)
    elseif bo == 1 then
        SetPedVoiceGroup(U, `S_M_Y_Casino_01_ASIAN_01`)
    elseif bo == 2 then
        SetPedVoiceGroup(U, `S_M_Y_Casino_01_ASIAN_02`)
    elseif bo == 3 then
        SetPedVoiceGroup(U, `S_M_Y_Casino_01_ASIAN_01`)
    elseif bo == 4 then
        SetPedVoiceGroup(U, `S_M_Y_Casino_01_WHITE_01`)
    elseif bo == 5 then
        SetPedVoiceGroup(U, `S_M_Y_Casino_01_WHITE_02`)
    elseif bo == 6 then
        SetPedVoiceGroup(U, `S_M_Y_Casino_01_WHITE_01`)
    elseif bo == 7 then
        SetPedVoiceGroup(U, `S_F_Y_Casino_01_ASIAN_01`)	
    elseif bo == 8 then
        SetPedVoiceGroup(U, `S_F_Y_Casino_01_ASIAN_02`)
    elseif bo == 9 then
        SetPedVoiceGroup(U, `S_F_Y_Casino_01_ASIAN_01`)
    elseif bo == 10 then
        SetPedVoiceGroup(U, `S_F_Y_Casino_01_ASIAN_02`)
    elseif bo == 11 then
        SetPedVoiceGroup(U, `S_F_Y_Casino_01_LATINA_01`)
    elseif bo == 12 then
        SetPedVoiceGroup(U, `S_F_Y_Casino_01_LATINA_02`)
    elseif bo == 13 then
        SetPedVoiceGroup(U, `S_F_Y_Casino_01_LATINA_01`)
    end
end
