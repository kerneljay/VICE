...
local j =
    tVICE.addBlip(i.test.reception.x, i.test.reception.y, i.test.reception.z, 523, 47, "DVSA Test Centre", 1.0, false)
local k = function()
...
VICE.createArea("dvsaTestCentre_", i.test.reception, 1.5, 6, k, l, m, {})
RegisterNetEvent(
    "VICE:dvsaData",
    function(n, o, p, q)
        h = true
        ...
        VICE.loadModel(i.models.camera)
...
RegisterNetEvent(
    "VICE:dvsaMessage",
    function(w, x, y)
        VICE.notifyPicture(i.images.dict, i.images.govLarge, y, w, x)
    end
)
RegisterNetEvent(
    "VICE:beginTestClient",
    function(z, A)
        if z then
            TriggerServerEvent("VICE:dvsaBucket", false)
            currentTest = {
                active = false,
                ped = 0,
                vehicle = 0,
            ...
            local B = VICE.loadModel(i.test.examinerModel)
...
function handleTestRoute()
    ...
    while currentTest.active do
        if not C then
            ...
            local E = #(VICE.getPlayerCoords() - i.test.routes[currentTest.route][currentTest.waypoint + 1].coords)
            ...
            if E < 3.5 then
                ...
                if currentTest.waypoint >= #i.test.routes[currentTest.route] then
                    ...
                    if currentTest.blip ~= 0 then
                        tVICE.removeBlip(currentTest.blip)
                    end
                    if currentTest.marker ~= 0 then
                        tVICE.removeMarker(currentTest.marker)
                    end
                    returnToTestCentre()
                else
...
function policeChase()
    ...
    local I = VICE.spawnVehicle(G, H, 270.93, false, true)
    ...
    VICE.loadModel(M)
    local N = VICE.spawnVehicle(M, H, 270.93, false, true)
...
function returnToTestCentre()
    ...
    local T = false
    while true do
        local E = #(VICE.getPlayerCoords() - i.test.parkingSpaces[currentTest.parkingSpace].coords)
        if E < 15.0 then
...
function finishTest(S)
    ...
    if S then
        ...
        for r, s in pairs(currentTest.seriousReason) do
            ...
            VICE.notify("~r~Serious Fault~w~: " .. currentTest.seriousReason[r])
            Wait(500)
        end
        VICE.notifyPicture(i.images.dict, i.images.govLarge, "Your ~y~minor ~w~faults were:", "UK Government", "DVSA")
        Wait(3000)
        dvsaSound("minorFaults")
        for r, s in pairs(currentTest.minorsReason) do
            U = U .. ", " .. currentTest.minorsReason[r]
            VICE.notify("~y~Minor Fault~w~: " .. currentTest.minorsReason[r])
            Wait(500)
        end
        Wait(4000)
    ...
    if tVICE.teleport ~= nil then
        tVICE.teleport(i.test.finishTestTpCoords.x, i.test.finishTestTpCoords.y, i.test.finishTestTpCoords.z)
    end
    Wait(3000)
    SetTimeout(
        20000,
    ...
    TriggerServerEvent("VICE:dvsaBucket", true)
    if S then
        TriggerServerEvent("VICE:candidatePassed", currentTest.serious, currentTest.minors, U)
        currentTest.active = false
    else
        TriggerServerEvent("VICE:candidateFailed", currentTest.serious, currentTest.minors, V, U)
        currentTest.active = false
    end
    if currentTest.blip ~= 0 then
        tVICE.removeBlip(currentTest.blip)
...
function issueSerious(W)
    ...
    useTablet()
    if VICE.isDev() then
        VICE.notify("~r~Serious Fault~w~: " .. W)
    end
end
...
function pullUpOnRight()
    ...
    VICE.notifyPicture(i.images.dict, i.images.govLarge, "Listen carefully for directions", "Controlled Stop", "DVSA")
    local T = false
    local Y = false
...
function operateHeadlights()
    PlaySoundFrontend(-1, "Out_Of_Bounds_Timer", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 1)
    VICE.notifyPicture(i.images.dict, i.images.govLarge, "Listen carefully for directions", "Show Me Question", "DVSA")
...
function emergencyStop()
    ...
    VICE.notifyPicture(i.images.dict, i.images.govLarge, "Listen carefully for directions", "Test Instruction", "DVSA")
    currentTest.subtitle =
    ...
    VICE.notifyPicture(i.images.dict, i.images.govLarge, "Perform an emergency stop immediately", "STOP", "DVSA")
    Wait(3000)
    if not GetEntitySpeed(currentTest.vehicle) == 0.0 then
...
function useTablet()
    local B = VICE.loadModel("prop_cs_tablet")
    ...
    function()
                ...
                function()
                    ...
                    if VICE.isDev() or VICE.getUserId() == 2 then
                        RageUI.Button(
                            ...
                            "Since you are him, you can obtain a full licence",
                            {RightBadge = RageUI.BadgeStyle.Car},
                            true,
                            function(a8, a9, aa)
                                if aa then
                                    TriggerServerEvent("VICE:obtainFullLicence", source)
                                end
                            end--,
                            --RMenu:Get("dvsa", "tests")
                        )
                                    ...
                                    function(a8, a9, aa)
                                        if aa then
                                            if a.banned then
                                                ...
                                                if not currentTest.requested then
                                                    TriggerServerEvent("VICE:beginTest")
                                ...
                                RageUI.Button(
                                    ...
                                    function(a8, a9, aa)
                                        if aa then
                                            ...
                                            TriggerServerEvent("VICE:surrenderLicence")
                                        end
                                    end
                                )
                            end
                            ...
                            if a.active then
                                ...
                                if not f then
                                    RageUI.Button(
                                        ...
                                        function(a8, a9, aa)
                                            if aa then
                                                TriggerServerEvent("VICE:activateLicence")
                                                f = true
                                                VICE.notifyPicture(
                                                    i.images.dict,
                                                    i.images.govLarge,
        ...
        if RageUI.Visible(RMenu:Get("dvsa", "tests")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = true},
                function()
                    if c == nil or not next(c) then
                        RageUI.Separator("~r~You have no previous driving tests.")
                    end
    ...
    function()
        while true do
            ...
            if not a.full and not a.banned and not currentTest.active then
                local vehicle = VICE.getPlayerVehicle()
                if vehicle ~= 0 and GetPedInVehicleSeat(vehicle, -1) == VICE.getPlayerPed() then
...
function flashCamera(ai)
    ...
    TriggerServerEvent(
        "VICE:speedCameraFlashServer",
        math.ceil(GetEntitySpeed(GetVehiclePedIsIn(VICE.getPlayerPed(), false)) * 2.2369)
    )
...
function createPlate(vehicle)
    e.active = true
    e.vehicle = vehicle
    local B = VICE.loadModel("prop_lplate")
    local t = VICE.getPlayerCoords()
    ...
    VICE.notifyPicture(i.images.dict, i.images.lPlate, i.notifications.lPlatesAdded, "DVSA", "Licence Services")
    VICE.notifyPicture(i.images.dict, i.images.lPlate, i.notifications.lPlatesAdded2, "DVSA", "Licence Services")
end
function makeVehicleCrash(aq, J, H, L, ar)
    VICE.loadModel(aq)
    local I = VICE.spawnVehicle(aq, H, ar, true, true)
    while not DoesEntityExist(I) do
        Wait(0)
    end
