...
function func_snowballs()
    while true do
        ...
        if IsNextWeatherType("XMAS") then
            ...
            if IsControlJustReleased(0, 119) and not IsPedInAnyVehicle(PlayerPedId(), true) and not IsPlayerFreeAiming(PlayerId()) and not IsPedSwimming(PlayerPedId()) and not IsPedSwimmingUnderWater(PlayerPedId()) and not IsPedRagdoll(PlayerPedId()) and not IsPedFalling(PlayerPedId()) and not IsPedRunning(PlayerPedId()) and not IsPedSprinting(PlayerPedId()) and GetInteriorFromEntity(PlayerPedId()) == 0 and not IsPedShooting(PlayerPedId()) and not IsPedUsingAnyScenario(PlayerPedId()) and not IsPedInCover(PlayerPedId(), 0) then
                VICE.loadAnimDict("anim@mp_snowball")
                TaskPlayAnim(PlayerPedId(), "anim@mp_snowball", "pickup_snowball", 8.0, -1, -1, 0, 1, 0, 0, 0)
                RemoveAnimDict("anim@mp_snowball")
                Citizen.Wait(1950)
		        VICE.allowWeapon("WEAPON_SNOWBALL")
