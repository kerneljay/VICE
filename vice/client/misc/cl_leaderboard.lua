local a = {
    ...
    rows = {
        {"ZK", "~b~BLUE TEAM", "", false, 5, 2, 0, "1.4"},
        {"mkay", "~b~BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"VICE", "~r~RED TEAM", "", false, 5, 0, 2, "0.5"},
        {"VICE", "~b~BLUE TEAM", "", false, 5, 0, 2, "1.4"},
        {"ZK", "~b~BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"VICE", "~r~RED TEAM", "", false, 5, 2, 0, "0.5"},
        {"VICE", "~b~BLUE TEAM", "", false, 5, 2, 0, "1.4"},
        {"SEAN", "~b~BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"SEAN", "~r~RED TEAM", "", false, 5, 2, 0, "0.5"}
    }
}
RegisterCommand(
    "leaderboardtest",
    function()
        if VICE.isDev() then
            local b = tVICE.createLeaderboard(a)
            Wait(1000)
            tVICE.closeLeaderboard(b)
        end
    end,
    false
)
...
function tVICE.closeLeaderboard(b)
    c[b] = false
end
function tVICE.createLeaderboard(m)
    local n = Scaleform("SC_LEADERBOARD")
    n.RunFunction("SET_DISPLAY_TYPE", {1})
    n.RunFunction("SET_MULTIPLAYER_TITLE", {"VICE RP Event"})
...
function tVICE.updateScoreboard(u, v, w)
    local x
    ...
    if u == -1 then
        ...
        if x then
            ...
            print("error: VICE:updateScoreboard couldn't find row with values", u, v, w)
        end
    end
end
RegisterNetEvent(
    "VICE:updateScoreboard",
    function(u, v, w)
        tVICE.updateScoreboard(u, v, w)
    end
)
function tVICE.setPlayerCanOpenLeaderboard(F)
    e = F
end
function tVICE.clearLeaderboardData()
    d = {columns = {}, rows = {}}
end
function tVICE.isLeaderboardSetup()
    return table.count(d.rows) > 0
end
function tVICE.removePlayerFromLeaderboard(u)
