...
RMenu:Get("luckywheel", "casino"):SetSubtitle("You may only spin the wheel once per restart.")
RegisterNetEvent("VICE:frameworkCleanup")
AddEventHandler(
    "VICE:frameworkCleanup",
    function()
        DeleteEntity(b)
    end
)
        ...
        RageUI.IsVisible(
            ...
            function()
                RageUI.ButtonWithStyle(
                    "Spin the wheel of VICE (50,000 Chips)",
                    "",
                    {RightLabel = "→→→"},
                    true,
                    function(e, f, g)
                        if g then
                            TriggerServerEvent("VICE:requestSpinLuckyWheel")
                        end
                    end
                )
            end,
...
Citizen.CreateThread(
    function()
        while not insideDiamondCasino or decor == nil do
            Wait(1000)
        end
        local i = VICE.loadModel("vw_prop_vw_luckywheel_02a")
        local j = VICE.loadModel(c)
...
function goToWheel()
    if not d then
        ...
        local D = false
        while not D do
            local E = GetEntityCoords(VICE.getPlayerPed())
        ...
        SetEntityHeading(VICE.getPlayerPed(), 10.9754)
        Wait(2000)
    ...
    function(D, E)
        if E then
            ...
            for K, L in pairs(a) do
                tVICE.addBlip(L.entryPosition.x, L.entryPosition.y, L.entryPosition.z, 681, 0, "VICE Wheel", 0.7, true)
                VICE.createArea("vicewheel_" .. K, L.entryPosition, 1.5, 6, F, H, I, {})
            end
        end
    end
)
RegisterNetEvent("VICE:syncLuckyWheel")
AddEventHandler(
    "VICE:syncLuckyWheel",
    function(M)
        Wait(900)
...
RegisterNetEvent("VICE:spinLuckyWheel")
AddEventHandler(
    "VICE:spinLuckyWheel",
    function()
        goToWheel()
...
RegisterNetEvent("VICE:spinLuckyWheelReaction")
AddEventHandler(
    "VICE:spinLuckyWheelReaction",
    function(N)
        local y = VICE.getPlayerPed()
