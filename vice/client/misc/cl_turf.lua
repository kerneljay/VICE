...
local h = {
    ...
    -- {position = vector3(-1109.8720703125,4942.9711914062,222.70581054688), distance = 170.0}, MLO
	{position = vector3(-1113.4279785156,4936.0390625,218.38432312012), distance = 170.0},-- WITHOUT MLO
...
local i = {
    ...
    {position = vector3(-1109.9168701172,4934.8041992188,218.34832763672), commissionRate = 0},
...
Citizen.CreateThread(function()
	local j = tVICE.addBlip(-552.90362548828, 5326.1733398438, 72.599586486816, 403, 3, "Meth Trader")
	local k = tVICE.addBlip(133.73462677002, -1305.0340576172, 29.268741607666, 403, 4, "Cocaine Trader")
	local l = tVICE.addBlip(103.47916412354, -1937.1091308594, 19.803705215454, 140, 2, "Weed Trader")
	local m = tVICE.addBlip(3569.8334960938, 3662.5893554688, 33.920593261718, 586, 1, "Heroin Trader")
	local n = tVICE.addBlip(-1118.4926757813, 4926.1889648438, 218.35691833496, 150, 1, "Large Arms Trader")
	local o = tVICE.addBlip(5065.6201171875, -4591.3857421875, 2.8652405738831, 150, 1, "Large Arms Trader")
	local p = tVICE.addBlip(1317.0300292969, 4309.8359375, 38.005485534668, 499, 8, "LSD North Trader", 0.8)
	local q = tVICE.addBlip(2485.7429199219, -430.13989257813, 92.992835998535, 499, 8, "LSD South Trader", 0.8)
	local r = {
		{
			title = "Weed Trader",
			type = "radius",
...
AddEventHandler("VICE:onClientSpawn",function(v, w)
	if w then
		TriggerServerEvent("VICE:refreshTurfOwnershipData")
		TriggerServerEvent("VICE:requestDrugPriceUpdate")
	end
end)
RegisterNetEvent("VICE:attackGangCapture",function(x, y)
	f = true
	a = y
	otherMembersStartGangCapture(x, y, h[x].position)
end)
RegisterNetEvent("VICE:updateTurfOwner",function(x, z)
	turfData[x].gangOwner = z
end)
RegisterNetEvent("VICE:defendGangCapture",function(x, y)
	a = y
	e = true
...
RegisterNetEvent("VICE:turfDefenseInterrupt",function()
	e = false
end)
RegisterNetEvent("VICE:initiateGangCaptureCheck",function(y, C)
	b[y] = C
end)
RegisterNetEvent("VICE:gotTurfOwnershipData",function(D)
...
function VICE.isPlayerInTurf()
    for E, F in pairs(h) do
        if #(VICE.getPlayerCoords() - F.position) < F.distance then
            return true
        end
    end
    return false
...
Citizen.CreateThread(function()
	for B, G in pairs(h) do
		tVICE.addMarker(G.position.x,G.position.y,G.position.z,0.7,0.7,0.5,200,50,0,200,50,24,false,false,true)
	end
	for H, I in pairs(i) do
		tVICE.addMarker(I.position.x,I.position.y,I.position.z,0.7,0.7,0.5,0,50,200,200,50,30,false,false,true)
	end
end)
local J = -1
	...
	if w then
		...
		local O = function(M)
			if IsControlJustPressed(1, 51) then
				if not PlayerIsInGang then
					VICE.notify("~r~You need to be in a gang to capture a turf!")
				elseif not f then
					-- Check if player has a weapon in hand
					local ped = PlayerPedId()
					local currentWeapon = GetSelectedPedWeapon(ped)
					if currentWeapon == GetHashKey("WEAPON_UNARMED") then
						VICE.notify("~r~You need a weapon in hand to capture a turf!")
						return
					end
					TriggerServerEvent("VICE:refreshTurfOwnershipData")
					TriggerServerEvent("VICE:checkTurfCapture", M.turfId)
				end
				Wait(10000)
			end
		end
		for B, P in pairs(h) do
			VICE.createArea("turf_" .. B, P.position, 1.5, 6, L, N, O, {turfId = B})
		end
		local Q = function(M)
		...
		local S = function(M)
			if M.draw then
				...
				DrawAdvancedText(0.578, 0.349, 0.005, 0.0028, 0.437, "VICE Trader Manager", 255, 255, 255, 255, 4, 0)
						...
						if T and tonumber(T) then
							...
							if T <= 25 and T >= 0 then
								...
								if GetGameTimer() > K + 30000 then
									...
									if B == 1 then
										TriggerServerEvent("VICE:setNewWeedPrice", T)
									elseif B == 2 then
										TriggerServerEvent("VICE:setNewCocainePrice", T)
									elseif B == 3 then
										TriggerServerEvent("VICE:setNewMethPrice", T)
									elseif B == 4 then
										TriggerServerEvent("VICE:setNewHeroinPrice", T)
									elseif B == 5 then
										TriggerServerEvent("VICE:setNewLargeArmsCommission", T)
									elseif B == 6 then
										TriggerServerEvent("VICE:setNewLSDNorthPrice", T)
									elseif B == 7 then
										TriggerServerEvent("VICE:setNewLSDSouthPrice", T)
									end
								else
									VICE.notify("Rate limit, please wait 30s to change the commission.")
								end
							end
						end
					end
		...
		for B, G in pairs(i) do
			VICE.createArea("turfmanager_" .. B, G.position, 1.5, 6, Q, R, S, {turfId = B, draw = false})
		end
	end
end)
RegisterNetEvent("VICE:captureOwnershipReturned",function(B, U, V)
	if not U then
		startGangCapture(B, h[B].position, V)
	else
		VICE.notify("Turf already owned!")
	end
end)
function otherMembersStartGangCapture(x, y, W)
    if b[y] then
        ...
        Citizen.CreateThread(function()
			local tmpMarker = tVICE.addMarker(W.x, W.y, W.z - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
			while f do
				local X = GetEntityCoords(VICE.getPlayerPed(), false)
				local Y = #(X - W)
				if Y >= 37.5 then
					VICE.notify("Turf capture cancelled, you got too far!")
					f = false
					TriggerServerEvent("VICE:gangDefenseLocationUpdate", y, "Attackers", true)
				elseif GetEntityHealth(VICE.getPlayerPed()) <= 102 then
					VICE.notify("Turf capture failed, you died")
					f = false
					TriggerServerEvent("VICE:gangDefenseLocationUpdate", y, "Attackers", true)
				end
				Wait(1000)
			end
			tVICE.removeMarker(tmpMarker)
		end)
        Citizen.CreateThread(function()
			while f do
				if not d[y] then
					DrawAdvancedText(0.931,0.934,0.005,0.0028,0.49,"TIME REMAINING TILL CAPTURE: " .. timer .. " SECONDS!",23,255,141,255,VICE.getFontId("Akrobat-ExtraBold"),0)
				else
					DrawAdvancedText(0.931,0.954,0.005,0.0028,0.49,"CAPTURE BLOCKED, KILL ENIMES TO CONTINUE TIMER.",255,23,141,255,VICE.getFontId("Akrobat-ExtraBold"),0)
				end
				Wait(0)
			end
		end)
...
function startGangCapture(x, W, y)
    if turfData[x].owned then
        VICE.notify("You already own this turf!")
        TriggerServerEvent("VICE:failCaptureTurfOwned", x)
    else
        TriggerServerEvent("VICE:initiateGangCapture", x, y)
        local timer = g
        while b[y] == nil do
            Wait(0)
        end
        if b[y] then
            ...
            Citizen.CreateThread(function()
				tmpMarker = tVICE.addMarker(W.x, W.y, W.z - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
				while f do
					local X = GetEntityCoords(VICE.getPlayerPed(), false)
					local Y = #(X - W)
					if Y >= 37.5 then
...
function defendGangCapture(x, a, Z, _, a0)
    ...
    Citizen.CreateThread(function()
		local tmpMarker = tVICE.addMarker(Z, _, a0 - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
		while e do
			local X = GetEntityCoords(VICE.getPlayerPed(), false)
			local Y = #(X - vector3(h[x].x, h[x].y, h[x].z))
			if Y < 37.5 and GetEntityHealth(VICE.getPlayerPed()) > 102 then
				TriggerServerEvent("VICE:gangDefenseLocationUpdate", a, "Defenders", true)
			elseif GetEntityHealth(VICE.getPlayerPed()) > 102 then
				TriggerServerEvent("VICE:gangDefenseLocationUpdate", a, "Defenders", false)
			elseif GetEntityHealth(VICE.getPlayerPed()) <= 102 then
				VICE.notify("Turf defense failed, you died")
				e = false
				TriggerServerEvent("VICE:gangDefenseLocationUpdate", a, "Defenders", false)
			else
				TriggerServerEvent("VICE:gangDefenseLocationUpdate", a, "Defenders", false)
...
RegisterNetEvent("VICE:setBlockedStatus",function(y, a1)
	d[y] = a1
end)
RegisterNetEvent("VICE:gangDefenseSuccess",function()
	e = false
end)
function gangCaptureSuccess(x, a)
    turfData[x].owned = true
    TriggerServerEvent("VICE:gangCaptureSuccess", a)
end
function gangDefenseSuccess()
    TriggerServerEvent("VICE:gangDefenseSuccess", a)
end
