...
Citizen.CreateThread(
    function()
        for d, e in pairs(a.peds) do
            tVICE.createDynamicPed(e.modelHash, e.position, e.heading, true, e.animDict, e.animName, 30, false, false)
            tVICE.addBlip(e.position.x, e.position.y, e.position.z, 357, 81, "Vehicle Impound", 0.8, false)
        end
        Wait(2000)
        local f = function()
            TriggerServerEvent("VICE:getImpoundedVehicles")
            RageUI.Visible(RMenu:Get("viceimpound", "main"), true)
        end
        local g = function()
            RageUI.CloseAll()
        end
        ...
        for d, e in pairs(a.peds) do
            VICE.createArea("vehicle_impound_" .. d, e.position, 3.0, 6, f, g, h, {})
        end
    end
)
RMenu.Add(
...
RMenu.Add("viceimpound", "vehicle_information", RageUI.CreateSubMenu(RMenu:Get("viceimpound", "main")))
RMenu.Add("viceimpound", "pay_fine_confirmation", RageUI.CreateSubMenu(RMenu:Get("viceimpound", "vehicle_information")))
local i = 0
RageUI.CreateWhile(
    1.0,
    RMenu:Get("viceimpound", "reasons"),
    function()
                ...
                if c and c ~= {} then
                    for p, q in pairs(c) do
                        RageUI.ButtonWithStyle(
                            ...
                            RMenu:Get("viceimpound", "vehicle_information")
                        )
                    end
                else
            ...
            function()
                if i ~= 0 then
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        RMenu:Get("viceimpound", "pay_fine_confirmation")
                    )
                end
            end,
            function()
            ...
            function()
                if i ~= 0 then
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(k, d, l)
                            if l then
                                TriggerServerEvent("VICE:releaseImpoundedVehicle", i.vehicle)
                            end
                        end,
                        RMenu:Get("viceimpound", "main")
                    )
...
RegisterNetEvent(
    ...
    function(x, u, y, z, A, B, C, D)
        ...
        if O ~= vector3(0, 0, 0) and N ~= vector3(0, 0, 0) then
            VICE.loadModel("flatbed")
            VICE.loadModel("a_m_m_prolhost_01")
            P = VICE.spawnVehicle("flatbed", O.x, O.y, O.z, M, false, true, true)
            ...
            TriggerServerEvent(
                "VICE:deleteImpoundEntities",
                x,
                NetworkGetNetworkIdFromEntity(G),
                NetworkGetNetworkIdFromEntity(F)
            )
            v = VICE.spawnVehicle(z, C.x, C.y, C.z, D, false, true, false)
            ...
            TriggerServerEvent("VICE:awaitTowTruckArrival", x, R, T)
...
function tVICE.impoundVehicleOptions(Z, _, x, a0)
    ...
    elseif a.disallowedVehicleClasses[a2] then
        ...
        TriggerServerEvent("VICE:fetchInfoForVehicleToImpound", Z, a1, x)
        RageUI.Visible(RMenu:Get("viceimpound", "reasons"), true)
    end
end
function tVICE.isVehicleImpounded(v)
    return c[v] ~= nil
end
...
exports("impound", tVICE.impoundVehicleOptions)
exports("isImpounded", tVICE.isVehicleImpounded)
