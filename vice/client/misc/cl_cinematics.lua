RMenu.Add(
    "mainmenu",
    "cinematic",
    RageUI.CreateMenu("", "", VICE.getRageUIMenuWidth(), VICE.getRageUIMenuHeight(), "menus", "cinematicmenu")
)
RMenu:Get("mainmenu", "cinematic"):SetSubtitle("VICE Cinematic Editor")
RMenu.Add(
    "load_scene",
    "cinematic",
...
RMenu:Get("load_scene", "cinematic"):SetSubtitle("Load Scene")
RMenu.Add(
    "scene_manager",
    "cinematic",
    RageUI.CreateSubMenu(
...
RMenu:Get("scene_manager", "cinematic"):SetSubtitle("S~w~cene Manager")
RMenu.Add(
    "camera_manager",
    "cinematic",
    RageUI.CreateSubMenu(
...
RMenu:Get("camera_manager", "cinematic"):SetSubtitle("C~w~amera Manager")
RMenu.Add(
...
RMenu:Get("screeneffect_manager", "cinematic"):SetSubtitle("S~w~creen Effect Manager")
RMenu.Add(
    "timecycle_manager",
    "cinematic",
    RageUI.CreateSubMenu(
...
RMenu:Get("timecycle_manager", "cinematic"):SetSubtitle("T~w~imecycle Effect Manager")
RMenu.Add(
    "shake_manager",
    "cinematic",
    RageUI.CreateSubMenu(
...
RMenu:Get("shake_manager", "cinematic"):SetSubtitle("S~w~hake Effect Manager")
RMenu.Add(
    "add_camera",
    "cinematic",
    RageUI.CreateSubMenu(
...
RMenu:Get("add_camera", "cinematic"):SetSubtitle("A~w~dd Camera")
RMenu.Add(
...
RMenu:Get("camera_focus_manager", "cinematic"):SetSubtitle("C~w~amera Focus")
RMenu.Add(
...
RMenu:Get("weather_time_manager", "cinematic"):SetSubtitle("T~w~ime/Weather Manager")
RMenu.Add(
    "dof_manager",
    "cinematic",
    RageUI.CreateSubMenu(
...
RMenu:Get("dof_manager", "cinematic"):SetSubtitle("D~w~epth of Field Manager")
local function a(b)
    RageUI.CloseAll()
...
Citizen.CreateThread(
    function()
        m = json.decode(GetResourceKvpString("vice_scenedata") or "{}")
    end
)
RegisterNetEvent(
    "VICE:openCinematicMenu",
    function()
        a(true)
    end
)
function VICE.createCinematicScene(P)
    m[P] = {}
    SetResourceKvp("vice_scenedata", json.encode(m))
    VICE.notify("~g~Scene created & saved.")
end
function tVICE.deleteCinematicScene(P)
    m[P] = nil
    SetResourceKvp("vice_scenedata", json.encode(m))
    VICE.notify("~g~Scene deleted & saved.")
end
function tVICE.createCamera(P, Q)
    if #m[P] == 0 then
        Q.transition = 100
    end
    table.insert(m[P], Q)
    ...
    SetResourceKvp("vice_scenedata", json.encode(m))
    VICE.notify("~g~Camera created & saved.")
end
function tVICE.modifyCamera(P, Q, R)
    m[P][R] = Q
    SetResourceKvp("vice_scenedata", json.encode(m))
    VICE.notify("~g~Camera modified & saved.")
end
function tVICE.deleteCamera(P, S)
    table.remove(m[P], S)
    SetResourceKvp("vice_scenedata", json.encode(m))
    VICE.notify("~g~Camera deleted & saved.")
end
function tVICE.modifyTransition(n, q, T)
    m[n][q].transition = tonumber(T)
    SetResourceKvp("vice_scenedata", json.encode(m))
    VICE.notify("~g~Transition change saved.")
end
function tVICE.modifyCameraBlending(n, q, U)
    m[n][q].blending = l[U]
    SetResourceKvp("vice_scenedata", json.encode(m))
    VICE.notify("~g~Camera Blending change saved.")
end
function tVICE.modifyScreeneffect(n, q, V)
    m[n][q].screeneffect = V
    SetResourceKvp("vice_scenedata", json.encode(m))
    VICE.notify("~g~Screeneffect change saved.")
end
function tVICE.modifyTimecycleEffect(n, q, W)
    m[n][q].timecycleEffect = W
    SetResourceKvp("vice_scenedata", json.encode(m))
    VICE.notify("~g~Timecycle Effect change saved.")
end
function VICE.setTimecycleIntensity(n, q, f)
    m[n][q].timecycleIntensity = f
    SetResourceKvp("vice_scenedata", json.encode(m))
    VICE.notify("~g~Timecycle intensity change saved.")
end
function tVICE.modifyShakeEffect(n, q, X)
    m[n][q].shake = X
    SetResourceKvp("vice_scenedata", json.encode(m))
    VICE.notify("~g~Shake change saved.")
end
function tVICE.modifyCameraType(n, q, type)
    m[n][q].type = type
    SetResourceKvp("vice_scenedata", json.encode(m))
    VICE.notify("~g~Camera type change saved.")
end
function tVICE.addCameraFocus(n, q, Y, Z)
    m[n][q].focusType = Y
    m[n][q].focusData = Z
    SetResourceKvp("vice_scenedata", json.encode(m))
    VICE.notify("~g~Follow player change saved.")
end
function tVICE.setCameraFov(n, q)
    m[n][q].fov = i[A]
    SetResourceKvp("vice_scenedata", json.encode(m))
    VICE.notify("~g~FOV change saved.")
end
    ...
    function()
        ...
        RageUI.IsVisible(
            ...
            function()
                ...
                if q and m[n][q] then
                    RMenu:Get("camera_manager", "cinematic"):SetSubtitle("" .. m[n][q].type)
                    RageUI.ButtonWithStyle(
                        "~r~Transition: " .. m[n][q].transition .. "ms",
                        "",
                        {RightLabel = "→→→"},
                        true,
                        function(a1, a2, a3)
                            if a3 then
                                ...
                                if tonumber(T) then
                                    tVICE.modifyTransition(n, q, T)
                                else
                                    VICE.notify("~r~Failed to change transition time.")
                                end
                            end
                        end,
                        RMenu:Get("camera_manager", "cinematic")
                    ...
                    RageUI.List(
                        ...
                        {},
                        true,
                        function(a1, a2, a3, a8)
                            if a8 ~= w then
                                w = a8
                                tVICE.modifyCameraType(n, q, j[w])
                            end
                        end,
                        function()
                        end,
                    ...
                    RageUI.List(
                        ...
                        function(a1, a2, a3, a8)
                            if a8 ~= x then
                                x = a8
                                tVICE.modifyCameraBlending(n, q, k[x])
                            end
                        end,
                        function()
                        end,
                        ...
                        function(a1, a2, a3, a8)
                            ...
                            if a8 ~= e then
                                ...
                                if m[n][q].timecycleEffect ~= "" then
                                    ...
                                    VICE.setTimecycleIntensity(n, q, f[e])
                                end
                            end
                        end,
                        function()
            ...
            function()
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(a1, a2, a3)
                        if a3 then
                            if o then
                                VICE.notify("~r~Can not add a camera whilst you are adding a camera.")
                                return
                            end
                            previewingCamera = nil
                            t = GetEntityCoords(PlayerPedId())
                ...
                for g = 1, #c do
                    RageUI.ButtonWithStyle(
                        ...
                        function(a1, a2, a3)
                            ...
                            if a3 then
                                tVICE.modifyScreeneffect(n, q, c[g])
                            end
                        end,
                        RMenu:Get("camera_manager", "cinematic")
                    )
                ...
                for g = 1, #d do
                    RageUI.ButtonWithStyle(
                        ...
                        function(a1, a2, a3)
                            ...
                            if a3 then
                                tVICE.modifyTimecycleEffect(n, q, d[g])
                            end
                        end,
                        RMenu:Get("camera_manager", "cinematic")
                    )
                ...
                for g = 1, #h do
                    RageUI.ButtonWithStyle(
                        ...
                        function(a1, a2, a3)
                            ...
                            if a3 then
                                tVICE.modifyShakeEffect(n, q, h[g])
                            end
                        end,
                        RMenu:Get("camera_manager", "cinematic")
                    )
        ...
        RageUI.IsVisible(
            ...
            function()
                ...
                RageUI.Separator("~r~Currently pointed at: " .. m[n][q].focusType .. " : " .. m[n][q].focusData)
                RageUI.ButtonWithStyle(
                    "Player",
                    "",
                    {RightLabel = "→→→"},
                    ...
                    function(a1, a2, a3)
                        if a3 then
                            ...
                            if a9 ~= "" then
                                tVICE.addCameraFocus(n, q, "player", a9)
                            end
                        end
                    end,
                    RMenu:Get("camera_manager", "cinematic")
                        ...
                        if a3 then
                            tVICE.clientPrompt(
                                ...
                                function(aa)
                                    ...
                                    tVICE.addCameraFocus(n, q, "coord", vector3(ad, ae, af))
                                end
                            )
                        end
                    end,
...
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            if (o or p) and noclipActive and VICE.getPlayerVehicle() ~= 0 then
                VICE.notify("~r~You may not noclip when in a vehicle.")
                VICE.toggleNoclip()
            end
            if o and noclipActive then
                drawNativeNotification("The red line is where the camera will be aiming.")
                ...
                local direction = RotationToDirection(Q.rotation)
                local lineEndPoint = vector3(Q.position.x + direction.x * 10, Q.position.y + direction.y * 10, Q.position.z + direction.z * 10)
                DrawLine(Q.position.x, Q.position.y, Q.position.z, lineEndPoint.x, lineEndPoint.y, lineEndPoint.z, 255, 0, 0, 255)
                if IsControlJustPressed(0, 22) then
                    tVICE.createCamera(n, Q)
                    VICE.toggleNoclip()
                    o = false
            ...
            if p and noclipActive then
                ...
                DrawLine(Q.position.x, Q.position.y, Q.position.z, lineEndPoint.x, lineEndPoint.y, lineEndPoint.z, 255, 0, 0, 255)
                if IsControlJustPressed(0, 22) then
                    tVICE.modifyCamera(n, Q, p)
                    VICE.toggleNoclip()
                    p = false
                    SetEntityCoords(PlayerPedId(), t)
                end
            end
            if previewingCamera then
                ...
                if a0.focusType ~= "" then
                    if a0.focusType == "player" then
                        ...
                        if aj then
                            ...
                            print("[VICE Cinematic] Failed to point cam at player, could not get entity.")
                        end
...
Citizen.CreateThread(
    function()
        while true do
            local al, am = VICE.getPlayerVehicle()
            ...
            if an ~= 0 and IsEntityAPed(an) and IsPedAPlayer(an) and not IsEntityVisible(an) then
                ...
                if ao ~= -1 then
                    ...
                    if ap > 0 then
                        local aq = VICE.clientGetUserIdFromSource(ap)
                        if not VICE.clientGetPlayerIsStaff(aq) then
