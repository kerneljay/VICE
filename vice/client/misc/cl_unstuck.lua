...
local function b(c, d, e)
    ...
    Citizen.CreateThread(function()
        ...
        if not g then
            ...
            Wait(500)
            DoScreenFadeIn(500)
            VICE.notifyPicture("CHAR_BLOCKED","CHAR_BLOCKED","Attempting to teleport to surface... \n\nIf you are still not on the surface please use /calladmin.","VICE","Utilities",nil,nil)
            TriggerServerEvent("VICE:unstuckSuccessful", d, e)
        else
            VICE.notifyPicture("CHAR_BLOCKED","CHAR_BLOCKED","You moved during the countdown so the teleportation was cancelled.","VICE","Utilities",nil,nil)
        end
        f = false
        g = false
    end)
...
RegisterCommand("unstuck",function(q, r, s)
    local h = PlayerPedId()
    local t, p = o()
    if not t or VICE.getPlayerCombatTimer() > 0 then
        VICE.notifyPicture("CHAR_BLOCKED","CHAR_BLOCKED","You are unable to use this right now. Use /calladmin if you still need assistance.","VICE","Utilities",nil,nil)
    else
        VICE.loadAnimDict("timetable@amanda@ig_4")
        TaskPlayAnim(h, "timetable@amanda@ig_4", "ig_4_base", 8.0, -8.0, -1, 0, 0.0, 0, 0, 0)
        RemoveAnimDict("timetable@amanda@ig_4")
        Wait(1000)
        b(45, VICE.getPlayerCoords(), p)
    end
end)
...
RegisterCommand("reset",function(q, r, s)
    local h = PlayerPedId()
    local t =
        VICE.getPlayerCombatTimer() > 0 or VICE.isPlayerInRedZone() or GetRoomKeyFromEntity(PlayerPedId()) ~= 0 or
        IsEntityInWater(h) or
        IsPedInAnyVehicle(h, false)
    if u < GetGameTimer() then
        if t then
            VICE.notifyPicture("CHAR_BLOCKED","CHAR_BLOCKED","You are unable to use this right now.","VICE","Utilities",nil,nil)
        else
            local v = VICE.getPlayerCoords() 
            tVICE.teleport(3094.446, -4811.093, 15.26162)
            Wait(500)
            tVICE.teleport(v.x, v.y, v.z)
            u = GetGameTimer() + 60000
        end
    else
        VICE.notifyPicture("CHAR_BLOCKED","CHAR_BLOCKED","You must wait 60 seconds before using this again.","VICE","Utilities",nil,nil)
    end
end)
