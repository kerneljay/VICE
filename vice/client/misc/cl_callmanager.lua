...
RegisterNetEvent("VICE:addEmergencyCall",function(o, p, q, r, s, t)
    local u = 0
    if t == "admin" and VICE.getStaffLevel() > 0 then
        -- if VICE.isNewPlayer() then
            VICE.notify("~b~Admin ticket received.")
        -- else
        --     VICE.notify("~b~Admin ticket received.")
        -- end
        PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
        local v = #(r - GetEntityCoords(VICE.getPlayerPed()))
        table.insert(d, {o, p, q, v, s, t, u})
        j()
        e[o] = r
    elseif t == "met" and VICE.globalOnPoliceDuty() then
        -- if VICE.isNewPlayer() then
            VICE.notify("~b~MET Police call received.")
        -- else
        --     VICE.notify("~b~MET Police call received.")
        -- end
        PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
        ...
        j()
        e[o] = r
    elseif t == "nhs" and VICE.globalNHSOnDuty() then
        -- if VICE.isNewPlayer() then
            VICE.notify("~b~NHS call received.")
        -- else
        --     VICE.notify("~b~NHS call received.")
        -- end
        PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
        ...
        j()
        e[o] = r
    elseif t == "aa" and VICE.globalOnAADuty() then
        -- if VICE.isNewPlayer() then
            VICE.notify("~b~AA call received.")
        -- else
        --     VICE.notify("~b~AA call received.")
        -- end
        PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
...
Citizen.CreateThread(
    function()
        while true do
            if a and VICE.getStaffLevel() > 0 then
                ...
                DrawAdvancedText(0.339, 0.522, 0.005, 0.0028, 0.61, "VICE CALL MANAGER", 255, 255, 255, 255, VICE.getFontId("Akrobat-ExtraBold"), 0)
                local w = 0
                ...
                if CursorInArea(0.4234, 0.4614, 0.9444, 0.9759) then
                    ...
                    if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                        PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                        if h then
                            TriggerServerEvent("VICE:TakeTicket", h)
                            a = not a
                            ...
                            if C then
                                TriggerServerEvent("VICE:TakeTicket", C)
                                a = not a
                                SetNewWaypoint(e[h].x, e[h].y)
                                TriggerEvent("VICE:showHUD", true)
                                globalHideEmergencyCallUI = false
                                ...
                                VICE.notify("~r~No calls available.")
                            end
                        end
                    end
                else
                ...
                -- if VICE.isNewPlayer() then
                --     drawNativeNotification("Press ~INPUT_ENTER_CHEAT_CODE~ to toggle the call manager.")
                -- end
            end
            Wait(0)
        end
...
Citizen.CreateThread(
    function()
        while true do
            if IsControlJustPressed(0, 243) and (VICE.getStaffLevel() > 0 or VICE.globalOnAADuty() or VICE.globalNHSOnDuty()  or VICE.globalOnPoliceDuty()) then
                a = not a
                if not a then
                    TriggerEvent("VICE:showHUD", true)
                    globalHideEmergencyCallUI = false
...
RegisterNetEvent("VICE:startPhoneAnim")
AddEventHandler("VICE:startPhoneAnim", function()
...
RegisterNetEvent("VICE:clearPhoneAnim")
AddEventHandler("VICE:clearPhoneAnim", function()
    local ped = PlayerPedId()
    if GetVehiclePedIsIn(ped, false) == 0 then
