...
local function c()
    if GetEntityHealth(VICE.getPlayerPed()) <= 102 then
        VICE.notify("~r~You try to move but you remember you're dead")
        return true
    elseif IsEntityDead(VICE.getPlayerPed()) then
        VICE.notify("~r~You try to move but you remember you're dead")
        return true
    else
        return false
...
RegisterCommand("bag",function(f, g)
    ...
    if DoesEntityExist(h) and VICE.canAnim() and not IsEntityDead(h) and d() then
...
function tVICE.FreezePlayerControls(variable)
...
function tVICE.globalSurrenderring(playerSrc)
    if VICE.isPlayerSurrendered() then
        return true
    else
        return false
    end
...
function surrender()
    local h = PlayerPedId()
    if DoesEntityExist(h) and VICE.canAnim() and not IsEntityDead(h) and (d() or isPlayerNearPrison()) then
        VICE.loadAnimDict("random@arrests")
        VICE.loadAnimDict("random@arrests@busted")
...
function VICE.isPlayerSurrendered()
...
function VICE.isPlayerSurrenderedNoProgressBar()
    return j()
end
local u
RegisterNetEvent(
    "VICE:startRobbingPlayer",
    function()
        u = GetGameTimer()
    end
)
RegisterNetEvent(
    "VICE:endRobbingPlayer",
    function()
        u = nil
    end
)
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if B then
                ...
                VICE.notify("Press ~r~[E]~w~ to take a picture!")
                B = true
            end
        end
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if y then
                ...
                Wait(120)
                VICE.notify("Press ~r~[E]~w~ to take a toke!")
                y = true
            end
        end
        while y do
            ...
            if IsControlJustPressed(0, 38) then
                ...
                Wait(100)
                VICE.notify("You take a huge rip!")
                Wait(7250)
            end
        end
    end,
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if v then
                ...
                Wait(120)
                VICE.notify("Press ~r~[E]~w~ to wave the wand.")
                v = true
            end
        end
...
RegisterCommand("eatpill", function(source, args, rawCommand)
    ...
    if DoesEntityExist(ped) and not IsEntityDead(ped) then
        ...
        if IsEntityPlayingAnim(ped, animDict, animName, 3) then
            TaskPlayAnim(ped, animDict, "exit", 3.0, 1.0, -1, 0, 0, false, false, false)
            ClearPedSecondaryTask(ped)
        else
            TaskPlayAnim(ped, animDict, animName, 3.0, 1.0, -1, 0, 0, false, false, false)
        end
            ...
            if
             ...
             then
                if not Y and globalNHSOnDuty then
                    ...
                    Y = false
                    DetachEntity(VICE.getPlayerPed(), false, false)
                    ClearPedSecondaryTask(VICE.getPlayerPed())
                    local a5 = GetClosestPlayer(3)
                    local a7 = GetPlayerServerId(a5)
                    if a7 ~= 0 then
                        TriggerServerEvent("VICE:animationSyncStop", a7)
                    end
                end
                Wait(250)
            end
...
RegisterNetEvent(
    ...
    function()
        ...
        SetPedMotionBlur(VICE.getPlayerPed(), true)
        SetPedMovementClipset(VICE.getPlayerPed(), "move_m@drunk@verydrunk", true)
        RemoveAnimSet("move_m@drunk@verydrunk")
        SetPedIsDrunk(VICE.getPlayerPed(), true)
        DoScreenFadeIn(1000)
        ClearPedSecondaryTask(VICE.getPlayerPed())
        DeleteObject(prop)
        Wait(60000)
        ...
        ResetPedMovementClipset(VICE.getPlayerPed(), 0)
        SetPedIsDrunk(VICE.getPlayerPed(), false)
        SetPedMotionBlur(VICE.getPlayerPed(), false)
        RemoveAnimDict(H)
    end
)
RegisterNetEvent(
...
RegisterNetEvent(
    ...
    function(ai)
        ...
        VICE.loadAnimDict("misshair_shop@hair_dressers")
        tVICE.setCanAnim(false)
        local e = PlayerPedId()
        ...
        VICE.loadModel("prop_clippers_01")
