RMenu.Add("VICEtattoos","mainMenu",RageUI.CreateMenu("","Tattoo Store",VICE.getRageUIMenuWidth(),VICE.getRageUIMenuHeight(),"shopui_title_tattoos2","shopui_title_tattoos2"))
...
AddEventHandler("VICE:onClientSpawn",function(D, E)
    if E then
        local F = function()
            ...
            c = true
            i = deepcopy(h)
            j = tVICE.getCustomization()
            RageUI.Visible(RMenu:Get("VICEtattoos", "mainMenu"), true)
        end
        local G = function()
            ...
            RageUI.Visible(RMenu:Get("VICEtattoos", "mainMenu"), false)
            tVICE.setCustomization(j)
            j = nil
            ClearPedDecorations(VICE.getPlayerPed())
            for H = 1, #i, 1 do
                AddPedDecorationFromHashes(VICE.getPlayerPed(),GetHashKey(i[H].collection),GetHashKey(e[i[H].collection][i[H].current].nameHash))
            end
            c = false
            h = {}
        end
        local I = function()
            ...
            if RageUI.Visible(RMenu:Get("VICEtattoos", "mainMenu")) then
        ...
        for B, J in pairs(f) do
            ...
            Wait(100)
            VICE.createArea("tattoParlour_" .. B, J, 1.5, 6, F, G, I)
            tVICE.addBlip(J.x, J.y, J.z, 75, 0, "Tattoo Parlour")
            tVICE.addMarker(J.x,J.y,J.z,0.6,0.6,0.6,10,255,81,170,50,9,false,false,true,"mptattoos1","tattoo_defend_safehouse",90.0,90.0,0.0)
        end
    end
end)
function deepcopy(K)
...
Citizen.CreateThread(function()
    ...
    while true do
        Wait(500)
        if tVICE.isInComa() and not P then
            P = true
        end
        if not tVICE.isInComa() and P then
            Wait(5000)
            TriggerServerEvent("VICE:getPlayerTattoos")
            P = false
        end
...
RegisterNetEvent("VICE:setTattoos",function(Q)
    Citizen.Wait(500)
    ...
    for H = 1, #R, 1 do
        ...
        AddPedDecorationFromHashes(VICE.getPlayerPed(),GetHashKey(R[H].collection),GetHashKey(e[R[H].collection][R[H].current].nameHash))
        table.insert(h, R[H])
    end
end)
                ...
                RageUI.List(T.name,get_num_tattoos(T.value),d[H].currentListIndex,"Tattoo Add/Removal Price: £1,000",{},true,function(U, V, W, X)
                    if V then
                        ...
                        if T.name == n.name then
                            local C = VICE.getPlayerPed()
                            if k then
                                if m and m["id"] <= #get_num_tattoos(T.value) and m["id"] >= 0 then
                                    X = m["id"]
                        ...
                        if c and not l and IsControlJustReleased(0, 178) then
                            ...
                            TriggerServerEvent("VICE:saveTattoos", i, 1000)
                        end
                    end
                    if W then
...
function drawTattoo(_, a0, a1)
    SetEntityHeading(VICE.getPlayerPed(), 297.7296)
    local C = VICE.getPlayerPed()
    local a2 = false
    if _ and a0 and a1 then
        if p then
            for H = 1, #i, 1 do
                if i[H] then
            ...
            if GetEntityModel(VICE.getPlayerPed()) == -1667301416 then
                VICE.loadCustomisationPreset("TattooMale")
            else
                VICE.loadCustomisationPreset("TattooFemale")
            end
            ClearPedDecorations(VICE.getPlayerPed())
            for H = 1, #i, 1 do
                AddPedDecorationFromHashes(VICE.getPlayerPed(),GetHashKey(i[H].collection),GetHashKey(e[i[H].collection][i[H].current].nameHash))
            end
            AddPedDecorationFromHashes(VICE.getPlayerPed(), GetHashKey(a0), GetHashKey(e[a0][_].nameHash))
            if not DoesCamExist(b) then
                b = CreateCam("DEFAULT_SCRIPTED_CAMERA", true)
                SetCamCoord(b, GetEntityCoords(VICE.getPlayerPed()))
                SetCamRot(b, 0.0, 0.0, 0.0)
                SetCamActive(b, true)
                RenderScriptCams(true, false, 0, true, true)
                SetCamCoord(b, GetEntityCoords(VICE.getPlayerPed()))
            end
            local S, a3, a4 = table.unpack(GetEntityCoords(VICE.getPlayerPed()))
