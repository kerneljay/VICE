...
function VICE.isPlayerRappeling()
    return k
end
function VICE.isPlayerInPoliceHeli()
    return l
end
local r = (a + b) * 0.5
local s = 0
...
local function X(Y)
    ...
    VICE.DrawText(0.45, 0.9, "Model: " .. _ .. "\nPlate: " .. a0, 0.55)
end
local function a1()
    while true do
        if l then
            local a2 = VICE.getPlayerPed()
            local heli = VICE.getPlayerVehicle()
            if IsHeliHighEnough(heli) then
                ...
                if
                    IsControlJustPressed(0, h) and
                        not (GetPedInVehicleSeat(heli, -1) == a2 or GetPedInVehicleSeat(heli, 0) == a2)
                 then
                    k = true
                    PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
        ...
        if m then
            ...
            n = RequestScaleformMovie("HELI_CAM")
            ...
            while m and not IsEntityDead(a2) and GetVehiclePedIsIn(a2) == heli and IsHeliHighEnough(heli) do
                ...
                if o then
                    ...
                    if ai - p >= 500 then
                        ...
                        TriggerServerEvent("VICE:updateSpotlight", ak)
                        p = ai
                    end
...
RegisterNetEvent(
    ...
    function(aw, ax)
        ...
        if az == 0 or az == VICE.getPlayerPed() then
            return
        end
