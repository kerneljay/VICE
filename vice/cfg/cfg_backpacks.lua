local cfg = {}

cfg.stores = {
	["JDSports"] = {
		_config = {130.56843566895,-1034.9497070313,28.555471420288,676,1,"JD Sports Bags","",true},
		["Gucci Bag (+20kg)"] = {5, 94, 0,60000,20},
		["Off White Bag (+15kg)"] = {5, 92, 0,40000,15},
		--["Green Hiking Bag (+40kg)"] = {5, 100, 1,120000,40},
		["Hunting Backpack (+35kg)"] = {5, 91, 0,100000,35},
	},
	["Rebel"] = {
		_config = {1540.7928466797,6332.4702148438,23.071977615356,676,1,"Rebel Backpacks","rebellicense.whitelisted",true},
		["Rebel Backpack (+70kg)"] = {5, 109, 0,250000,70},
	},
}

cfg.bags = {
	["offwhitebag"] = "Off White Bag (+15kg)",
	["guccibag"] = "Gucci Bag (+20kg)",
	["huntingbackpack"] = "Hunting Backpack (+35kg)",
	["rebelbackpack"] = "Rebel Backpack (+70kg)",
}

return cfg