...
function VICE.setArea(source,name,x,y,z,radius,height,cb_enter,cb_leave)
  local areas = client_areas[source] or {}
  client_areas[source] = areas

  areas[name] = {enter=cb_enter,leave=cb_leave}
  VICEclient.setArea(source,{name,x,y,z,radius,height})
end

-- delete a player area
function VICE.removeArea(source,name)
  -- delete remote area
  VICEclient.removeArea(source,{name})
