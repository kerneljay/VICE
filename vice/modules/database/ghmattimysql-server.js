! function(modules) {
    ...
    const server_server = new server(function() {
        const config = JSON.parse(LoadResourceFile(GetCurrentResourceName(), "config.json")),
            configFromString = function() {
                const connectionString = GetConvar("mysql_connection_string", "mysql://root@localhost/vice");
                let cfg = {};
    ...
    }, !0), onNet("syntax:request-data", () => {
        const src = source;
        emitNet("syntax:update-resource-data", src, server_server.profiler.profiles.resources), emitNet("syntax:update-time-data", src, server_server.profiler.profiles.executionTimes), emitNet("syntax:update-slow-queries", src, server_server.profiler.profiles.slowQueries)
    }), onNet("syntax:request-server-status", () => {
        const src = source;
        server_server.execute("SHOW GLOBAL STATUS", data => {
            emitNet("syntax:update-status", src, data)
        }, null, "syntax").then(([result, cb]) => {
            cb(result)
        }).catch(() => !1), server_server.execute("SHOW GLOBAL VARIABLES", data => {
            emitNet("syntax:update-variables", src, data)
        }, null, "syntax").then(([result, cb]) => {
            cb(result)
        }).catch(() => !1)
    })
