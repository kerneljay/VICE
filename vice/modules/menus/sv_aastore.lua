...
RegisterNetEvent("VICE:BuyAAStoreItem")
AddEventHandler("VICE:BuyAAStoreItem", function(item, amount)
    local source = source
    ...
    for k,v in pairs(cfg.shopAAItems) do
        if item == v.itemID then
            if VICE.hasPermission(user_id, "aa.menu") then
                ...
                VICE.notify(source, '~r~You are not on duty as an AA Mechanic, Head to the city hall to clock on.')
                TriggerClientEvent("vice:PlaySound", source, "playCasinoLose")
            end
        end
    end
