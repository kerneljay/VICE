...
RegisterNetEvent("VICE:attemptBeginDeliverooJob")
AddEventHandler("VICE:attemptBeginDeliverooJob",function()
    local source=source
    local user_id=VICE.getUserId(source)
    local deliverooJobTable={}
    if VICE.hasGroup(user_id,"Deliveroo")then
        if not usersInDeliverooJob[user_id] then
            ...
            TriggerClientEvent("VICE:beginDeliverooJob",source)
            Wait(100)
            TriggerClientEvent("VICE:deliverooJobSetNextBlip",source,deliverooStops[deliverooJobTable.stopNumber].stopPosition)
        else
            VICE.notify(source, "You are already a Deliveroo Driver.")
        end
    else
        VICE.notify(source, "~r~You are not clocked on as a Deliveroo Driver.")
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(deliverooStops)do
                    if VICE.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(VICE.getUserSource(k)))-deliverooStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            VICE.giveBankMoney(k,pay)
                            TriggerClientEvent("VICE:deliverooJobReachedNextStop",VICE.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#deliverooStops then
                                TriggerClientEvent("VICE:deliverooJobEnd", VICE.getUserSource(k), true)
                                v.currentJob.jobActive=false
                                usersInDeliverooJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("VICE:deliverooJobSetNextBlip",VICE.getUserSource(k),deliverooStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
