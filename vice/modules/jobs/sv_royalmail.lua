...
RegisterNetEvent("VICE:attemptBeginRoyalMailJob")
AddEventHandler("VICE:attemptBeginRoyalMailJob",function()
    local source=source
    local user_id=VICE.getUserId(source)
    local RoyalMailJobTable={}
    if VICE.hasGroup(user_id,"Royal Mail Driver")then
        if not usersInRoyalMailJob[user_id] then
            ...
            TriggerClientEvent("VICE:beginRoyalMailJob",source)
            Wait(100)
            TriggerClientEvent("VICE:royalMailJobSetNextBlip",source,RoyalMailDrops[RoyalMailJobTable.stopNumber].dropPosition)
        else
            VICE.notify(source, "~r~You are already a Royal Mail Driver.")
        end
    else
        VICE.notify(source, "~r~You are not clocked on as a Royal Mail Driver.")
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(RoyalMailDrops)do
                    if VICE.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(VICE.getUserSource(k)))-RoyalMailDrops[v.currentJob.stopNumber].dropPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            VICE.giveBankMoney(k,pay)
                            TriggerClientEvent("VICE:royalMailReachedNextStop",VICE.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition,pay)
                            if v.currentJob.stopNumber==#RoyalMailDrops then
                                TriggerClientEvent("VICE:royalMailJobEnd", VICE.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInRoyalMailJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("VICE:royalMailJobSetNextBlip",VICE.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition)
                            end
                        end
                    end
                end
