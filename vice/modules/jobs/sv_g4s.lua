...
RegisterNetEvent("VICE:toggleShiftG4S")
AddEventHandler("VICE:toggleShiftG4S", function(Status)
    local source=source
    local user_id=VICE.getUserId(source)
    if VICE.hasGroup(user_id,"G4S Driver")then
        if Status then
            currentUsersOnG4SJob[user_id]={currentJob=""}
            TriggerClientEvent("VICE:startShiftG4S",source,math.random(1000,9999))
            jobsCompleted[user_id] = 0
            Wait(10000)
            TriggerClientEvent("VICE:updateJobInformation",source,GetCollection(user_id))
        else
            if currentUsersOnG4SJob[user_id] then
                currentUsersOnG4SJob[user_id] = nil
                jobsCompleted[user_id] = nil
                TriggerClientEvent("VICE:endShiftG4S",source)
            end
        end
    else
        VICE.notify(source, "~r~You are not clocked on as a G4S Driver.")
    end
end)

RegisterNetEvent("VICE:acceptJob")
AddEventHandler("VICE:acceptJob",function()
    local source=source
    local user_id=VICE.getUserId(source)
    if VICE.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
            if k == user_id then    
                ...
                TriggerClientEvent("VICE:updateJobInformation",source,v.currentJob)
            end
        end
    else
        VICE.notify(source, "~r~You are not clocked on as currentUsersOnG4SJob G4S Driver.")
    end
end)

RegisterNetEvent("VICE:updateMoneyInVehicle")
AddEventHandler("VICE:updateMoneyInVehicle", function()
    local source=source
    local user_id=VICE.getUserId(source)
    if VICE.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
...
RegisterNetEvent("VICE:updateMoneyOutVehicle")
AddEventHandler("VICE:updateMoneyOutVehicle", function()
    local source=source
    local user_id=VICE.getUserId(source)
    if VICE.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
            ...
            if v.currentJob.jobActive then
                if VICE.getUserSource(k)then
                    if not v.currentJob.collected then
                        if #(GetEntityCoords(GetPlayerPed(VICE.getUserSource(k)))-v.currentJob.collectionCoords)<1.5 then
                            v.currentJob.collected=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("VICE:updateJobInformation",VICE.getUserSource(k),v.currentJob)
                            TriggerClientEvent("VICE:requestMoneyInVehicle",VICE.getUserSource(k))
                        end
                    elseif v.currentJob.moneyInVehicle and not v.currentJob.depositing and not v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(VICE.getUserSource(k)))-v.currentJob.depositCoords)<20 then
                            v.currentJob.depositing=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("VICE:updateJobInformation",VICE.getUserSource(k),v.currentJob)
                            TriggerClientEvent("VICE:requestMoneyOutVehicle",VICE.getUserSource(k))
                        end
                    elseif not v.currentJob.deposited and v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(VICE.getUserSource(k)))-v.currentJob.depositCoords)<1.5 then
                            v.currentJob.deposited=true
                            v.currentJob.depositing=false
                            TriggerClientEvent("VICE:updateJobInformation",VICE.getUserSource(k),GetCollection(k))
                            local pay = grindBoost*math.random(8000,12000)
                            VICE.giveBankMoney(k,pay)
                        end
                    end
                end
            end
...
RegisterCommand('g4s', function(source, args)
    ...
    for k,v in pairs(currentUsersOnG4SJob) do
        if k == user_id then
            TriggerClientEvent("VICE:openG4SMenu", source)
        end
    end
end)