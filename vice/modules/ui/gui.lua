...
function VICE.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  VICEclient.openMenuData(source,{menudata})
end

-- force close player menu
function VICE.closeMenu(source)
  VICEclient.closeMenu(source,{})
end
...
function VICE.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result
  VICEclient.prompt(source,{title,default_text})
end
...
function VICE.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  VICEclient.request(source,{id,text,time}) -- send request to client
...
function VICE.registerMenuBuilder(name, builder)
...
function VICE.buildMenu(name, data, cbr)
...
function VICE.openMainMenu(source)
  VICE.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    VICE.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tVICE.closeMenu(id)
...
function tVICE.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tVICE.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tVICE.requestResult(id,ok)
...
function tVICE.openMainMenu()
  VICE.openMainMenu(source)
end
...
function VICE.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for VICE.addStaticMenuChoices calls
...
AddEventHandler("VICE:onServerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    VICEclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
