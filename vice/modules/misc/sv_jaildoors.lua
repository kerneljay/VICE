...
AddEventHandler("VICE:onServerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
        TriggerClientEvent('VICE:policeSyncAllDoors', source, doors, doorStates)
    end
end)

RegisterNetEvent("VICE:policeSetDoorState")
AddEventHandler("VICE:policeSetDoorState", function(doorHash, state)
    local source = source
    local user_id = VICE.getUserId(source)
    if VICE.hasPermission(user_id, 'police.armoury') then
        doorStates[doorHash] = state
        TriggerClientEvent('VICE:policeSyncDoor', -1, doorHash, state)
    end
end)

RegisterNetEvent("VICE:enterPoliceSyncDoors")
AddEventHandler("VICE:enterPoliceSyncDoors", function()
    local source = source
    TriggerClientEvent('VICE:gotAllDoorStates', source, doorStates)
end)