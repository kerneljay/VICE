    ...
    if source then
        ...
        if chairId > 7 then
            if VICE.hasGroup(user_id,"Highroller") then
                ...
                VICE.notify(source, '~r~You need to be a Highroller to sit at this table.')
                return
            end
        end
        if chairId <= 7 then
...
AddEventHandler("Blackjack:setBlackjackBet",function(gameId,betAmount,chairId)
    ...
    local user_id = VICE.getUserId(source)
    local chips = nil
    if gameId and betAmount and chairId then 
                    ...
                    MySQL.query("casinochips/get_chips", {user_id = user_id}, function(rows, affected)
                        ...
                        if chips >= betAmount then
                            if chairId < 8 and betAmount > 100000 then
                                VICE.notify(source, 'Maximum bet at this table is £100,000.')
                                return
                            end
                            ...
                            VICE.sendDCLog('blackjack-bet',"VICE Blackjack Logs", "> Player Name: **"..VICE.getPlayerName(user_id).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Amount: **"..getMoneyStringFormatted(tostring(betAmount)).."**")
                        else 
...
AddEventHandler("Blackjack:standBlackjack",function(gameId,nextCardCount)
    local source = source
    if source and gameId and nextCardCount then 
...
for i=0,31,1 do
    Citizen.CreateThread(function()
        local seed = math.floor(os.clock() * 100000000000)
                        ...
                        for cardIndex=0,1,1 do
                            for chairID=chairIdInitial,chairIdFinal do
                                if blackjackTables[chairID] ~= false then
                                    ...
                                    if blackjackGameData[gameId][source][1] then 
                                            ...
                                            if blackjackGameData[gameId][source][2][nextCardCount] == true then --if hit 
                                                ...
                                                if currentHand > 21 then
                                                    ...
                                                    if lostAmount > 10000000 then
                                                        TriggerClientEvent('chatMessage', -1, "^7Diamond Casino |", { 0, 255, 0 }, "^7"..VICE.getPlayerName(VICE.getUserId(source)).."^7 has LOST "..tostring(getMoneyStringFormatted(lostAmount)).." chips!", "alert")
                                                        VICE.sendDCLog('blackjack-bet',"VICE Blackjack Logs", "> Player Name: **"..VICE.getPlayerName(VICE.getUserId(source)).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..VICE.getUserId(source).."**\n> Amount: **"..getMoneyStringFormatted(tostring(lostAmount)).."**\n> Outcome: **Lost (Bust)**")
                                                    end
                                                    ...
                                                    if playerPing then
                                                        if playerPing > 0 then
                                                            ...
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', -1, "^7Diamond Casino |", { 0, 255, 0 }, "^7"..VICE.getPlayerName(VICE.getUserId(source)).."^7 has WON "..tostring(getMoneyStringFormatted(potentialWinAmount)).." chips!", "goodalert")
                                                                VICE.sendDCLog('blackjack-bet',"VICE Blackjack Logs", "> Player Name: **"..VICE.getPlayerName(VICE.getUserId(source)).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..VICE.getUserId(source).."**\n> Amount: **"..getMoneyStringFormatted(tostring(potentialWinAmount)).."**\n> Outcome: **Won (Dealer Busts)**")
                                                            end
                                                        end
                                                    end
                                                ...
                                                elseif currentHand == dealerHand then
                                                    ...
                                                    if playerPing then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("blackjack:notify",source,"~o~You tied with the dealer.")
                                                            VICE.sendDCLog('blackjack-bet',"VICE Blackjack Logs", "> Player Name: **"..VICE.getPlayerName(VICE.getUserId(source)).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..VICE.getUserId(source).."**\n> Amount: **"..getMoneyStringFormatted(tostring(potentialPushAmount)).."**\n> Outcome: **Pushed (Tied)**")
                                                        end
                                                    end
                                                else
                                                    if playerPing then
                                                        if playerPing > 0 then
                                                            ...
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', -1, "^7Diamond Casino |", { 255, 0, 0 }, "^7"..VICE.getPlayerName(VICE.getUserId(source)).."^7 has LOST "..tostring(getMoneyStringFormatted(potentialPushAmount)).." chips!", "alert")
                                                                VICE.sendDCLog('blackjack-bet',"VICE Blackjack Logs", "> Player Name: **"..VICE.getPlayerName(VICE.getUserId(source)).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..VICE.getUserId(source).."**\n> Amount: **"..getMoneyStringFormatted(tostring(potentialPushAmount)).."**\n> Outcome: **Lost (Lower Hand)**")
                                                            end
                                                        end
                                                    end
                                                end
...
function getCurrentHand(gameId,userId)
    if blackjackGameData[gameId][userId]["cardData"] then 
        local hand = 0
