...
RegisterNetEvent("VICE:BuyStoreItem")
AddEventHandler("VICE:BuyStoreItem", function(item, amount)
    local source = source
        ...
        if item == v.itemID then
            if VICE.getInventoryWeight(user_id) <= 25 then
                ...
                if item ~= "ticket" then
                    ...
                    VICE.notify(source, '~g~You have been entered into the lottery.')
                    TriggerEvent("VICE:setPersonalAmountBought", user_id, amount)
                    TriggerClientEvent("vice:PlaySound", source, "playMoney")
                end
            else
