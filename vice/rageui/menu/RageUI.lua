...
function RageUI.Visible(Menu, Value)
    ...
    if noclipActive and VICE.getStaffLevel() < 4 and Menu.Sprite.Dictionary ~= "new_editor" then
        return
    end
            ...
            if RageUI.CurrentMenu then
                if RageUI.CurrentMenu() then
                    ...
                    if Glare then
                        ...
                        -- Adjust GlareX slightly to the left to move it inside the banner
                        local adjustedGlareX = GlareX - 0.013 -- Subtract an intermediate value for fine-tuning

                        DrawScaleformMovie(ScaleformMovie, adjustedGlareX, GalreY, Glarewidth / 430, Glareheight / 100, 255, 255, 255, 255, 0)
                    end
...
function RageUI.Description()
    if RageUI.CurrentMenu and RageUI.CurrentMenu.Description then
        if RageUI.CurrentMenu() then
            RageUI.ItemsSafeZone(RageUI.CurrentMenu)
        ...
        if RageUI.CurrentMenu() then
            ...
            if RageUI.CurrentMenu.Controls.Back.Enabled and RageUI.CurrentMenu.Closable then
                if RageUI.CurrentMenu.Controls.Back.Pressed then
                    ...
                    if RageUI.CurrentMenu.Closed then
                        collectgarbage()
                        RageUI.CurrentMenu.Closed()
                    end
...
function RageUI.DrawContent(settings, items, panels)
    if (settings.header and settings.glare ~= nil) then
...
function RageUI.CreateWhile(wait, enabled, closure, type)
    table.insert(RageUI.Closures, {enabled = enabled, closure = closure, type = type})
end

Citizen.CreateThread(function()
    while true do
        for _, item in ipairs(RageUI.Closures) do
