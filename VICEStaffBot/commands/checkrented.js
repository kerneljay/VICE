...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    let count = 0
    fivemexports.vice.execute("SELECT * FROM `vice_user_vehicles` WHERE rentedid = ?", [params[0]], (result) => {
        if (result.length > 0) {
        var rentedvehicles = []
        ...
        for (i = 0; i < results; i++) {
            ...
            fivemexports.vice.execute("SELECT * FROM `vice_users` WHERE id = ?", [renter], (result) => {
                a = new Date()
