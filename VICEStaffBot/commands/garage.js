...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    let count = 0
    fivemexports.vice.execute("SELECT * FROM `vice_user_vehicles` WHERE user_id = ?", [params[0]], (result) => {
        var vehicles = []
        for (i = 0; i < result.length; i++) {
            let results =  result.length
            let vehicle = result[i]; // get the current vehicle
            fivemexports.vice.execute("SELECT * FROM vice_user_vehicles WHERE vehicle = ?", [vehicle.vehicle.toLowerCase()], (res) => {
