...
exports.runcmd = async(fivemexports, client, message, params) => {
    ...
    fivemexports.vice.execute("SELECT * FROM `vice_user_ids` WHERE user_id = ?", [params[0]], (discordid) => {
        ...
        fivemexports.vice.execute("SELECT * FROM vice_user_moneys WHERE user_id = ?", [params[0]], (result) => {
            if (result) {
                bank = result[0].bank
                wallet = result[0].wallet
                fivemexports.vice.execute("SELECT * FROM `vice_users` WHERE id = ?", [params[0]], (userdata) => {
                    if (userdata) {
                        ...
                        fivemexports.vice.execute("SELECT * FROM `vice_users` WHERE id = ?", [params[0]], (result) => {
                            if (result) {
