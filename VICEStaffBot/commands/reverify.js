...
exports.runcmd = async(fivemexports, client, message, params) => {
    ...
    if (!params[0] && !parseInt(params[0]) || !params[1] && !parseInt(params[1])) {
        ...
        fivemexports.vice.execute("UPDATE `vice_verification` SET discord_id = ?, verified = 1 WHERE user_id = ?", [params[1], params[0]], async (result) => {
            if (result) {
                let embed = {
                    "title": "Verify",
                ...
                await message.member.addRole("1481830175934251089").then().catch(console.error);
            }
        });
    }
}
