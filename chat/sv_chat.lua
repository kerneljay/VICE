...
local function routeMessage(source, author, message, mode, fromConsole)
    if source >= 1 then
        author = exports["vice"]:GetDiscordName(source)
    end
    ...
    if mode == "OOC" then
        TriggerEvent("VICE:ooc", source, message)
        return
    end
    if mode == "Gang" then 
    ...
    if mode == "Admin" then
        for k,v in ipairs(GetPlayers()) do
            if exports['vice']:hasPermission(v, "admin.tickets") then
    ...
    if not WasEventCanceled() then
		...
		for word in pairs(blockedWords) do
			if(string.gsub(string.gsub(string.gsub(string.gsub(string.gsub(string.gsub(message:lower(), "-", ""), ",", ""), "%.", ""), " ", ""), "*", ""), "+", ""):find(blockedWords[word])) then
				--TriggerClientEvent('VICE:chatFilterScaleform', source, 10, 'That word is not allowed.')
                TriggerClientEvent('VICE:chatFilterScaleform', source, 10, 'That word is not allowed.', author, user_id)
				CancelEvent()
				return
			end
		end
...
AddEventHandler('__cfx_internal:commandFallback', function(command)
    local name = exports["vice"]:GetDiscordName(source)
