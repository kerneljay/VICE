...
local function removePatternsFromString(content)
  ...
  for characterIndex = 1, #content do
    ...
    if specialSINCO[character] then
...
local function getPlayerNameSafe()
  if GetResourceState("vice") == "started" then
    local name = GetPlayerName(GetPlayerServerId(PlayerId()))
...
function containsMyName(args)
  local myName = string.lower(GetPlayerName(GetPlayerServerId(PlayerId())))
...
local function getUserIdSafe()
  if GetResourceState("vice") == "started" then
    local user_id = exports["vice"]:getUserId()
  ...
  if not hideChat then
    ...
    SendNUIMessage({
      ...
      message = {
        ...
        isPermMention = containsMyName(args),
        mode = modeName
      }
    })
  end
  ...
  for resIdx = 0, GetNumResources() - 1 do
    ...
    if GetResourceState(resource) == 'started' then
      ...
      if numThemes > 0 then
        ...
        local themeData = json.decode(GetResourceMetadata(resource, 'chat_theme_eamb') or 'null')
...
AddEventHandler("VICE:hideChat",function(flag)
  hideChat = flag
end)

AddEventHandler("VICE:chatReduceOpacity", function(flag)
